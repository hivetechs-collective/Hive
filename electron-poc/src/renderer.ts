  ...
  (event) => {
    ...
    if (
      ...
      (event.error instanceof Event ||
        event.error.constructor.name.includes("Event"))
    ) {
      ...
      <div class="activity-bar-unified">
        ...
        <div class="sidebar-bottom-section" style="position: absolute; bottom: 0; width: 100%;">
          ...
          <button class="activity-btn" data-view="documentation" aria-label="Documentation">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
              <path d="M14,2H6A2,2,0,0,0,4,4V20a2,2,0,0,0,2,2H18a2,2,0,0,0,2-2V8ZM15,3.5,18.5,7H15ZM18,20H6V4H13V8a1,1,0,0,0,1,1h4Z"/>
              <path d="M8,13h8v2H8Zm0,4h8v2H8Zm0-8h5v2H8Z"/>
            </svg>
            <span class="activity-tooltip">Documentation (Ctrl+/)</span>
          </button>
  ...
  if (isCurrentlyActive) {
    ...
    if (targetPanel) {
      ...
      if (panelType === "explorer") {
        ...
        if (container) {
          console.log(
            "Explorer panel activated, currentOpenedFolder:",
            currentOpenedFolder,
          );
          ...
          if (currentOpenedFolder) {
            console.log(
              "[Explorer Activation] currentOpenedFolder value:",
              currentOpenedFolder,
            );
            console.log(
              "[Explorer Activation] currentOpenedFolder type:",
              typeof currentOpenedFolder,
            );

            // Check if explorer exists and is showing the wrong folder
            const needsUpdate =
              window.fileExplorer &&
              window.fileExplorer.getCurrentPath() !== currentOpenedFolder;

            // If explorer doesn't exist OR container is empty OR showing wrong folder, create/update it
            if (
              !window.fileExplorer ||
              !container.querySelector(".explorer-folders-view") ||
              needsUpdate
            ) {
              if (needsUpdate) {
                console.log(
                  "Explorer showing wrong folder. Current:",
                  window.fileExplorer.getCurrentPath(),
                  "Should be:",
                  currentOpenedFolder,
                );
              }
              console.log(
                "Creating/updating file explorer for:",
                currentOpenedFolder,
              );
              ...
              console.log(
                "Explorer already showing correct folder:",
                currentOpenedFolder,
              );
            }
          } else {
            ...
            setTimeout(() => {
              ...
              const cloneRepoBtn = document.getElementById(
                "welcome-clone-repo-btn",
              );
              if (cloneRepoBtn) {
                cloneRepoBtn.addEventListener("click", () =>
                  window.cloneRepository(),
                );
              }
            }, 0);
          }
          ...
          const addFileBtn = document.querySelector(
            '.sidebar-actions button[title="New File"]',
          );
          const addFolderBtn = document.querySelector(
            '.sidebar-actions button[title="New Folder"]',
          );
          const refreshBtn = document.querySelector(
            '.sidebar-actions button[title="Refresh Explorer"]',
          );
          const collapseBtn = document.querySelector(
            '.sidebar-actions button[title="Collapse Folders in Explorer"]',
          );
        ...
        if (container) {
          ...
          if (!window.gitUI) {
            console.log(
              "Git panel activated, currentOpenedFolder:",
              currentOpenedFolder,
            );
            ...
            if (currentOpenedFolder && window.gitAPI) {
                ...
                .then(() => {
                  ...
                  setTimeout(() => {
                    try {
                      ...
                      console.log(
                        "Creating VSCodeSCMView for folder:",
                        currentOpenedFolder,
                      );
              ...
              try {
                ...
                console.error(
                  "Failed to create VSCodeSCMView (no folder):",
                  error,
                );
                console.error("Error stack:", error.stack);
              }
            ...
            console.log(
              "Git UI exists, updating to folder:",
              currentOpenedFolder,
            );
...
function addChatMessage(
  ...
  messageType: "info" | "success" | "warning" | "error" | "user" = "user",
) {
...
function updateStageStatus(
  ...
  if (stageElement) {
    const progressFill = stageElement.querySelector(
      ".progress-fill",
    ) as HTMLElement;
    ...
    switch (status) {
      case "ready":
        ...
        progressFill.style.background =
          "linear-gradient(90deg, var(--hive-yellow) 0%, var(--hive-blue) 100%)";
        addLogEntry(
          `🔄 ${stage.charAt(0).toUpperCase() + stage.slice(1)} stage ready`,
          "info",
        );
        break;
      case "running":
        ...
        progressFill.style.background =
          "linear-gradient(90deg, var(--terminal-warning) 0%, var(--terminal-info) 100%)";
        addLogEntry(
          `⚡ ${stage.charAt(0).toUpperCase() + stage.slice(1)} stage running...`,
          "info",
        );
        break;
      case "completed":
        ...
        progressFill.classList.remove("running");
        progressFill.style.background =
          "linear-gradient(90deg, var(--terminal-success) 0%, var(--terminal-info) 100%)";
        addLogEntry(
...
function openSettingsTab() {
  console.log(
    "[Settings] Opening settings as tab, editorTabs exists:",
    !!window.editorTabs,
  );
  ...
  if (window.editorTabs) {
    ...
    window.editorTabs.openCustomTab(
      "settings",
      "⚙️ Settings",
      settingsContent,
      {
...
document
  ...
  ?.addEventListener("click", async () => {
    ...
    try {
      ...
      addLogEntry(
        `⏱️  Response time: ${new Date(result.timestamp).toLocaleTimeString()}`,
        "info",
      );
    } catch (error) {
...
document
  .getElementById("run-consensus-btn")
  ?.addEventListener("click", async () => {
    if (isProcessing) {
    ...
    if (!currentConversationId) {
      ...
      addLogEntry(
        `📝 New conversation started: ${currentConversationId}`,
        "info",
      );
    }
    ...
    consensusWebSocket.startConsensus(
      testQuery,
      currentProfileName,
...
document
  .getElementById("consensus-settings-btn")
  ?.addEventListener("click", async () => {
    ...
    if (modal) {
      ...
      try {
        const profileName =
          localStorage.getItem("activeProfile") || "Free Also";
        const profile = await (window as any).settingsAPI.getProfile(
          profileName,
        );
...
document
  .getElementById("close-consensus-settings")
  ?.addEventListener("click", () => {
    const modal = document.getElementById("consensus-settings-modal");
...
document
  .getElementById("cancel-consensus-settings")
  ?.addEventListener("click", () => {
    const modal = document.getElementById("consensus-settings-modal");
    if (modal) modal.style.display = "none";
  });
...
document
  .getElementById("save-consensus-settings")
  ?.addEventListener("click", async () => {
    const maxRoundsInput = document.getElementById(
      "max-consensus-rounds",
    ) as HTMLInputElement;
    ...
    try {
      ...
      await (window as any).settingsAPI.updateProfileMaxRounds(
        profileName,
        maxRounds,
      );
...
document.getElementById("send-chat")?.addEventListener("click", async () => {
  ...
  if (query.startsWith("TEST:")) {
    try {
      const testResult = await (window as any).backendAPI.testClaudeDebug(
        query,
      );
  ...
  if (true) {
    // Force this path to always run for DirectConsensusEngine
    isProcessing = true;
      ...
      try {
        ...
        switch (message.type) {
          case "llm_started":
            ...
            updateModelDisplay(
              message.llm,
              `Round ${message.round}: ${message.model.split("/").pop() || message.model}`,
            );
            break;
    ...
    if (consensusAPI) {
      ...
      consensusAPI.onConsensusStatus((data: any) => {
        ...
        if (
          (data.consensusType === "routing" ||
            data.consensusType === "conversing" ||
            data.consensusType === "stage_running" ||
            data.consensusType === "curating") &&
          !escapeHandler
        ) {
          escapeHandler = (e: KeyboardEvent) => {
            if (e.key === "Escape") {
              ...
              const consensusTypeElement =
                document.getElementById("consensus-type");
        ...
        if (consensusTypeElement && data.consensusType) {
          ...
          if (
            data.achieved &&
            (data.consensusType === "unanimous" ||
              data.consensusType === "majority" ||
              data.consensusType === "curator_override")
          ) {
            consensusTypeElement.style.animation = "";
          ...
          if (data.achieved && displayText) {
            console.log(
              `✅ DISPLAYING FINAL CLASSIFICATION: "${displayText}" with color ${color}`,
            );
          }
        }
        ...
        (window as any).api.receive(
          ...
          (data: any) => {
            ...
            if (consensusTypeElement) {
              ...
              consensusTypeElement.style.color =
                data.classificationType === "unanimous"
                  ? "#4CAF50" // Green
                  : data.classificationType === "majority"
                    ? "#FFC107" // Yellow/Amber
                    : data.classificationType === "curator_override"
                      ? "#FF9800" // Orange
              ...
              console.log(
                `✅ DISPLAYED FINAL CLASSIFICATION: "${data.displayText}"`,
              );
            } else {
          ...
          if (consensusType) {
            ...
            if (consensusTypeElement) {
              ...
              if (displayText) {
                ...
                console.log(
                  `✅ CONSENSUS COMPLETE - DISPLAYING: "${displayText}" in ${color}`,
                );
              }
            }
          }
      ...
      consensusAPI.onVisualUpdate((data: any) => {
        ...
        addLogEntry(
          `Visual Update: ${data.type} - ${JSON.stringify(data)}`,
          "info",
        );
          ...
          case "stage-complete":
            ...
            if (data.tokens) {
              const tokenElement = document.querySelector(
                `.consensus-stage.${data.stage} .stage-tokens`,
              );
              if (tokenElement) {
      ...
      consensusAPI.onConsensusComplete((data: any) => {
        ...
        try {
          ...
          if (analyticsPanel && analyticsPanel.style.display !== "none") {
            try {
              (window as any).analyticsDashboard?.refresh?.();
            } catch {}
          }
        } catch (e) {
          console.error(
            "Failed to persist analytics on consensus-complete:",
            e,
          );
        }
      });
      ...
      consensusAPI.onRoundUpdate((data: any) => {
        ...
        roundIndicator.style.cssText =
          "color: #3794ff; font-weight: bold; margin: 10px 0;";
      ...
      consensusAPI.onStageUpdate((data: any) => {
        ...
        console.log(
          "📊 Neural Graphic status:",
          neuralConsciousness ? "exists" : "missing",
        );
        ...
        if (activeProfile && data.status === "running") {
          const modelName =
            activeProfile[`${data.stage}`] ||
            activeProfile[`${data.stage}_model`] ||
            "Model";
        ...
        if (data.status === "running") {
          ...
          if (neuralConsciousness) {
            ...
            const neuralPhase =
              data.stage === "route" ? "classification" : data.stage;
            neuralConsciousness.updatePhase(
              neuralPhase as
                | "classification"
                | "generator"
                | "refiner"
          ...
          if (progressIntervals.has(data.stage)) {
            ...
            console.log(
              `🔍 CLEARED existing interval for stage: ${data.stage}`,
            );
          }
      ...
      consensusAPI.onConsensusVoteUpdate((data: any) => {
        ...
        addLogEntry(
          `Consensus check - Generator: ${data.generator}, Refiner: ${data.refiner}, Validator: ${data.validator}`,
          "info",
        );
        // Display consensus opinions in chat
        const consensusDiv = document.createElement("div");
        consensusDiv.className = "consensus-check";
        consensusDiv.style.cssText =
          "background: #1e1e1e; padding: 10px; margin: 10px 0; border-left: 3px solid #3794ff;";
    ...
    try {
      ...
      if (!activeProfile || !activeProfile.generator) {
        console.error(
          "No active profile loaded! Cannot proceed with consensus.",
        );
        addChatMessage(
          "Error: No profile loaded. Please select a profile in settings.",
          false,
          "error",
        );
        return;
      ...
      console.log(
        "[AnimateStages] Waiting for real consensus engine updates...",
      );
  ...
  if (!consensusWebSocket || !consensusWebSocket.isConnected()) {
    ...
    setTimeout(() => {
      if (consensusWebSocket?.isConnected()) {
        ...
        addLogEntry(
          "WebSocket connection failed, using REST API fallback",
          "warning",
        );
  ...
  const currentProfileName = activeProfile?.name || "Balanced Performer";
  console.log(
    "[Consensus] Using profile for user query:",
    currentProfileName,
    "activeProfile:",
    activeProfile,
  );
  ...
  try {
    ...
    if (typeof consensusResponse === "string") {
      ...
      const message =
        ...
        JSON.stringify(consensusResponse);
    ...
    if (error instanceof TypeError && error.message === "Failed to fetch") {
      addLogEntry(
        `❌ Network error: Cannot connect to backend service`,
        "error",
      );
      addLogEntry(`💡 Make sure the backend server is running`, "warning");
      addChatMessage(
        `Network Error: Cannot reach the backend server. Please ensure it's running.`,
        true,
      );
    } else {
      const errorMessage =
...
async function initializeWebSocket() {
  ...
  consensusWebSocket = new ConsensusWebSocket(wsUrl, {
    ...
    onStageCompleted: (stage, tokens, cost) => {
      ...
      addLogEntry(
        `✅ ${stage} completed (${tokens} tokens, ${formatCost(cost)})`,
        "success",
      );
    },
    ...
    onConsensusComplete: (result, finalTokens, finalCost) => {
      ...
      if (result && result.trim()) {
        ...
        addLogEntry(
          `💾 Conversation context updated (${conversationHistory.length} messages)`,
          "info",
        );
      }
      ...
      addLogEntry(
        `🎯 Consensus complete! Total: ${formatTokens(finalTokens)} tokens, ${formatCost(finalCost)}`,
        "success",
      );
    ...
    onLLMStarted: (round: number, llm: string, model: string) => {
      ...
      updateModelDisplay(
        llm,
        `Round ${round}: ${model.split("/").pop() || model}`,
      );

      addLogEntry(
        `🤖 Round ${round}: ${llm.toUpperCase()} deliberating with ${model}`,
        "info",
      );
    },
    ...
    onTokenUpdate: (
      ...
      addLogEntry(
        `📊 Round ${round}: ${currentLLM.toUpperCase()} added tokens (Total: ${tokens})`,
        "info",
      );
    },
  });
  ...
  setTimeout(() => {
    ...
    if (consensusWebSocket.isConnected()) {
      ...
      addLogEntry(
        "⚠️ WebSocket not connected - messages will use REST API",
        "warning",
      );
    }
  }, 500);
...
function updateModelDisplay(stage: string, model: string) {
  ...
  if (modelElement) {
    ...
    const displayModel =
      model.length > 30 ? model.substring(0, 27) + "..." : model;
...
setTimeout(async () => {
  ...
  try {
    if ((window as any).backendAPI) {
      ...
      addLogEntry(
        `✅ Backend health check passed: ${health.service} v${health.version}`,
        "success",
      );
    } else {
      throw new Error("Backend API not available");
...
addLogEntry(
  "🔧 Click buttons above to test the Electron + Rust architecture",
  "info",
);
  ...
  if (currentOpenedFolder && window.gitAPI) {
    try {
      ...
      if (status && status.isRepo) {
        ...
        const getWorking = (f: any) =>
          (f.working_dir || f.working || " ") as string;
        const untracked = files.filter(
          (f) => getWorking(f) === "?" || f.index === "?",
        ).length;
        ...
        let countsSpan = branchElement!.querySelector(
          `.${countsClass}`,
        ) as HTMLElement | null;
        if (!countsSpan) {
        ...
        const mkCount = (
          ...
          const data = clickable
            ? `data-action="scroll" data-group="${group}"`
            : "";
        ...
        html += mkBadge(
          ...
          ahead,
          "push",
          ahead > 0 ? "Push (ahead)" : "Nothing to push",
        );
        html += mkBadge(
          "codicon-cloud-download",
          behind,
          "pull",
          behind > 0 ? "Pull (behind)" : "Up to date",
        );
        if (ahead > 0 && behind > 0) {
          html += mkBadge(
            "codicon-sync",
        ...
        if (
          repoRoot &&
          (window as any).currentOpenedFolder &&
          repoRoot !== (window as any).currentOpenedFolder
        ) {
          html += mkAction(
            "codicon-repo",
            `Switch Git to root: ${repoRoot}`,
            "switch-root",
          );
        }
        ...
        countsSpan.onclick = async (ev: MouseEvent) => {
          ...
          if (action === "push" && ahead > 0) {
            try {
              await (window as any).gitAPI.push();
            } catch (e) {
              console.error("Push failed:", e);
            }
          } else if (action === "pull" && behind > 0) {
            try {
              await (window as any).gitAPI.pull();
            } catch (e) {
              console.error("Pull failed:", e);
            }
          } else if (action === "sync" && ahead > 0 && behind > 0) {
            try {
              await (window as any).gitAPI.sync();
            } catch (e) {
              console.error("Sync failed:", e);
            }
          } else if (action === "scroll") {
            try {
              const group = target.getAttribute("data-group") as
                | "staged"
                | "changes"
                | "untracked"
                | null;
              if (group) {
                ...
                setTimeout(() => {
                  try {
                    (window as any).scmView?.scrollToGroup(group);
                  } catch {}
                }, 150);
              }
            } catch {}
        ...
        if (
          branchElement &&
          !branchElement.getAttribute("data-context-bound")
        ) {
          branchElement.addEventListener("contextmenu", (ev) => {
            ...
            try {
              ...
              if (
                ...
                menu.appendChild(
                  mkItem("Switch SCM to Git Root", async () => {
                    try {
                      await (window as any).gitAPI.setFolder(repoRoot);
                      await updateGitStatusBar();
                    } catch (e) {
                      console.error(e);
                    }
              ...
              menu.appendChild(
                mkItem("Use Opened Folder as SCM Root", async () => {
                  try {
                    if ((window as any).currentOpenedFolder) {
                      await (window as any).gitAPI.setFolder(
                        (window as any).currentOpenedFolder,
                      );
                      await updateGitStatusBar();
                    }
              ...
              if (repoRoot) {
                menu.appendChild(
                  mkItem("Reveal Git Root in Finder", async () => {
                    try {
                      await (window as any).electronAPI.openPath(repoRoot);
                    } catch (e) {
                      console.error(e);
                    }
                  }),
...
async function updateStatusBar() {
  try {
    ...
    const conversationsElement = document.getElementById(
    ...
    if (settings.hiveKey) {
      ...
      if (result.hiveValid && result.licenseInfo) {
        ...
        if (planElement) {
          ...
          const tier =
            (result.licenseInfo.tier || "Free").charAt(0).toUpperCase() +
        ...
        console.log(
          "D1 validation complete, will fetch actual usage from local DB",
        );
      } else {
        // Invalid license
  ...
  try {
    ...
    if (settings.activeProfileId || settings.activeProfileName) {
      ...
      if (matchingProfile) {
        ...
        if (generatorElement)
          generatorElement.textContent = matchingProfile.generator || "--";
        if (refinerElement)
          refinerElement.textContent = matchingProfile.refiner || "--";
        if (validatorElement)
          validatorElement.textContent = matchingProfile.validator || "--";
        if (curatorElement)
          curatorElement.textContent = matchingProfile.curator || "--";
...
function updateStatusBarResponsive() {
  ...
  if (userElement) {
    const fullEmail =
      userElement.getAttribute("data-full-email") ||
    ...
    if (width < 480) {
      ...
      if (parts.length === 2) {
        const username =
          parts[0].length > 8 ? parts[0].substring(0, 5) + "..." : parts[0];
        const domain =
          parts[1].length > 10
            ? "@" + parts[1].substring(0, 7) + "..."
            : "@" + parts[1];
        userElement.textContent = username + domain;
      } else {
        userElement.textContent =
          fullEmail.length > 15
            ? fullEmail.substring(0, 12) + "..."
            : fullEmail;
      }
    } else if (width < 1200) {
      // Medium - show most of email
      userElement.textContent =
        fullEmail.length > 20 ? fullEmail.substring(0, 17) + "..." : fullEmail;
    } else {
  ...
  if (planElement) {
    const fullPlan =
      planElement.getAttribute("data-full-plan") ||
  ...
  if (conversationsElement) {
    const fullText =
      conversationsElement.getAttribute("data-full-text") ||
      conversationsElement.textContent ||
      "";
    if (width < 480) {
      // Ultra compact
...
function attachCliToolEventListeners() {
  ...
  const uninstallAllBtn = document.getElementById(
    "uninstall-all-cli-tools-btn",
  );
  if (uninstallAllBtn) {
...
async function renderCliToolsPanel(forceRefresh: boolean = false) {
  ...
  if (container && (container.innerHTML.trim() === "" || forceRefresh)) {
    ...
    container.innerHTML = `
            <div class="cli-tools-panel">
                <div class="cli-tools-header">
                    <h2 class="cli-tools-heading">AI CLI Tools Management</h2>
                    <p class="cli-tools-subheading">Install and manage AI-powered coding assistants</p>
                </div>
                <div class="cli-tools-batch-actions">
                    <button id="install-all-cli-tools-btn" class="cli-tools-action primary">
                        <span class="codicon codicon-cloud-download"></span>
                        <span>Install All Tools</span>
                    </button>
                    <button id="update-all-cli-tools-btn" class="cli-tools-action secondary">
                        <span class="codicon codicon-sync"></span>
                        <span>Update All Tools</span>
                    </button>
                    <button id="uninstall-all-cli-tools-btn" class="cli-tools-action danger">
                        <span class="codicon codicon-trash"></span>
                        <span>Uninstall All</span>
                    </button>
                    <div id="batch-status" class="cli-tools-action secondary" style="display:none;"></div>
                </div>
    ...
    try {
      ...
      gridContainer.appendChild(
        createCliToolCard({
          ...
          name: "Gemini CLI",
          description:
            "Google's free AI coding assistant with 1M token context",
          status: geminiStatus,
      ...
      gridContainer.appendChild(
        createCliToolCard({
          ...
          name: "Qwen Code",
          description:
            "AI-powered command-line workflow tool (2000 req/day free)",
          status: qwenStatus,
      ...
      gridContainer.appendChild(
        createCliToolCard({
          ...
          name: "OpenAI Codex",
          description:
            "OpenAI's agentic coding CLI with GPT-5 and o-series models",
          status: codexStatus,
          docsUrl:
    ...
    console.log(
      "[CLI Tools] Panel already rendered (use forceRefresh=true to refresh)",
    );
  }
}
...
function createCliToolCard(toolInfo: CliToolCardInfo): HTMLDivElement {
  const { id, name, description, status, docsUrl, badgeText } = toolInfo;
  ...
  const badgeMarkup = badgeText
    ? `<span class="cli-tool-badge">${badgeText}</span>`
    : "";

  let warningMarkup = "";
  if (id === "grok" && isInstalled && version === "0.0.29") {
    warningMarkup = `
            <div class="cli-tool-warning">
                <strong>Known issue with Grok v0.0.29.</strong>
                <br />ESM module resolution bug prevents launching. Downgrade with:
                <br /><code>npm uninstall -g @vibe-kit/grok-cli && npm install -g @vibe-kit/grok-cli@0.0.28</code>
            </div>
        `;
  }
  ...
  if (isInstalled) {
    actionButtons.push(
      `<button class="cli-tool-btn primary" data-action="launch" data-tool-id="${id}">Launch</button>`,
    );
    actionButtons.push(
      `<button class="cli-tool-btn" data-action="refresh" data-tool-id="${id}">Details</button>`,
    );
    actionButtons.push(
      `<button class="cli-tool-btn primary" data-action="update" data-tool-id="${id}">Update</button>`,
    );
    actionButtons.push(
      `<button class="cli-tool-btn danger" data-action="uninstall" data-tool-id="${id}">Uninstall</button>`,
    );
  } else {
    actionButtons.push(
      `<button class="cli-tool-btn primary" data-action="install" data-tool-id="${id}">Install</button>`,
    );
  }
  actionButtons.push(
    `<button class="cli-tool-btn ghost" data-action="docs" data-url="${docsUrl}">Docs</button>`,
  );

  card.innerHTML = `
        <div class="cli-tool-card-header">
            <div>
                <h4 class="cli-tool-card-title">${name}</h4>
                <p class="cli-tool-card-subtitle">${description}</p>
            </div>
            ${badgeMarkup}
        </div>
        ${warningMarkup}
        <div class="tool-status">
        <div class="cli-tool-card-status">
            <div class="cli-tool-card-status-item">
                <span class="label">Status</span>
                ${statusPill}
            </div>
            <div class="cli-tool-card-status-item">
                <span class="label">Version</span>
                <span class="value" data-version="${id}">${version}</span>
            </div>
            <div class="cli-tool-card-status-item">
        ...
        <div class="cli-tool-card-actions">
            ${actionButtons.join("")}
        </div>
    `;
...
function createStaticToolCard(
  id: string,
  name: string,
  description: string,
  badgeText: string | null,
  _badgeColor: string | null,
  docsUrl: string,
): HTMLDivElement {
  const card = document.createElement("div");
  card.className = "cli-tool-card";
...
async function refreshSidebarToolIcon(toolId: string): Promise<void> {
  ...
  const btn = document.querySelector(
    `.cli-quick-launch[data-tool="${toolId}"]`,
  ) as HTMLElement;
  if (!btn) return;
...
async function configureCliTool(toolId: string): Promise<void> {
  console.log(
    `[CLI Tools] Configure requested for ${toolId} - DEPRECATED: Configuration is now automatic during installation`,
  );

  // Since configuration is now automatic, just show a message
  alert(
    `${toolId} is already configured! Configuration is now done automatically during installation.`,
  );
  return;
...
async function updateCliTool(toolId: string): Promise<void> {
  ...
  try {
    ...
    if (result && result.success) {
      ...
      console.error(
        `[CLI Tools] Failed to update ${toolId}:`,
    ...
    for (const toolId of toolsToInstall) {
      ...
      try {
        ...
        if (result?.success) {
          ...
          failedCount++;
          console.error(
            `[CLI Tools] Failed to install ${toolId}:`,
            result?.error,
          );
        }
      } catch (error) {
    ...
    for (const toolId of toolsToUpdate) {
      ...
      try {
        ...
        if (result?.success) {
          updatedCount++;
          console.log(
            `[CLI Tools] Successfully updated ${toolId} to version ${result.version}`,
          );
          ...
          if (card) {
            ...
            const versionSpan = card.querySelector(
...
async function launchCliTool(toolId: string): Promise<void> {
  ...
  if (!toolStatus.installed) {
    console.log(
      `[CLI Tools] ${toolId} is not installed. Redirecting to CLI Tools panel...`,
    );

    // Switch to CLI Tools panel
    const cliToolsButton = document.querySelector(
      '[data-view="cli-tools"]',
    ) as HTMLElement;
    if (cliToolsButton) {
      ...
      setTimeout(() => {
        ...
        if (toolCard) {
          ...
          const statusDiv = toolCard.querySelector(
            ".tool-status",
          ) as HTMLElement;
          if (statusDiv) {
  ...
  try {
    ...
    if (result.success) {
      console.log(
        `[CLI Tools] ${toolId} launched successfully with command: ${result.command}`,
      );
      ...
      if (card) {
        const statusDiv = card.querySelector(".tool-status") as HTMLElement;
        if (statusDiv) {
          const resumeText = result.command?.includes("--resume")
            ? " (resumed)"
            : "";
...
const saveConsensusAnalytics = async (
  totalTokens: number,
  totalCost: number,
  ...
  const lastUserMessage =
    userMessages?.[userMessages.length - 1]?.textContent || "Consensus query";
  ...
  try {
    ...
    if (electronAPI && electronAPI.saveConversation) {
      ...
      if (saved) {
        console.log(
          "✅ Conversation saved to database with cost $" +
            totalCost.toFixed(4),
        );
  ...
  addLogEntry(
    `📊 Analytics updated: Query #${sessionMetrics.totalQueries}, Total Cost: $${sessionMetrics.totalCost.toFixed(4)}`,
    "info",
  );
};
...
setTimeout(() => {
  ...
  const isolatedTerminalPanelElement = document.getElementById(
  ...
  if (isolatedTerminalPanelElement) {
    ...
    if (window.electronAPI.onLaunchAIToolTerminal) {
      ...
      window.electronAPI.onLaunchAIToolTerminal(
        (data: {
          toolId: string;
          toolName: string;
          command: string;
          ...
          console.log(
            "📦 [Renderer] Received launch-ai-tool-terminal event:",
            data,
          );
          ...
          if (terminal) {
            console.log(
              "📦 [Renderer] Current opened folder:",
              window.currentOpenedFolder,
            );
            ...
            console.error(
              "❌ [Renderer] Terminal panel not initialized! isolatedTerminal is null",
            );
          }
        },
      );
      ...
      console.warn(
        "⚠️ [Renderer] onLaunchAIToolTerminal not available in electronAPI",
      );
    }
    ...
    const centerArea = document.getElementById("center-area");
    const isolatedTerminalPanel = document.querySelector(
      ".isolated-terminal-panel",
    ) as HTMLElement;

    // Isolated terminal panel collapse/expand (exactly like consensus panel)
    const toggleIsolatedTerminal = document.getElementById(
  ...
  const neuralContainer = document.getElementById(
    "neural-consciousness-container",
  );
  ...
  document.addEventListener("click", (e) => {
    ...
    if (target.classList.contains("panel-close")) {
      ...
      if (panelId && panels[panelId]) {
        ...
        const btn = document.querySelector(
          `.activity-btn[data-view="${panelId}"]`,
        );
...
function setupResizeHandles() {
  ...
  if (terminalResize && terminalSection) {
    ...
    terminalResize.addEventListener("mousedown", (e) => {
      ...
      startHeight = parseInt(
        window.getComputedStyle(terminalSection).height,
        10,
      );
  ...
  if (consensusResize && consensusPanel) {
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      if (centerArea) {
        ...
        const sidebarWidth =
          document.getElementById("left-sidebar")?.offsetWidth || 0;
        const terminalWidth =
          document.getElementById("isolated-terminal-panel")?.offsetWidth || 0;
        const remainingWidth =
          windowWidth - sidebarWidth - terminalWidth - newWidth;
  ...
  const toggleConsensusPanel = document.getElementById(
  ...
  const sidebarConsensusToggle = document.getElementById(
    "sidebar-consensus-toggle",
  );
...
async function handleOpenFolder(folderPath: string) {
  try {
    ...
    console.log(
      "[handleOpenFolder] Folder path value:",
    ...
    (window as any).currentOpenedFolder = currentOpenedFolder;
    console.log(
      "[handleOpenFolder] Set currentOpenedFolder to:",
      currentOpenedFolder,
    );
    ...
    if (explorerContainer) {
      ...
      if (isExplorerVisible) {
        ...
        console.log(
          "[handleOpenFolder] Explorer is not visible, clearing instance for later recreation",
        );
        if (window.fileExplorer) {
    ...
    if (window.gitAPI) {
      ...
      if (gitContainer) {
        ...
        setTimeout(async () => {
          ...
          window.scmView = window.gitUI;
          console.log(
            "[Menu] Source Control view refreshed for folder:",
            folderPath,
          );
        ...
        console.log(
          "[Menu] Git container not found! SCM view may not be visible.",
        );
      }
    }
...
async function openUnifiedCloneDialog(): Promise<void> {
  ...
  const paneUrl = byId("pane-url") as HTMLElement;
  const paneGh = byId("pane-github") as HTMLElement;
  const paneGl = byId("pane-gitlab") as HTMLElement;
  byId("tab-url").addEventListener("click", () => {
    paneUrl.style.display = "block";
    paneGh.style.display = "none";
    paneGl.style.display = "none";
  });
  byId("tab-github").addEventListener("click", () => {
    paneUrl.style.display = "none";
    paneGh.style.display = "block";
    paneGl.style.display = "none";
  });
  byId("tab-gitlab").addEventListener("click", () => {
    paneUrl.style.display = "none";
    paneGh.style.display = "none";
    paneGl.style.display = "block";
  });
  byId("clone-cancel").addEventListener("click", () =>
    document.body.removeChild(overlay),
  );
  byId("clone-confirm").addEventListener("click", async () => {
    try {
      let url = (
        modal.querySelector("#clone-url") as HTMLInputElement
      )?.value?.trim();
      if (paneGh.style.display !== "none") {
        const owner = (
          modal.querySelector("#gh-owner") as HTMLInputElement
        )?.value?.trim();
        const repo = (
          modal.querySelector("#gh-repo") as HTMLInputElement
        )?.value?.trim();
        if (owner && repo) url = `https://github.com/${owner}/${repo}.git`;
      } else if (paneGl.style.display !== "none") {
        const group = (
          modal.querySelector("#gl-group") as HTMLInputElement
        )?.value?.trim();
        const repo = (
          modal.querySelector("#gl-repo") as HTMLInputElement
        )?.value?.trim();
      ...
      console.error("Clone dialog error:", e);
    }
  });
  ...
  (window as any).electronAPI.on?.("reset-all-progress", () => {
    ...
    stages.forEach((stage) => {
      ...
      const progressBar = document.querySelector(
        `[data-stage="${stage}"] .stage-progress-fill`,
      );
      if (progressBar) {
    ...
    setTimeout(() => {
      ...
      if (chatContainer) {
        chatContainer.innerHTML =
          '<div class="message bot-message">Welcome to Hive Consensus! Try asking me a question.</div>';
...
async function saveRecentFolder(folderPath: string) {
  try {
    ...
    try {
      ...
      console.warn(
        "[Recent] addRecentFolder failed, falling back to settings:",
        e,
      );
    }
...
window.addEventListener("openFolderInExplorer", async (event: any) => {
  ...
  if (!path) return;
  const homeDir =
    (window as any).electronAPI?.homeDir ||
    "/Users/" + (window as any).electronAPI?.username ||
    "";
  const expandedPath = path.replace(/^~/, homeDir);
  console.log(
    "[Welcome] Delegating folder open to handleOpenFolder:",
    expandedPath,
  );
...
function openFileAndFocusEditor(filePath: string) {
  ...
  setTimeout(() => {
    const activeContainer = document.querySelector(
      '.editors-container .editor-container[style*="display: block"], .editors-container .editor-container:not([style])',
    ) as HTMLElement | null;
    const rectEditorArea = editorArea?.getBoundingClientRect();
    const rectEditors = (
      document.querySelector(".editors-container") as HTMLElement | null
    )?.getBoundingClientRect();
    const rectActive = activeContainer?.getBoundingClientRect();
    console.log(
      "[Editor Debug] editor-area visible:",
      editorArea ? getComputedStyle(editorArea).display !== "none" : false,
      "size:",
      rectEditorArea?.width,
      "x",
      rectEditorArea?.height,
    );
    console.log(
      "[Editor Debug] editors-container size:",
      rectEditors?.width,
      "x",
      rectEditors?.height,
    );
    console.log(
      "[Editor Debug] active editor size:",
      rectActive?.width,
      "x",
      rectActive?.height,
    );
    const welcomePanel = document.getElementById("welcome-panel");
    console.log(
      "[Editor Debug] welcome-panel display:",
      welcomePanel ? getComputedStyle(welcomePanel).display : "missing",
    );
  }, 50);
}
...
window.addEventListener("git-status-changed", () => {
  try {
    ...
    console.error("Failed to update status bar from git-status-changed:", e);
  }
});
