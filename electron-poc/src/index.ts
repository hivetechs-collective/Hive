...
async function getHiveLicenseToken(): Promise<string | null> {
  return new Promise((resolve) => {
    if (!db) return resolve(null);
    db.get(`SELECT value FROM configurations WHERE key = 'hive_license_key'`, [], (err: any, row: any) => {
      resolve(row?.value || null);
    });
  });
}
...
function scheduleD1UsageSync(): void {
  ...
  setTimeout(() => { processD1QueueOnceWrapper(); }, 10_000);
  // Then every 5 minutes
  setInterval(() => { processD1QueueOnceWrapper(); }, 5 * 60 * 1000);
}
