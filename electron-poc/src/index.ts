...
import { LazyGitManager } from './lazygit-manager';
...
// LazyGit IPC Handlers
let lazyGitProcess: ChildProcess | null = null;
...
ipcMain.handle('lazygit-start', async () => {
  try {
    console.log('Starting LazyGit...');
    const manager = LazyGitManager.getInstance();
    ...
    lazyGitProcess = await manager.spawnLazyGit(projectDir);
    
    console.log('LazyGit process spawned');
    
    // Set up stdout/stderr handlers to send data to renderer
    if (lazyGitProcess.stdout) {
      lazyGitProcess.stdout.on('data', (data: Buffer) => {
        if (mainWindow) {
          mainWindow.webContents.send('lazygit-data', data.toString());
        }
      });
    }
    
    if (lazyGitProcess.stderr) {
      lazyGitProcess.stderr.on('data', (data: Buffer) => {
        if (mainWindow) {
          mainWindow.webContents.send('lazygit-data', data.toString());
        }
      });
    ...
    lazyGitProcess.on('error', (err) => {
      console.error('LazyGit process error:', err);
      if (mainWindow) {
        mainWindow.webContents.send('lazygit-data', `\x1b[1;31mError: ${err.message}\x1b[0m\n`);
      }
    });
    
    lazyGitProcess.on('exit', (code) => {
      console.log(`LazyGit process exited with code ${code}`);
      if (mainWindow) {
        mainWindow.webContents.send('lazygit-exit', code);
      }
      lazyGitProcess = null;
    });
...
ipcMain.handle('lazygit-stop', async () => {
  if (lazyGitProcess) {
    lazyGitProcess.kill();
    lazyGitProcess = null;
  }
...
ipcMain.handle('lazygit-write', async (_, data: string) => {
  if (lazyGitProcess && lazyGitProcess.stdin) {
    lazyGitProcess.stdin.write(data);
    return { success: true };
  }
  return { success: false, error: 'LazyGit not running' };
});

ipcMain.handle('lazygit-resize', async (_, cols: number, rows: number) => {
  if (lazyGitProcess) {
    // Send resize signal to process
    process.kill(lazyGitProcess.pid!, 'SIGWINCH');
    return { success: true };
  }
