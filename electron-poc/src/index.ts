...
import {
  cliToolsDetector,
  setProcessManagerReference,
} from "./main/cli-tools/detector";
...
import {
  ...
  syncOpenRouter,
} from "./maintenance/OpenRouterSync";
import { processD1UsageQueueOnce } from "./maintenance/D1UsageSync";
...
const CLI_TOOLS_CONFIG_PATH = path.join(
  os.homedir(),
  ".hive",
...
const initDatabase = () => {
  ...
  db.run(
    `INSERT OR IGNORE INTO users (id, email, tier) VALUES ('default', 'default@hive.ai', 'FREE')`,
  );
  ...
  db.all(`PRAGMA table_info(messages)`, [], (err: any, cols: any[]) => {
    ...
    if (!names.has("stage"))
      addCol(`ALTER TABLE messages ADD COLUMN stage TEXT`);
    if (!names.has("model_used"))
      addCol(`ALTER TABLE messages ADD COLUMN model_used TEXT`);
    if (!names.has("tokens_used"))
      addCol(`ALTER TABLE messages ADD COLUMN tokens_used INTEGER DEFAULT 0`);
    if (!names.has("cost"))
      addCol(`ALTER TABLE messages ADD COLUMN cost REAL DEFAULT 0`);
    if (!names.has("consensus_path"))
      addCol(`ALTER TABLE messages ADD COLUMN consensus_path TEXT`);
    if (!names.has("consensus_rounds"))
      addCol(`ALTER TABLE messages ADD COLUMN consensus_rounds INTEGER`);
    if (!names.has("parent_message_id"))
      addCol(`ALTER TABLE messages ADD COLUMN parent_message_id TEXT`);
    if (!names.has("timestamp"))
      addCol(
        `ALTER TABLE messages ADD COLUMN timestamp TEXT DEFAULT CURRENT_TIMESTAMP`,
      );
  });
  ...
  `,
    ...
    (err: any, rows: any[]) => {
      if (!err && rows) {
        const hasMaxRounds = rows.some(
          (col: any) => col.name === "max_consensus_rounds",
        );
        if (!hasMaxRounds) {
          db.run(
            `ALTER TABLE consensus_profiles ADD COLUMN max_consensus_rounds INTEGER DEFAULT 3`,
            (err: any) => {
              if (!err) {
                console.log(
                  "✅ Added max_consensus_rounds column to consensus_profiles table",
                );
              }
            },
          );
...
const initGitManager = async (folderPath?: string) => {
  if (!folderPath) {
    logger.info(
      "[Main] No folder path provided, GitManager will return null status",
    );
  ...
  try {
    ...
    if (!preferOpened) {
      ...
      if (detectedRoot && detectedRoot !== folderPath) {
        ...
        logger.info(
          `[Main] Auto-detected Git root: ${detectedRoot} (from ${folderPath})`,
        );
      }
    } else {
      logger.info(
        "[Main] Respecting preference: using opened folder as Git root",
      );
    }
  } catch {}
  ...
  if (GIT_MANAGER_VERSION === 2) {
    logger.info(
      "[Main] Using EnhancedGitManager with authentication support for:",
      repoPathToUse,
    );
...
void createApplicationMenu();
  ...
  // Schedule D1 usage sync processing
  try {
    scheduleD1UsageSync();
  } catch (e) {
    logger.warn("[D1UsageQueue] schedule error", { error: e });
  }
...
async function runOpenRouterSyncOnce(): Promise<void> {
  try {
    ...
    if (!key) {
      logger.info("[OpenRouterSync] No OpenRouter API key set; skipping sync");
      return;
    }
    if (!db) {
      logger.warn("[OpenRouterSync] DB not initialized");
      return;
    }
    const res = await syncOpenRouter(db, key);
    logger.info(
      `[OpenRouterSync] providers=${res.providersUpserted} models=${res.modelsUpserted} deactivated=${res.modelsDeactivated}`,
    );
...
async function getHiveUserId(): Promise<string> {
  return new Promise((resolve) => {
    ...
    db.get(
      `SELECT value FROM configurations WHERE key = 'hive_user_id' LIMIT 1`,
      [],
      (err: any, row: any) => {
...
async function getHiveLimits(): Promise<{ limit: number | "unlimited" }> {
  return new Promise((resolve) => {
    ...
    db.get(
      `SELECT value FROM configurations WHERE key = 'hive_remaining'`,
      [],
      (errR: any, rowR: any) => {
        const remaining = rowR?.value;
        if (remaining === "unlimited") return resolve({ limit: "unlimited" });
        db.get(
          `SELECT value FROM configurations WHERE key = 'hive_daily_limit'`,
          [],
          (errL: any, rowL: any) => {
...
async function enforceUsageLimitOrThrow(): Promise<void> {
  ...
  if (used >= (limits.limit as number)) {
    throw new Error(
      "Daily conversation limit reached. Please purchase credits or try again tomorrow.",
    );
  }
}

// D1 Usage queue scheduler
async function getHiveLicenseToken(): Promise<string | null> {
  return new Promise((resolve) => {
    if (!db) return resolve(null);
    db.get(
...
function scheduleD1UsageSync(): void {
  // Kick off shortly after startup
  setTimeout(() => {
    processD1QueueOnceWrapper();
  }, 10_000);
  ...
  const getCfg = (key: string) =>
    new Promise<string | null>((resolve) =>
      db!.get(
        `SELECT value FROM configurations WHERE key = ?`,
        [key],
        (_: any, row: any) => resolve(row?.value || null),
      ),
    );
  const getCount = (sql: string) =>
    new Promise<number>((resolve) =>
      db!.get(sql, [], (_: any, row: any) => resolve(row?.c || 0)),
    );
  const [modelsLast, usageLast, profilesLast, activeModels, pendingUsage] =
    await Promise.all([
      getCfg("openrouter_last_sync"),
      getCfg("usage_last_sync"),
      getCfg("profiles_last_migration"),
      getCount(
        `SELECT COUNT(*) as c FROM openrouter_models WHERE is_active = 1`,
      ),
      getCount(
        `SELECT COUNT(*) as c FROM d1_usage_queue WHERE status IN ('pending','error')`,
      ),
    ]);
  return {
    ok: true,
...
ipcMain.handle("profiles-migrate-v2", async () => {
  ...
  const rows: any[] = await new Promise((resolve, reject) =>
    db!.all("SELECT * FROM consensus_profiles", [], (e: any, r: any[]) =>
  ...
  for (const row of rows) {
    ...
    const stageIds: Record<string, string | null> = {
      generator: null,
      refiner: null,
      validator: null,
      curator: null,
    ...
    for (const [k, model] of stages) {
      const map: any = await new Promise((resolve) =>
        db!.get(
          "SELECT internal_id FROM openrouter_models WHERE openrouter_id = ? LIMIT 1",
          [model],
    ...
    await new Promise((resolve) =>
      db!.run(
        `INSERT INTO consensus_profiles_v2 (
...
ipcMain.handle(
  "profiles-rebind-active",
  async (_e, profileIdOrName: string) => {
    if (!db) return { ok: false, error: "DB not initialized" };
...
const registerGitHandlers = () => {
  ...
  ipcMain.handle("git-status", async () => {
    ...
    try {
      ...
      let repoPath: string;
      if (
        "getRepoPath" in gitManager &&
        typeof (gitManager as any).getRepoPath === "function"
      ) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      logger.info(
        "[Main] git-log returning:",
        result ? result.substring(0, 100) + "..." : "empty",
      );
      return result;
    },
  );
  ...
  ipcMain.handle("git-push-chunked", async () => {
    ...
    try {
      ...
      if (
        "getRepoPath" in gitManager &&
        typeof gitManager.getRepoPath === "function"
      ) {
  ...
  ipcMain.handle(
    "git-file-diff",
    async (_, commitHash: string, filePath: string) => {
      if (!gitManager) return "";
  ...
  ipcMain.handle(
    "git-clone",
    async (_, url: string, parentDirectory: string) => {
      try {
        ...
        const repoNameMatch =
          url.replace(/\\\\/g, "/").split("/").pop() || "repository";
  ...
  ipcMain.handle("db-welcome-analytics-log", async (_, action: string) => {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        db!.exec(
          `INSERT INTO welcome_analytics (action, timestamp) VALUES ('${safeAction}', CURRENT_TIMESTAMP);`,
        );
        resolve({ success: true });
      } catch (err) {
        logger.error("[WelcomeAnalytics] Failed to log action", {
          action,
          error: err,
        });
        reject(err);
  ...
  ipcMain.handle("db-integrity-check", async () => {
    return new Promise((resolve, reject) => {
      ...
      db.get("PRAGMA integrity_check", (err, row: any) => {
        if (err) reject(err);
        else resolve({ result: row ? Object.values(row)[0] : "unknown" });
      });
    });
  });
  ...
  ipcMain.handle("db-backup", async (_evt, opts: any) => {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!destPath) {
          reject("Destination path required");
          return;
        }
        const password: string | undefined =
          typeof opts === "object" ? opts.password || undefined : undefined;
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        try {
          ...
          fs.closeSync(fd);
          if (
            header.toString() === "HIVEENC1" ||
            header.toString() === "HIVEENC2"
          ) {
            if (!password) {
        ...
        srcDb.get("PRAGMA integrity_check", (err: any, row: any) => {
          ...
          (db as any).close((closeErr: any) => {
            ...
            try {
              ...
              if (candidatePath.endsWith(".tmprestore")) {
                try {
...
const registerDialogHandlers = () => {
    ...
    try {
      ...
      let dirSetting: string | null = await new Promise((resolve) => {
        db!.get(
          ...
          ["backup.dir"],
          (err: any, row: any) => resolve(err ? null : row ? row.value : null),
        );
      });
      if (!dirSetting || dirSetting.trim() === "")
        dirSetting = process.env.HIVE_BACKUP_DIR || null;
      const dir =
        dirSetting && dirSetting.trim() !== "" ? dirSetting : getBackupDir();
  ...
  ipcMain.handle(
    "show-input-dialog",
    async (_, title: string, defaultValue?: string) => {
...
const registerFileSystemHandlers = () => {
  ...
  ipcMain.handle("fs-get-tree", async (_, rootPath?: string) => {
    ...
    if (!rootPath) {
      logger.info(
        "[Main] fs-get-tree called without root path, returning empty",
      );
      return [];
    }
  ...
  ipcMain.handle("fs-get-directory", async (_, dirPath: string) => {
    ...
    logger.info(
      `[Main] fs-get-directory returning ${result?.length || 0} items for ${dirPath}`,
    );
    return result;
  });
  ...
  ipcMain.handle(
    "fs-create-file",
    async (_, dirPath: string, fileName: string) => {
      try {
  ...
  ipcMain.handle(
    "fs-move-file",
    async (_, sourcePath: string, targetPath: string) => {
      try {
        const fs = require("fs").promises;
...
async function performBackupWithRetention(): Promise<string | null> {
  ...
  const dest = path.join(
    dir,
    `hive-ai-backup-${ts}.sqlite${compress ? ".gz" : ""}`,
  );
...
async function scheduleAutoBackupIfDue() {
  ...
  const settings = await new Promise<Record<string, string | null>>(
    (resolve) => {
  ...
  if (!enabled) {
    ...
    if (overdue && mainWindow) {
      const res = await dialog.showMessageBox(mainWindow, {
        ...
        message:
          "Protect your data: enable auto backup or create a backup now?",
        detail:
          "Auto backups are currently disabled and your last backup appears to be older than recommended.",
      });
      if (res.response === 0) {
        // Enable weekly by default
        db!.run(
          "INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)",
          ["backup.autoEnabled", "1"],
        );
        db!.run(
          "INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)",
...
const initializeProcessManager = () => {
  ...
  if (app.isPackaged) {
    ...
    if (fs.existsSync(unpackedPath)) {
      ...
      logger.info(
        `[Main] Using unpacked Memory Service at: ${memoryServicePath}`,
      );
    } else {
      // Fallback to extraction if unpacked version not found (shouldn't happen with v2.0 build)
      logger.warn(
        "[Main] Unpacked Memory Service not found, falling back to extraction",
      );
  ...
  function registerWebSocketBackend() {
    logger.info(
      "[ProcessManager] Registering WebSocket backend at:",
      consensusBackendPath,
    );
    ...
    if (app.isPackaged) {
      ...
      try {
        ...
        if (!needsExtraction && fs.existsSync(versionFile)) {
          try {
            const extractedVersion = fs
              .readFileSync(versionFile, "utf8")
              .trim();
            needsExtraction = extractedVersion !== appVersion;
            if (needsExtraction) {
              logger.info(
                `[ProcessManager] App version changed (${extractedVersion} -> ${appVersion}), re-extracting Python runtime`,
              );
            }
          } catch {
        ...
        if (needsExtraction) {
          logger.info(
            `[ProcessManager] Extracting Python runtime to ${extractedBase}...`,
          );
          ...
          if (fs.existsSync(extractedBase)) {
            try {
              ...
              logger.warn(
                "[ProcessManager] Could not remove old extraction, will overwrite:",
                err,
              );
            }
          }
          ...
          logger.info(
            `[ProcessManager] Successfully extracted Python runtime to ${extractedBase}`,
          );
        } else {
          logger.info(
            `[ProcessManager] Python runtime already extracted at ${extractedBase}`,
          );
        }
        ...
        finalModelScript = path.join(
          extractedBase,
          "models",
          "model_service_wrapper.py",
        );

        logger.info(
          "[ProcessManager] Using extracted Python path:",
          finalPythonPath,
        );
        logger.info(
          "[ProcessManager] Using extracted model script:",
          finalModelScript,
        );
      } catch (err) {
  ...
  processManager.on("process:message", (name: string, msg: any) => {
    if (name === "memory-service") {
      if (msg.type === "ready") {
        ...
        const port =
          msg.port || processManager.getProcessStatus("memory-service")?.port;
        logger.info(
          `[Main] Memory Service available on port ${port} for direct API access`,
        );
  ...
  processManager.on("process:unhealthy", (name: string, error: any) => {
    ...
    mainWindow?.webContents.send("process-health", {
      name,
      healthy: false,
      error: error.message,
    });
  ...
  processManager.on("aihelper:error", (data: any) => {
    ...
    if (data.issue?.includes("model_service_wrapper.py")) {
      logger.error(
        "[Main] CRITICAL: Wrong Python script configured - consensus will fail!",
      );
      logger.error(
        "[Main] Fix: Update model script path to use model_service_wrapper.py",
      );
    }
  });
...
const handleMemoryServiceDbQuery = (msg: any) => {
  ...
  db.all(msg.sql, msg.params, (error, rows) => {
    logger.info(
      "[Main] Database query result:",
      error ? `Error: ${error.message}` : `${rows?.length || 0} rows`,
    );
...
const registerMemoryServiceHandlers = () => {
  ...
  ipcMain.handle("memory-service-start", async () => {
    ...
    try {
      ...
      if (started) {
        ...
        if (processInfo?.port) {
          logger.info(
            `[Main] Memory Service started on dynamic port ${processInfo.port}`,
          );
        }
      }
  ...
  ipcMain.handle("memory-service-stats", async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:${port}/api/v1/memory/stats`,
      );
      if (response.ok) {
        ...
        logger.debug(
          `[Main] Corrected connected tools count: ${actualConnectedCount}`,
        );
        return stats;
      }
    } catch (error) {
  ...
  ipcMain.handle("memory-service-tools", async () => {
    try {
      ...
      logger.info(
        `[Main] Connected tools via CLI detector: ${connectedTools.length}`,
      );
      return connectedTools;
    } catch (error) {
      logger.error(
        "[Main] Failed to get connected tools via CLI detector:",
        error,
      );
      return [];
    }
  ...
  ipcMain.handle("memory-service-activity", async (_, limit: number = 50) => {
    try {
      ...
      if (!port) throw new Error("Memory Service not running");
      const response = await fetch(
        `http://localhost:${port}/api/v1/memory/activity?limit=${limit}`,
      );
      if (response.ok) {
...
function loadJsonFileSafely(filePath: string): any {
  try {
    ...
    try {
      ...
      logger.warn(
        `[Main] Could not backup corrupted file ${filePath}:`,
        renameError,
      );
    }
    return {};
...
function updateCursorMcpConfiguration(toolId: string, token?: string) {
  ...
  if (!memoryStatus || !memoryStatus.port) {
    logger.debug(
      "[Main] Skipping Cursor MCP update - Memory Service not running",
    );
    return;
  }
  ...
  logger.info(
    `[Main] Updated Cursor MCP configuration for Hive memory service on port ${port}`,
  );
}
...
function getEnhancedPath(): string {
  ...
  return [
    ...new Set([...pathAdditions, ...(process.env.PATH || "").split(":")]),
  ]
    .filter(Boolean)
    .join(":");
}
  ...
  ipcMain.handle("cli-tool-install", async (_, toolId: string) => {
    ...
    try {
      ...
      if (!toolConfig.installCommand) {
        ...
        return {
          success: false,
          error: `Installation not available for ${toolConfig.name}`,
        };
      }
      ...
      logger.info(
        `[Main] Running installation command: ${toolConfig.installCommand}`,
      );
      ...
      try {
        ...
        try {
          if (toolConfig.versionCommand) {
            ...
            if (toolId === "gemini-cli") {
              const match = versionResult.stdout.match(
                /(?:gemini-cli\/|v?)(\d+\.\d+\.\d+)/,
              );
          ...
          logger.warn(
            `[Main] Could not get version after installation:`,
            versionError,
          );
        }
        ...
        if (!toolsWithoutMemoryService.includes(toolId)) {
          ...
          try {
            ...
            const memoryServicePort =
              processManager.getProcessStatus("memory-service")?.port;
            ...
            const memoryStatus =
              processManager.getProcessStatus("memory-service");
            const dynamicEndpoint = memoryStatus?.port
              ? `http://localhost:${memoryStatus.port}`
              : "http://localhost:3000";

            logger.info(
              `[Main] Setting up direct API integration for ${toolId} with endpoint: ${dynamicEndpoint}`,
            );
            ...
            logger.warn(
              `[Main] Could not configure Memory Service for ${toolId}:`,
              configError,
            );
        ...
        if (toolId === "cursor-cli") {
          try {
            ...
            logger.warn(
              "[Main] Failed to configure Cursor MCP integration:",
              cursorConfigError,
            );
          }
        }
        ...
        try {
            ...
            try {
              ...
              if (stats.isSymbolicLink()) {
                ...
                if (target === actualDbPath) {
                  logger.info(
                    `[Main] Database symlink already exists and is correct`,
                  );
                  needsSymlink = false;
                } else {
                  // Remove incorrect symlink
                  logger.info(
                    `[Main] Removing incorrect symlink pointing to ${target}`,
                  );
                ...
                logger.info(
                  `[Main] Removing non-symlink file at ${symlinkPath} (size: ${stats.size} bytes)`,
                );
                fs.unlinkSync(symlinkPath);
              }
            } catch (err) {
              logger.debug(
                `[Main] Could not check/remove existing file at ${symlinkPath}: ${err}`,
              );
            }
          }
          ...
          if (needsSymlink) {
            ...
            logger.info(
              `[Main] Created ~/.hive-ai.db symlink pointing to ${actualDbPath}`,
            );
          }
          ...
          logger.info(
            `[Main] Creating MEMORY.md symlink for ${toolId} context guide`,
          );
            ...
            try {
              ...
              if (stats.isSymbolicLink()) {
                ...
                if (target === memoryGuidePath) {
                  logger.info(
                    `[Main] MEMORY.md symlink already exists and is correct`,
                  );
          ...
          if (needsMemorySymlink) {
            ...
            logger.info(
              `[Main] Created ~/.MEMORY.md symlink for AI context guide`,
            );
          }
        } catch (symlinkError) {
          logger.warn(
            `[Main] Could not create symlinks for ${toolId}:`,
            symlinkError,
          );
        ...
        logger.info(
          `[Main] ${toolConfig.name} installed and configured successfully with intelligent memory integration, version: ${version}`,
        );
        return {
          success: true,
          version,
          message: `Installed ${toolConfig.name} version ${version} with Hive intelligence`,
        };
      } catch (error: any) {
    ...
    try {
      ...
      if (toolId === "cursor-cli") {
        ...
        try {
          ...
          const version =
            versionResult.stdout.trim().match(/(\d+\.\d+\.\d+)/)?.[0] ||
            "Unknown";
          ...
          try {
            ...
            logger.warn(
              "[Main] Failed to refresh Cursor MCP configuration after update:",
              cursorConfigError,
            );
          }

          logger.info(
            `[Main] Cursor CLI updated successfully to version ${version}`,
          );
          return {
            success: true,
            version,
      ...
      if (toolId === "aider") {
        ...
        try {
          ...
          logger.info(
            `[Main] ${toolId} updated successfully to version ${version}`,
          );
          return {
            success: true,
            version,
      ...
      const updatePackage =
        toolId === "grok" ? `${packageName}@0.0.28` : packageName;
        ...
        if (
          ...
          try {
            ...
            try {
              ...
              logger.warn(
                `[Main] Strategy 3 failed, trying Strategy 4: Manual cleanup for ${toolId}:`,
              ...
              if (reinstallError.message.includes("ENOTEMPTY")) {
                try {
                  logger.info(
                    `[Main] Strategy 4: Manual cleanup for ${toolId}`,
                  );

                  // Comprehensive manual cleanup based on Gemini CLI GitHub issues
                  logger.info(
                    `[Main] Performing comprehensive cleanup for ${toolId}`,
                  );
                  ...
                  try {
                    ...
                    for (const file of files) {
                      if (
                        file.includes("gemini-cli") ||
                        file.includes(".gemini-cli-")
                      ) {
                        const tempPath = path.join(globalDir, file);
                        await fs.rm(tempPath, { recursive: true, force: true });
                        logger.info(
                          `[Main] Removed temp directory: ${tempPath}`,
                        );
                      }
                    }
                  } catch (tempCleanupError) {
                    logger.debug(
                      `[Main] Temp cleanup had issues:`,
                      tempCleanupError,
                    );
                  }
                  ...
                  logger.error(
                    `[Main] All 4 strategies failed for ${toolId}:`,
                    manualError.message,
                  );
...
Or try: npm install -g ${installCmd} --force --no-cache
                  ...
                  throw new Error(
                    `All automated strategies failed. ${manualInstructions}`,
                  );
                }
              } else {
      ...
      if (updateSucceeded) {
        try {
          ...
          logger.warn(
            `[Main] Could not get version for ${toolId}:`,
            versionError,
          );
        }
        ...
        logger.info(
          `[Main] Clearing detector cache for ${toolId} after successful update`,
        );
        cliToolsDetector.clearCache(toolId);

        logger.info(
          `[Main] ${toolId} updated successfully using ${strategyUsed} to version ${version}`,
        );
        return {
          success: true,
          version,
          message: `Updated to version ${version} (${strategyUsed})`,
        };
      } else {
      ...
      if (
        ...
        return {
          success: false,
          error:
            "Permission denied. Try running the app with elevated permissions or update manually.",
        };
      }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          logger.warn(
            `[Main] Command ${toolConfig.command} still exists after uninstall`,
          );
          ...
          logger.info(
            `[Main] Command ${toolConfig.command} no longer exists - uninstall successful`,
          );
        }
        ...
        if (toolId === "grok") {
          ...
          logger.info(
            `[Main] Keeping Grok configuration at ~/.grok for potential reinstall`,
          );
        }
        ...
        return {
          success: true,
          message: `${toolConfig.name} has been uninstalled`,
        };
      } catch (error: any) {
        ...
        if (
          ...
          logger.info(
            `[Main] Tool may already be uninstalled: ${error.message}`,
          );
          cliToolsDetector.clearCache(toolId);
          return {
            success: true,
            message: `${toolConfig.name} was not installed or has been removed`,
          };
        }
  ...
  ipcMain.handle("cli-tool-configure", async (_, toolId: string) => {
    ...
    try {
      ...
      const registerPromise = new Promise<{ token: string }>(
        (resolve, reject) => {
      ...
      logger.info(
        `[Main] Configured ${toolId} for direct API access with endpoint: ${memoryServiceEndpoint}`,
      );
      ...
      if (toolId === "cursor-cli") {
        try {
          ...
          logger.warn(
            "[Main] Failed to update Cursor MCP configuration during configure:",
            cursorConfigError,
          );
        }
      }
      ...
      logger.info(
        `[Main] Successfully configured ${toolId} with Memory Service`,
      );
      return {
        success: true,
  ...
  ipcMain.handle(
    "cli-tool-launch",
    async (_, toolId: string, projectPath?: string) => {
      logger.info(
        `[Main] Launching CLI tool: ${toolId}${projectPath ? ` in ${projectPath}` : ""}`,
      );
      ...
      try {
        ...
        logger.info(
          `[Main] Getting AIToolsDatabase instance with unified database...`,
        );
        let aiToolsDb: any;
        try {
          ...
          logger.info(
            `[Main] AIToolsDatabase instance obtained with unified hive-ai.db`,
          );
        } catch (dbError: any) {
          logger.error(
            `[Main] Failed to get AIToolsDatabase instance:`,
            dbError,
          );
          logger.error(
            `[Main] AIToolsDatabase error message:`,
            dbError?.message,
          );
        ...
        if (aiToolsDb) {
          hasBeenLaunched = aiToolsDb.hasBeenLaunchedBefore(
            toolId,
            selectedPath,
          );
          logger.info(
            `[Main] Database check - Tool: ${toolId}, Path: ${selectedPath}, Previously launched: ${hasBeenLaunched}`,
          );
          ...
          if (launchInfo) {
            logger.info(
              `[Main] Previous launch info: Count: ${launchInfo.launch_count}, Last: ${launchInfo.last_launched_at}`,
            );
          }
        } else {
          logger.warn(
            `[Main] AIToolsDatabase not available, skipping launch tracking`,
          );
        }
        ...
        if (toolId === "claude-code") {
          ...
          const grokConfigPath = path.join(
            os.homedir(),
            ".grok",
            "user-settings.json",
          );
          ...
          if (fs.existsSync(grokConfigPath)) {
            try {
              const grokConfig = JSON.parse(
                fs.readFileSync(grokConfigPath, "utf-8"),
              );
          ...
          if (!hasGrokApiKey) {
            ...
            logger.info(
              "[Main] Grok API key not configured, will launch setup wizard",
            );
        ...
        logger.info(
          `[Main] Using command: ${command} (previously launched: ${hasBeenLaunched})`,
        );
        ...
        if (mainWindow && !mainWindow.isDestroyed()) {
          ...
          logger.info(
            `[Main] Sending menu-open-folder event for: ${selectedPath}`,
          );
          logger.info(
            `[Main] MainWindow exists: ${!!mainWindow}, isDestroyed: ${mainWindow?.isDestroyed()}`,
          );
          ...
          setTimeout(async () => {
            ...
            logger.info(
              `[Main] Sending launch-ai-tool-terminal event with command: ${command}`,
            );
            ...
            if (!mainWindow.webContents.isLoading()) {
              ...
              logger.warn(
                `[Main] Window is still loading, waiting for ready...`,
              );
              mainWindow.webContents.once("did-finish-load", () => {
                logger.info(
                  `[Main] Window finished loading, sending event now`,
                );
                mainWindow.webContents.send("launch-ai-tool-terminal", {
                  toolId: toolId,
                  toolName: toolName,
        ...
        logger.info(
          `[Main] Completed launch sequence for ${toolId} in ${selectedPath}`,
        );
...
const registerConsensusHandlers = () => {
  console.log(
    "🔥 Registering consensus IPC handlers with database:",
    db ? "initialized" : "NULL",
  );
  ...
  ipcMain.handle("backend-consensus", async (_, query: string) => {
    try {
      ...
      console.log(
        "DirectConsensusEngine: backend-consensus called with query:",
        query,
      );
      ...
      if (!consensusEngine) {
        console.log(
          "🔥 Database at handler time:",
          db ? "initialized" : "NULL",
        );
        const {
          DirectConsensusEngine,
        } = require("./consensus/DirectConsensusEngine");
  ...
  ipcMain.handle(
    "backend-consensus-quick",
    async (_, data: { query: string; profile?: string }) => {
      ...
      logger.info(
        `🚀🚀🚀 backend-consensus-quick CALLED with query: ${data.query}, profile: ${data.profile}`,
      );
  ...
  */ // END OF OLD COMPLEX VERSION
  ...
  logger.info(
    "🔥🔥🔥 CONSENSUS HANDLERS REGISTRATION COMPLETED - All handlers registered!",
  );
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!backendPort) {
        ...
        reject(
          new Error("WebSocket backend not running or port not allocated"),
        );
        return;
      }
      ...
      if (url.includes(":8765")) {
        ...
        logger.info(
          `[Main] Replaced hardcoded port with dynamic port: ${backendPort}`,
        );
      }
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      if (!wsConnection || wsConnection.readyState === WebSocket.CLOSED) {
        ...
        const backendInfo =
          processManager.getProcessStatus("websocket-backend");
        const backendPort = backendInfo?.port;
...
ipcMain.handle(
  "db-save-session",
  async (_, folderPath: string, tabs: any[], activeTab: string | null) => {
    return new Promise((resolve, reject) => {
      if (!db) {
        reject("Database not initialized");
        return;
       ...
       VALUES (?, ?, ?, ?)`,
        ...
        (err) => {
          if (err) {
            ...
            logger.info(
              `[Session] Saved session for ${folderPath} with ${tabs.length} tabs`,
            );
            resolve({ success: true });
          }
        },
      ...
      (err, row: any) => {
        if (err) {
          ...
          try {
            ...
            logger.info(
              `[Session] Loaded session for ${folderPath} with ${tabs.length} tabs`,
            );
...
ipcMain.handle(
  "db-add-recent-folder",
  async (_, folderPath: string, tabCount: number = 0) => {
    return new Promise((resolve, reject) => {
      if (!db) {
        reject("Database not initialized");
        return;
      ...
      (err, row: any) => {
        ...
        db.get(
          ...
          (err2, row2: any) => {
            ...
            db.get(
              "SELECT value FROM consensus_settings WHERE key = ?",
              ["active_profile_id"],
              (err3, row3: any) => {
                if (row3) {
                  ...
                  db.get(
                    "SELECT profile_name FROM consensus_profiles WHERE id = ?",
                    [row3.value],
                    (errName, rowName: any) => {
                      if (rowName)
                    ...
                    db.get(
                      ...
                      (errLimit, rowLimit: any) => {
                        ...
                        db.get(
                          "SELECT value FROM configurations WHERE key = ?",
                          ["hive_remaining"],
                          (errRemaining, rowRemaining: any) => {
                            if (rowRemaining)
                              settings.hiveRemaining = parseInt(
                                rowRemaining.value,
                              );
                                ...
                                db.get(
                                  ...
                                  (err5, row5: any) => {
                                    ...
                                    db.get(
                                      ...
                                      (err6, row6: any) => {
                                        if (row6)
                                          settings.maxDailyConversations =
                                            row6.value;
                                        // Load Git preference
                                        db.get(
                                          "SELECT value FROM configurations WHERE key = ?",
                                          ["git_prefer_opened_folder_root"],
                                          (err7, row7: any) => {
                                            if (row7)
                                              settings.gitPreferOpenedFolderRoot =
                                                row7.value === "true";
                                            resolve(settings);
                                          },
                                        );
    ...
    if (upperKey.startsWith("HIVE-")) {
      ...
      if (
        ...
        try {
          ...
          const response = await fetch(
            "https://gateway.hivetechs.io/v1/session/validate",
            {
              method: "POST",
              headers: {
...
ipcMain.handle(
  "settings-update-profile-max-rounds",
  async (_, profileName, maxRounds) => {
    return new Promise((resolve, reject) => {
      if (!db) {
        reject("Database not initialized");
        return;
      ...
      db.run(
        ...
        (err) => {
          if (err) {
            ...
            console.log(
              `✅ Updated max_consensus_rounds to ${maxRounds} for profile: ${profileName}`,
            );
            resolve(true);
          }
        },
    ...
    try {
      ...
      if (settings.maxDailyConversations !== undefined) {
        db.run(
          ...
          [
            "max_daily_conversations",
            settings.maxDailyConversations.toString(),
            0,
            userId,
            timestamp,
            timestamp,
      ...
      if (settings.gitPreferOpenedFolderRoot !== undefined) {
        db.run(
          ...
          [
            "git_prefer_opened_folder_root",
            settings.gitPreferOpenedFolderRoot ? "true" : "false",
            0,
            userId,
            timestamp,
            timestamp,
...
ipcMain.handle("settings-reset", async () => {
  return new Promise((resolve, reject) => {
    ...
    db.run(
      'DELETE FROM configuration WHERE key NOT IN ("openrouter_api_key", "hive_license_key")',
      (err) => {
        if (err) {
          reject(err);
        } else {
  ...
  async (
    ...
    `,
      ...
      `,
            ...
            (err2) => {
              if (err2)
                logger.error("Error saving to knowledge_conversations:", err2);
            },
          );
      ...
      `,
            [userId, data.conversationId, timestamp],
            (err3) => {
              if (err3)
                logger.error("Error saving to conversation_usage:", err3);
            },
          );
      ...
      `,
              ...
              (err4) => {
                if (err4)
                  logger.error("Error saving performance metrics:", err4);
              },
            );
          }
      ...
      `,
          ...
          logger.info(
            `✅ Saved conversation ${data.conversationId} to database`,
          );
          resolve(true);
        },
      );
...
ipcMain.handle("get-analytics", async (_e, period?: "24h" | "7d" | "30d") => {
    ...
    `,
      ...
      `,
        ...
        `,
          ...
          `,
                  [userId],
                  (errAllTime, rowAllTime: any) => {
                    if (errAllTime)
                      logger.error(
                        "Error getting all-time cost data:",
                        errAllTime,
                      );
                    ...
                    analyticsData.tokenUsage = {
                      total:
                        (rowAllTime?.total_input || 0) +
                        (rowAllTime?.total_output || 0),
        ...
        `,
                      ...
                      (err3, rows3: any[]) => {
                        if (err3)
                          logger.error("Error getting recent activity:", err3);
                        ...
                        analyticsData.recentActivity = (rows3 || []).map(
                          (row: any) => ({
                            ...
                            status: "completed",
                            tokens:
                              (row.total_tokens_input || 0) +
                              (row.total_tokens_output || 0),
          ...
          `,
                          ...
                          (err4, rows4: any[]) => {
                            if (err4) {
                              logger.error(
                                "Error getting model usage from stage_outputs:",
                                err4,
                              );
              ...
              `,
                                ...
                                (err5, rows5: any[]) => {
                                  ...
                                  (rows5 || []).forEach((row: any) => {
                                    const modelName =
                                      row.model?.split("/").pop() || row.model; // Simplify model names
                          ...
                          const processHour = (i: number) => {
                            if (i < 0) {
                              ...
                              analyticsData.successRate =
                                analyticsData.totalQueries > 0 ? 100 : 0;
  ...
  ipcMain.handle(
    "get-analytics-ro",
    async (_e, period?: "24h" | "7d" | "30d") => {
      try {
        if (!dbFilePath) throw new Error("DB path not set");
        const ro = new Database(
          dbFilePath,
          (Database as any).OPEN_READONLY || undefined,
        );
        const userId = "3034c561-e193-4968-a575-f1b165d31a5b";
            ...
            aggregated.alerts = (aggregated.alerts || []).concat([
              {
                type: "info",
                message:
                  "No data for current user; showing aggregated analytics",
                timestamp: new Date().toISOString(),
              },
            ]);
            data = aggregated;
