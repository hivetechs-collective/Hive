import { GitStatus, GitFileStatus, GitBranch, GitCommit } from './types/git';
...
export class GitUI {
  ...
  private async refresh() {
    try {
      this.currentStatus = await window.gitAPI.getStatus();
      this.render();
    } catch (error) {
  ...
  private renderFile(file: GitFileStatus, section: string): string {
  ...
  private getStatusIcon(file: GitFileStatus): { icon: string; class: string } {
    // Check staged status first
    if (file.index === 'A') return { icon: 'A', class: 'status-added' };
    if (file.index === 'M') return { icon: 'M', class: 'status-modified' };
    if (file.index === 'D') return { icon: 'D', class: 'status-deleted' };
    if (file.index === 'R') return { icon: 'R', class: 'status-renamed' };
    
    // Check working tree status
    if (file.working === '?') return { icon: 'U', class: 'status-untracked' };
    if (file.working === 'M') return { icon: 'M', class: 'status-modified' };
    if (file.working === 'D') return { icon: 'D', class: 'status-deleted' };
  ...
  showCommitDialog() {
    ...
    const input = document.getElementById('commit-input') as HTMLInputElement;
    if (box && input) {
  ...
  async doCommit() {
    ...
    if (input && input.value.trim()) {
      try {
        await window.gitAPI.commit(input.value.trim());
