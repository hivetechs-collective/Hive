...
exports.IsolatedTerminalPanel = void 0;
...
class IsolatedTerminalPanel {
    constructor(container) {
        this.tabs = new Map();
        this.activeTabId = null;
        this.terminalCounter = 1;
        this.terminalDataListenerSetup = false;
    ...
    createTerminalTab() {
        ...
        `;
            ...
            try {
                ...
                const { Terminal } = yield Promise.resolve().then(() => __importStar(require('xterm')));
                const { FitAddon } = yield Promise.resolve().then(() => __importStar(require('xterm-addon-fit')));
                const { Unicode11Addon } = yield Promise.resolve().then(() => __importStar(require('@xterm/addon-unicode11')));
                const { WebLinksAddon } = yield Promise.resolve().then(() => __importStar(require('@xterm/addon-web-links')));
    ...
    switchToTab(tabId) {
        ...
        this.tabs.forEach((tab, id) => {
            const tabBtn = this.tabsContainer.querySelector(`button:nth-child(${Array.from(this.tabs.keys()).indexOf(id) + 1})`);
            if (tabBtn) {
        ...
        if (selectedTab) {
            if (selectedTab.element) {
                ...
                const terminal = selectedTab.terminal;
                const fitAddon = selectedTab.fitAddon;
    ...
    closeTab(tabId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (tab) {
                ...
                if (terminalAPI) {
                    try {
                        yield terminalAPI.killTerminalProcess(tabId);
                ...
                const remainingTerminals = Array.from(this.tabs.keys()).filter(id => id.startsWith('terminal-') && !id.startsWith('terminal-ai-tool-'));
    ...
    startConsoleCapture(terminalId, fallbackElement) {
     ...
     */
    createToolTerminal(toolId, toolName, workingDirectory) {
     ...
     */
    launchToolProcess(tabId, toolId, workingDirectory) {
        return __awaiter(this, void 0, void 0, function* () {
            const tab = this.tabs.get(tabId);
            if (!tab || !tab.element) {
    ...
    addLogEntry(terminalId, level, message, fallbackElement) {
...
exports.IsolatedTerminalPanel = IsolatedTerminalPanel;
...
//# sourceMappingURL=IsolatedTerminalPanel.js.map