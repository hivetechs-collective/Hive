...
try {
  ...
  // Robust shims: call npm-cli.js/npx-cli.js via the bundled Node runtime.
  // The stock Node dist's bin/npm expects ../lib/cli.js which may not be present
  // in some distributions. These shims avoid that by directly invoking the CLI entrypoints.
  const writeRobustNpmShim = (dest) => {
    const shim = `#!/bin/sh
BIN_DIR="$(CDPATH= cd -- "$(dirname "$0")" && pwd)"
NODE_BIN="$BIN_DIR/node"
NPM_CLI="$BIN_DIR/node-dist/lib/node_modules/npm/bin/npm-cli.js"
...
`;
  ...
  const writeRobustNpxShim = (dest) => {
    const shim = `#!/bin/sh
BIN_DIR="$(CDPATH= cd -- "$(dirname "$0")" && pwd)"
NODE_BIN="$BIN_DIR/node"
NPX_CLI="$BIN_DIR/node-dist/lib/node_modules/npm/bin/npx-cli.js"
...
`;
  ...
  writeRobustNpmShim(npmShim);
  writeRobustNpxShim(npxShim);

  // Extra safety: ensure node-dist/lib/cli.js exists and forwards to npm's CLI
  try {
    const cliForwardPath = path.join(nodeDistTarget, 'lib', 'cli.js');
    const npmCliImpl = path.join(nodeDistTarget, 'lib', 'node_modules', 'npm', 'lib', 'cli.js');
    if (!fs.existsSync(cliForwardPath) && fs.existsSync(npmCliImpl)) {
      const forwarder = `// Auto-generated by build script\nmodule.exports = require('./node_modules/npm/lib/cli.js');\n`;
      fs.writeFileSync(cliForwardPath, forwarder);
    }
  } catch (e) {
    // Non-fatal
  }
