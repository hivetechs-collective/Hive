...
use hive_ai::analysis::{RepositoryIntelligence, LanguageDetector, SymbolIndex, DependencyAnalyzer};
...
fn bench_language_detection(c: &mut Criterion) {
    ...
    for (filename, content) in test_files {
        group.bench_with_input(
            ...
            |b, (filename, content)| {
                b.iter(|| {
                    black_box(detector.detect_language(filename, content))
                })
            },
        );
    }
...
"#;
            ...
            tokio::fs::write(project_path.join("package.json"), package_json).await.unwrap();
            ...
            |b, &file_count| {
                b.to_async(&rt).iter(|| async {
                    ...
                    for i in 0..file_count {
                        let filename = format!("file_{}.rs", i);
                        let content = create_sample_rust_file(1000);
                        tokio::fs::write(project_path.join("src").join(&filename), content).await.unwrap();
                    }
