...
impl ModeDetector {
    ...
    pub fn explain_choice(&self, query: &str, mode: &ModeType) -> HiveResult<String> {
        ...
        let explanation = if result.primary_mode == *mode {
            format!(
                "Selected {} mode because: {}",
                mode_to_string(mode),
                result.reasoning.join("; ")
            )
        } else {
            format!(
                "{} mode was not selected. Primary choice was {} mode.",
                mode_to_string(mode),
    ...
    ) -> HiveResult<Vec<(ModeType, f32)>> {
        let result = self.analyze_query(query, context)?;

        let mut alternatives: Vec<(ModeType, f32)> = result
            .scores
            .into_iter()
    ...
    pub fn analyze_query(
        ...
        let (primary_mode, confidence) = scores
            .iter()
    ...
    fn analyze_keywords(
        &self,
        query: &str,
        scores: &mut HashMap<ModeType, f32>,
        reasoning: &mut Vec<String>,
    ) -> HiveResult<()> {
        let query_lower = query.to_lowercase();

        // Check planning patterns
        let planning_matches = self
            .patterns
            .planning_patterns
            .iter()
    ...
    fn apply_user_preferences(
        ...
        match context.user_preferences.detail_level {
            DetailLevel::High => {
                *scores.get_mut(&ModeType::Planning).unwrap() +=
                    0.1 * self.weights.user_preference_weight;
                reasoning.push("High detail preference favors planning".to_string());
            }
            DetailLevel::Low => {
                *scores.get_mut(&ModeType::Execution).unwrap() +=
                    0.1 * self.weights.user_preference_weight;
...
mod tests {
    ...
    fn test_planning_mode_detection() {
        ...
        let mode = detector
            .detect_mode("How should I plan this new feature?", &context)
            .unwrap();
    ...
    fn test_execution_mode_detection() {
        ...
        let mode = detector
            .detect_mode("Implement authentication system", &context)
            .unwrap();
    ...
    fn test_analysis_mode_detection() {
        ...
        let mode = detector
            .detect_mode("What does this function do?", &context)
            .unwrap();
    ...
    fn test_user_preference_influence() {
        ...
        let result = detector
            .analyze_query("implement feature", &context)
            .unwrap();
