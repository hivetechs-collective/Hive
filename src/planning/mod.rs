...
pub use self::integration::{RepositoryContext, RepositoryIntelligence};
...
pub use self::types::{Dependency, ModeType, Plan, PlanningContext, Risk, Task, UserPreferences};
...
impl PlanningEngine {
    ...
    pub async fn create_plan(
        ...
        let tasks = self
            .decomposer
            .decompose(description, &context, &self.consensus_engine)
            .await?;
        ...
        let timeline = self
            .timeline_estimator
            .estimate(&tasks, &dependency_graph)?;
    ...
    pub async fn create_plan_with_repository(
        ...
        context = self
            .repository_intelligence
            .enhance_planning_context(context, &repo_context)?;
        ...
        metadata.insert(
            ...
            serde_json::json!(repo_context.quality_indicators.code_quality_score),
        );
        metadata.insert(
            "technical_debt_score".to_string(),
            serde_json::json!(repo_context.code_metrics.technical_debt.score),
        );
        ...
        for task in &mut plan.tasks {
            let task_context = self
                .repository_intelligence
                .get_task_context(task, &repo_context)
                .await?;

            // Add relevant files to task metadata
            if !task_context.relevant_files.is_empty() {
                let files_json = task_context
                    .relevant_files
                    .iter()
                    .map(|f| f.to_string_lossy().to_string())
                    .collect::<Vec<_>>();
                metadata.insert(
                    format!("task_{}_files", task.id),
                    serde_json::json!(files_json),
                );
            }
            ...
            if !task_context.suggested_approach.is_empty() {
                task.description = format!(
                    "{}\n\nSuggested Approach: {}",
                    task.description, task_context.suggested_approach
                );
            }
        }
    ...
    pub async fn execute_plan(
        ...
        plan: &Plan,
        validate: bool,
    ) -> HiveResult<PlanExecutionResult> {
        ...
        let execution_order = self
            .dependency_resolver
            .get_execution_order(&plan.dependencies)?;

        for task_id in execution_order {
            let task = plan.tasks.iter().find(|t| t.id == task_id).ok_or_else(|| {
                crate::core::error::HiveError::Planning(format!(
    ...
    pub async fn switch_mode(
        ...
        self.mode_switcher
            .switch(current_mode, target_mode, context)
            .await
    }
    ...
    pub fn get_mode_recommendation(
        &self,
        query: &str,
        context: &PlanningContext,
    ...
    async fn generate_title(&self, description: &str) -> HiveResult<String> {
        ...
        let prompt = format!(
            "Generate a concise title (max 6 words) for this task: {}",
            description
        );
    ...
    fn optimize_plan(&self, plan: &mut Plan) -> HiveResult<()> {
        ...
        self.dependency_resolver
            .optimize_ordering(&mut plan.tasks, &plan.dependencies)?;

        // Adjust timelines based on parallelization opportunities
        self.timeline_estimator
            .optimize_timeline(&mut plan.timeline, &plan.dependencies)?;
    ...
    fn validate_dependencies(
        ...
        for dep_id in &task.dependencies {
            let is_completed = completed_results
                .iter()
