...
impl TimelineEstimator {
    ...
    pub fn optimize_timeline(
        ...
        if let Some(latest_end) = timeline
            .task_schedules
            .values()
            ...
            .max()
        {
            ...
            timeline.total_duration = Duration::seconds(
                latest_end
                    .signed_duration_since(timeline.start_date)
                    .num_seconds(),
            );
        }
    ...
    fn calculate_adjusted_durations(
        &self,
        tasks: &[Task],
    ) -> HiveResult<HashMap<String, Duration>> {
        ...
        for task in tasks {
            ...
            let productivity_adjusted =
                base_duration.num_seconds() as f32 / self.working_hours.productivity_factor;
    ...
    fn find_critical_path(
        ...
        while let Some(task_id) = queue.pop_front() {
            let task_finish = earliest_finish
                .get(&task_id)
                .copied()
            ...
            if let Some(dependents) = adj_list.get(&task_id) {
                for dep_id in dependents {
                    ...
                    let current_start = earliest_start
                        .get(dep_id)
                        .copied()
                        .unwrap_or(Duration::zero());
    ...
    fn find_parallel_opportunities(
        ...
        for (task_id, task) in &dependencies.nodes {
            ...
            for (other_id, other_task) in &dependencies.nodes {
                if task_id != other_id
                    && !self.has_dependency_conflict(task_id, other_id, dependencies)
                {
        ...
        for task_id in task_ids {
            if let Some(schedule) = timeline.task_schedules.get_mut(task_id) {
                let duration = schedule
                    .planned_end
                    .signed_duration_since(schedule.planned_start);
    ...
    fn add_working_duration(
        &self,
        start: DateTime<Utc>,
        duration: Duration,
    ) -> HiveResult<DateTime<Utc>> {
