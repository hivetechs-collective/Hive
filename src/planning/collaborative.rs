...
impl CollaborativePlanner {
    ...
    pub fn assign_tasks(
        ...
        if let Some(assignments) = manual_assignments {
            ...
            let optimal_assignments = self
                .assignment_optimizer
                .optimize(&plan.tasks, &session.participants)?;
    ...
    pub fn resolve_conflict(
        ...
        let resolution = session.conflicts[conflict_index]
            .resolution_options
            .iter()
        ...
        self.conflict_resolver
            .apply_resolution(session, &conflict, &resolution)?;
    ...
    fn check_resource_overload(
        &self,
        session: &PlanningSession,
        plan: &Plan,
    ) -> HiveResult<Vec<Conflict>> {
        ...
        for (task_id, assignment) in &session.assignments {
            if let Some(task) = plan.tasks.iter().find(|t| &t.id == task_id) {
                *workload_map
                    ...
                    .or_insert(0.0) += task.estimated_duration.num_hours() as f32;
            }
        }
            ...
            if let Some(&workload) = workload_map.get(&participant.id) {
                ...
                if days_needed > 20.0 {
                    ...
                    conflicts.push(Conflict {
                        ...
                        description: format!(
                            "{} is overloaded with {:.1} days of work",
                            participant.name, days_needed
                        ),
        ...
        for (task_id, assignment) in &session.assignments {
            if let Some(task) = plan.tasks.iter().find(|t| &t.id == task_id) {
                if let Some(participant) = session
                    .participants
                    .iter()
                    .find(|p| p.id == assignment.assignee_id)
                {
                    // Check if participant has required skills
                    let missing_skills: Vec<String> = task
                        .required_skills
                        .iter()
                    ...
                    if !missing_skills.is_empty() {
                        conflicts.push(Conflict {
                            ...
                            resolution_options: vec![
                                ResolutionOption {
                                    id: Uuid::new_v4().to_string(),
                                    description: "Reassign to team member with required skills"
                                        .to_string(),
                                ...
                                ResolutionOption {
                                    id: Uuid::new_v4().to_string(),
                                    description: "Provide training or pair with experienced member"
                                        .to_string(),
    ...
    fn check_schedule_conflicts(
        ...
        let mut task_schedules: HashMap<String, Vec<(DateTime<Utc>, DateTime<Utc>, String)>> =
            HashMap::new();
                ...
                for j in i + 1..schedules.len() {
                    ...
                    if start1 < end2 && start2 < end1 {
                        if let Some(participant) =
                            session.participants.iter().find(|p| p.id == assignee_id)
                        {
                            conflicts.push(Conflict {
                                id: Uuid::new_v4().to_string(),
                                ...
                                description: format!(
                                    "{} has overlapping tasks scheduled",
                                    participant.name
                                ),
                                severity: ConflictSeverity::High,
                                resolution_options: vec![
                                    ...
                                    ResolutionOption {
                                        id: Uuid::new_v4().to_string(),
                                        description: "Reassign one task to another team member"
                                            .to_string(),
...
mod tests {
    ...
    fn test_planning_session_creation() {
        ...
        let session = planner
            .create_session("plan123".to_string(), participants)
            .unwrap();
    ...
    fn test_consensus_calculation() {
        ...
        let mut session = planner
            .create_session("plan123".to_string(), vec![])
            .unwrap();

        // Add votes
        planner
            .add_vote(&mut session, "item1", "user1", VoteType::Approve, None)
            .unwrap();
        planner
            .add_vote(&mut session, "item1", "user2", VoteType::Approve, None)
            .unwrap();
        planner
            .add_vote(&mut session, "item1", "user3", VoteType::Approve, None)
            .unwrap();
        planner
            .add_vote(&mut session, "item1", "user4", VoteType::Abstain, None)
            .unwrap();
