...
impl ModeSwitcher {
    ...
    pub async fn switch(
        ...
        let switch_result = self.execute_switch(current_mode, &target_mode, context).await?;
    ...
    pub async fn execute_switch(
        ...
        if !self.is_transition_allowed(from_mode, to_mode) {
            return Ok(SwitchResult {
                ...
                warnings: vec![format!("Direct transition from {:?} to {:?} not allowed", from_mode, to_mode)],
        ...
        // Prepare target mode
        self.prepare_target_mode(to_mode, &preserved_context, context).await?;

        // Execute transition
        let success = self.perform_transition(from_mode, to_mode, &preserved_context).await?;

        // Post-transition validation
        if success {
            self.validate_transition(to_mode, &preserved_context).await?;
    ...
    pub fn is_transition_allowed(&self, from: &ModeType, to: &ModeType) -> bool {
        ...
        self.transition_rules.allowed_transitions
            .get(from)
    ...
    pub fn get_transition_path(&self, from: &ModeType, to: &ModeType) -> HiveResult<Vec<ModeType>> {
        ...
        Err(HiveError::Planning(format!("No transition path from {:?} to {:?}", from, to)))
    }
    ...
    pub fn calculate_transition_cost(&self, from: &ModeType, to: &ModeType) -> f32 {
        ...
        self.transition_rules.transition_costs
    ...
    pub fn get_mode_recommendations(&self, mode: &ModeType, context: &PlanningContext) -> Vec<String> {
        let mut recommendations = Vec::new();

        match mode {
            ModeType::Planning => {
                recommendations.push("Focus on breaking down complex tasks into manageable steps".to_string());
                recommendations.push("Use risk analysis to identify potential issues early".to_string());
                if context.team_size > 1 {
                    recommendations.push("Consider collaborative planning features for team coordination".to_string());
                }
            }
            ModeType::Execution => {
                ...
                if context.existing_codebase {
                    recommendations.push("Run tests frequently to ensure changes don't break existing functionality".to_string());
                }
            }
            ModeType::Hybrid => {
                recommendations.push("Balance planning and execution based on task complexity".to_string());
                recommendations.push("Switch to focused modes when needed for deep work".to_string());
                recommendations.push("Use this mode for complex projects requiring multiple perspectives".to_string());
            }
            ModeType::Analysis => {
                recommendations.push("Use repository intelligence for comprehensive code understanding".to_string());
                recommendations.push("Focus on understanding patterns and relationships".to_string());
    ...
    async fn preserve_context(&self, mode: &ModeType, context: &PlanningContext) -> HiveResult<PreservedContext> {
    ...
    async fn prepare_target_mode(&self, mode: &ModeType, preserved: &PreservedContext, context: &PlanningContext) -> HiveResult<()> {
        // Prepare the target mode with preserved context
        // In a real implementation, this would set up mode-specific resources
        Ok(())
    }

    async fn perform_transition(&self, from: &ModeType, to: &ModeType, preserved: &PreservedContext) -> HiveResult<bool> {
    ...
    async fn validate_transition(&self, mode: &ModeType, preserved: &PreservedContext) -> HiveResult<()> {
    ...
    fn suggest_transition_path(&self, from: &ModeType, to: &ModeType) -> Vec<String> {
        ...
        if self.is_transition_allowed(from, &ModeType::Hybrid) && 
           self.is_transition_allowed(&ModeType::Hybrid, to) {
            vec![format!("Try switching to Hybrid mode first, then to {:?}", to)]
        } else {
    ...
    fn init_transition_rules() -> TransitionRules {
        ...
        allowed_transitions.insert(ModeType::Planning, vec![
            ModeType::Execution,
            ModeType::Hybrid,
            ModeType::Analysis,
        ]);
        ...
        allowed_transitions.insert(ModeType::Execution, vec![
            ModeType::Planning,
            ModeType::Hybrid,
            ModeType::Analysis,
        ]);
        ...
        allowed_transitions.insert(ModeType::Hybrid, vec![
            ModeType::Planning,
            ModeType::Execution,
            ModeType::Analysis,
            ModeType::Learning,
        ...
        allowed_transitions.insert(ModeType::Analysis, vec![
            ModeType::Planning,
            ModeType::Execution,
            ModeType::Hybrid,
        ]);
        ...
        allowed_transitions.insert(ModeType::Learning, vec![
            ModeType::Hybrid,
        ]);
...
impl ContextPreserver for DefaultContextPreserver {
    fn preserve(&self, context: &ModeContext) -> HiveResult<PreservedContext> {
        Ok(PreservedContext {
            ...
            breadcrumbs: context.session_history.iter()
...
mod tests {
    ...
    fn test_transition_costs() {
        ...
        let cost_low = switcher.calculate_transition_cost(&ModeType::Planning, &ModeType::Execution);
        let cost_same = switcher.calculate_transition_cost(&ModeType::Planning, &ModeType::Planning);
    ...
    fn test_transition_path() {
        ...
        // Direct path
        let path = switcher.get_transition_path(&ModeType::Planning, &ModeType::Execution).unwrap();
        assert_eq!(path.len(), 2);
        assert_eq!(path[0], ModeType::Planning);
        assert_eq!(path[1], ModeType::Execution);
        
        // Same mode
        let path = switcher.get_transition_path(&ModeType::Planning, &ModeType::Planning).unwrap();
    ...
    async fn test_mode_switch() {
        ...
        let result = switcher.switch(&ModeType::Planning, ModeType::Execution, &context).await;
        assert!(result.is_ok());
