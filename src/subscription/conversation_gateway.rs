...
use super::{ConversationVerification, SubscriptionInfo, SubscriptionTier};
...
impl ConversationGateway {
    ...
    pub async fn request_conversation_authorization(
        &self,
        question: &str,
        license_key: &str,
    ) -> Result<ConversationAuthorization> {
        ...
        if !response.status().is_success() {
            ...
            let user_info = if let Ok(error_response) =
                serde_json::from_str::<PreConversationResponse>(&error_text)
            {
                error_response
                    .user
                    .as_ref()
        ...
        let result: PreConversationResponse =
        ...
        if !result.allowed {
            ...
            return Err(GatewayError::UsageLimitExceeded {
                used,
                limit,
                plan,
                user,
        ...
        tracing::info!(
            "Conversation authorized ({} remaining)",
            result
                .remaining
        ...
        Ok(ConversationAuthorization {
            ...
            remaining: result.remaining.or(result.remaining_conversations), // Let it be None for unlimited
            limit: result.limits.map(|l| l.daily).or(result.plan_limit), // Let it be None for unlimited
            expires_at: if let Some(expires) = result.expires_at {
                DateTime::parse_from_rfc3339(&expires)?.with_timezone(&Utc)
            } else {
    ...
    pub async fn report_conversation_completion(
        ...
        let usage_proof =
            self.generate_usage_proof(conversation_token, conversation_id, question_hash);
    ...
    pub async fn get_quick_usage_status(&self, license_key: &str) -> Result<(u32, u32)> {
        ...
        if response.status().is_success() {
            ...
            let remaining = result.usage.as_ref().map(|u| u.remaining).unwrap_or(0);
