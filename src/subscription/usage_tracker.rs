...
use super::{CreditPack, SubscriptionInfo, SubscriptionTier, UsageStatistics};
...
impl UsageTracker {
    ...
    pub async fn load_data(&mut self) -> Result<()> {
        ...
        let (subscription, usage) = tokio::task::spawn_blocking(
            move || -> Result<(Option<SubscriptionInfo>, Option<UsageStatistics>)> {
                ...
                let last_conversation: Option<String> = conn
                    .query_row(
    ...
    pub async fn check_usage_before_conversation(
        &mut self,
    ) -> Result<(bool, Option<UsageNotification>)> {
            ...
            _ => {
                ...
                return Ok((
                    ...
                    Some(UsageNotification {
                        ...
                        message:
                            "Please configure your Hive license key to use consensus features."
                                .to_string(),
        ...
        let monthly_percentage =
            (usage.monthly_used as f64 / subscription.monthly_limit as f64) * 100.0;
        ...
        if daily_used >= subscription.daily_limit {
            ...
            let notification = self.generate_usage_warning(
                subscription,
                usage,
                daily_percentage,
                monthly_percentage,
    ...
    fn generate_usage_warning(
        ...
        if daily_percentage >= 90.0 || monthly_percentage >= 90.0 {
            Some(UsageNotification {
                ...
                message: self.generate_critical_warning_message(
                    subscription,
                    usage,
                    daily_percentage,
                    monthly_percentage,
        ...
        // High usage warning (75%+ usage)
            ...
            Some(UsageNotification {
                ...
                message: self.generate_high_usage_message(
                    subscription,
                    usage,
                    daily_percentage,
                    monthly_percentage,
        ...
        // Moderate usage (50%+ usage) - only for free tier
            ...
            && (daily_percentage >= 50.0 || monthly_percentage >= 50.0)
        {
            Some(UsageNotification {
                notification_type: NotificationType::Info,
                title: "📊 Usage Update".to_string(),
                message: self.generate_moderate_usage_message(
                    subscription,
                    usage,
                    daily_percentage,
                    monthly_percentage,
    ...
    fn generate_limit_reached_message(
        subscription: &SubscriptionInfo,
        usage: &UsageStatistics,
    ) -> String {
        let mut message = format!(
            "You've reached your daily allowance on the {} tier:\n\n",
            subscription.tier
        );
        message.push_str(&format!(
            "**Daily Allowance**: {}/{} conversations used today\n",
            usage.daily_used, subscription.daily_limit
        ));
        message.push_str(&format!(
            "**Monthly Usage**: {}/{} conversations this month\n\n",
        ...
        if subscription.tier == SubscriptionTier::Free {
            ...
            message
                .push_str("• **Standard ($10/month)**: 100 daily, 2,000 monthly conversations\n");
            ...
            if let Some(next_tier) = Self::get_next_tier(&subscription.tier) {
                message.push_str(&format!(
                    ...
                    next_tier,
                    next_tier.price(),
                    next_tier.daily_limit(),
                    next_tier.monthly_limit()
                ));
            }
    ...
    fn generate_high_usage_message(
        ...
        let mut message = format!(
            "You're using your {} subscription heavily this period.\n\n",
            subscription.tier
        );
        message.push_str("**Current Usage:**\n");
        message.push_str(&format!(
    ...
    pub async fn get_usage_display(&mut self) -> Result<String> {
        ...
        let subscription = match &self.subscription {
            ...
            None => {
                return Ok(
                    "📊 Usage information unavailable - please configure your license".to_string(),
                )
            }
        };
        ...
        let daily_percentage =
            ((usage.daily_used as f64 / subscription.daily_limit as f64) * 100.0).round() as u32;
        let monthly_percentage = ((usage.monthly_used as f64 / subscription.monthly_limit as f64)
            * 100.0)
            .round() as u32;
