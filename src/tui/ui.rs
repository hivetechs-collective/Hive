...
use ratatui::{
    ...
    text::{Line, Span, Text},
    widgets::{Block, Borders, Clear, List, ListItem, Paragraph, Wrap, StatefulWidget},
    Frame,
};
...
impl TuiInterface {
    ...
    pub fn draw(
        ...
        let consensus_height = if consensus_progress.is_some() { 6u16 } else { 0u16 };
    ...
    fn draw_messages_with_consensus(
        ...
        let items: Vec<ListItem> = display_items
            ...
            .map(|(content, style)| {
                ...
                if content.contains("EXECUTIVE SUMMARY") || content.contains("PERFORMANCE METRICS") || 
                   content.contains("Consensus Journey") || content.contains("╔") {
                    let content_type = ContentType::detect(content);
                    let formatted_text = ContentFormatter::format_with_highlighting(content, content_type);
    ...
    fn draw_consensus_panel(&self, frame: &mut Frame, area: Rect, progress: &ConsensusProgress) {
        ...
        let (progress_text, progress_color) = match simple {
            ...
            SimpleConsensusProgress::Generating(pct) => (format!("⚡ Generating... {}%", pct), Color::Green),
            SimpleConsensusProgress::Refining(pct) => (format!("✨ Refining... {}%", pct), Color::Blue),
            SimpleConsensusProgress::Validating(pct) => (format!("🔍 Validating... {}%", pct), Color::Magenta),
            SimpleConsensusProgress::Curating(pct) => (format!("📝 Curating... {}%", pct), Color::Cyan),
        ...
        let consensus_paragraph = Paragraph::new(progress_text)
            .style(Style::default().fg(progress_color).add_modifier(Modifier::BOLD))
            .block(Block::default()
                .borders(Borders::ALL)
