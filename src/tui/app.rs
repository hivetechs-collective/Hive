...
impl TuiApp {
        ...
        while let Ok(event) = self.event_receiver.try_recv() {
            match event {
                ...
                TuiEvent::Error(error) => {
                    self.add_message(&format!("❌ Error: {}", error), MessageType::Error)
                        .await?;
                }
    ...
    async fn process_input(&mut self) -> Result<()> {
        ...
        self.add_message(&format!("> {}", input), MessageType::UserInput)
            .await?;

        // Process the command
        self.input_handler
            .process_command(&input, self.event_sender.clone())
            .await?;
    ...
    async fn add_welcome_message(&mut self) -> Result<()> {
        ...
        self.add_message(&welcome_content, MessageType::Welcome)
            .await?;
