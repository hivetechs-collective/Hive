...
impl ContentFormatter {
    ...
    fn format_executive_summary(content: &str) -> Text {
        let lines: Vec<Line> = content.lines().map(|line| {
    ...
    fn format_code_block(content: &str) -> Text {
        let lines: Vec<Line> = content.lines().map(|line| {
            ...
            } else if line.contains("fn ") || line.contains("function ") || line.contains("def ") {
                // Function definitions
                Line::from(vec![
                    Span::styled(line, Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD))
                ])
    ...
    fn format_performance_metrics(content: &str) -> Text {
        let lines: Vec<Line> = content.lines().map(|line| {
            if line.contains("┌") || line.contains("┐") || line.contains("└") || line.contains("┘") || line.contains("├") || line.contains("┤") || line.contains("┼") {
            ...
            } else if line.contains("Duration:") || line.contains("Tokens:") || line.contains("Cost:") {
                // Metrics with values
                ...
                if parts.len() >= 2 {
                    Line::from(vec![
                        ...
                        Span::styled(parts[1], Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                    ])
                } else {
                    Line::from(vec![
                        Span::styled(line, Style::default().fg(Color::White))
                ...
                let spans: Vec<Span> = chars.iter().map(|c| {
                    match c {
            ...
            if line.contains("$") {
                ...
                if parts.len() >= 2 {
                    Line::from(vec![
                        ...
                        Span::styled(parts[1], Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
                    ])
                } else {
                    Line::from(vec![
                        Span::styled(line, Style::default().fg(Color::White))
