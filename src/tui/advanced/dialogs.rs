...
impl DialogManager {
    ...
    fn render_file_picker(&self, frame: &mut Frame, area: Rect, theme: &crate::tui::themes::Theme) {
        ...
        let path_widget = Paragraph::new(path_text).block(
            Block::default()
    ...
    pub fn handle_key_event(&mut self, key: KeyEvent) -> DialogResult {
        match &self.active_dialog {
            Some(DialogType::About) | Some(DialogType::Welcome) => match key.code {
    ...
    fn handle_file_picker_key(&mut self, key: KeyEvent) -> DialogResult {
        match key.code {
            ...
            KeyCode::Down => {
                if self.file_picker.selected_index
                    < self.file_picker.entries.len().saturating_sub(1)
                {
                    self.file_picker.selected_index += 1;
                }
                DialogResult::Continue
    ...
    fn handle_theme_selector_key(&mut self, key: KeyEvent) -> DialogResult {
        match key.code {
            ...
            KeyCode::Enter => {
                let theme_name =
                    self.theme_selector.themes[self.theme_selector.selected_index].clone();
                self.close_dialog();
