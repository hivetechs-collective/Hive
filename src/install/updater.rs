...
pub enum UpdateStatus {
    ...
    /// Update in progress
    Updating { progress: f32, status: String },
    /// Update completed
    Updated {
        old_version: String,
        new_version: String,
...
impl UpdateManager {
    ...
    fn find_platform_asset<'a>(&self, assets: &'a [ReleaseAsset]) -> Result<&'a ReleaseAsset> {
        ...
        Err(anyhow::anyhow!(
            "No suitable release asset found for platform: {}",
            platform
        ))
    }
    ...
    async fn apply_update(&self, new_binary_path: &PathBuf) -> Result<()> {
        ...
        if !output.status.success() {
            ...
            return Err(anyhow::anyhow!(
                "Update verification failed, restored backup"
            ));
        }
    ...
    pub async fn configure_auto_updates(&self, interval_hours: u32) -> Result<()> {
        ...
        println!(
            "✅ Auto-updates configured (check every {} hours)",
            interval_hours
        );
