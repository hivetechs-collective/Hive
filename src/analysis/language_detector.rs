...
impl LanguageDetector {
    ...
    pub fn detect_from_patterns(&self, content: &str) -> Option<Language> {
        // Rust patterns
        if content.contains("fn main()") || content.contains("impl ") || content.contains("use std::") {
            return Some(Language::Rust);
        }
        
        // Python patterns
        if content.contains("def ") || content.contains("import ") || content.contains("from ") && content.contains(" import ") {
        ...
        // Java patterns
        if content.contains("public class ") || content.contains("private class ") || content.contains("public static void main") {
            return Some(Language::Java);
        }
        
        // C++ patterns
        if content.contains("#include <") || content.contains("std::") || content.contains("namespace ") {
        ...
        // Ruby patterns
        if content.contains("def ") && content.contains("end") || content.contains("class ") && content.contains("end") {
        ...
        // Swift patterns
        if content.contains("import ") && (content.contains("UIKit") || content.contains("Foundation")) {
...
mod tests {
    ...
    fn test_extension_detection() {
        ...
        assert_eq!(detector.detect_from_path(Path::new("test.rs")).unwrap(), Language::Rust);
        assert_eq!(detector.detect_from_path(Path::new("test.py")).unwrap(), Language::Python);
        assert_eq!(detector.detect_from_path(Path::new("test.ts")).unwrap(), Language::TypeScript);
        assert_eq!(detector.detect_from_path(Path::new("test.js")).unwrap(), Language::JavaScript);
        assert_eq!(detector.detect_from_path(Path::new("test.go")).unwrap(), Language::Go);
        assert_eq!(detector.detect_from_path(Path::new("test.java")).unwrap(), Language::Java);
        assert_eq!(detector.detect_from_path(Path::new("test.cpp")).unwrap(), Language::Cpp);
        assert_eq!(detector.detect_from_path(Path::new("test.c")).unwrap(), Language::C);
        assert_eq!(detector.detect_from_path(Path::new("test.rb")).unwrap(), Language::Ruby);
        assert_eq!(detector.detect_from_path(Path::new("test.php")).unwrap(), Language::PHP);
        assert_eq!(detector.detect_from_path(Path::new("test.swift")).unwrap(), Language::Swift);
    }
    ...
    fn test_shebang_detection() {
        ...
        assert_eq!(detector.detect_from_shebang(python_content), Some(Language::Python));
        
        let node_content = "#!/usr/bin/env node\nconsole.log('Hello')";
        assert_eq!(detector.detect_from_shebang(node_content), Some(Language::JavaScript));
        
        let ruby_content = "#!/usr/bin/env ruby\nputs 'Hello'";
        assert_eq!(detector.detect_from_shebang(ruby_content), Some(Language::Ruby));
    }
    ...
    fn test_pattern_detection() {
        ...
        assert_eq!(detector.detect_from_patterns(rust_content), Some(Language::Rust));
        
        let python_content = "def main():\n    print('Hello')";
        assert_eq!(detector.detect_from_patterns(python_content), Some(Language::Python));
        
        let go_content = "package main\n\nfunc main() {\n    fmt.Println(\"Hello\")\n}";
        assert_eq!(detector.detect_from_patterns(go_content), Some(Language::Go));
    }
