...
use crate::core::ast::{
    AstNode, CodeMetrics, ErrorSeverity, ImportInfo, NodeMetadata, ParseError, ParseResult, Symbol,
    SymbolKind,
};
use crate::core::{HiveError, Language, Position};
...
"#;
    ...
    fn test_incremental_parsing() {
        ...
        let (new_tree, parse_time) = parser
            .parse_incremental(old_source, new_source, &tree, &edit)
            .unwrap();
    ...
    println!("Hello");
        ...
        assert!(highlights
            .iter()
            .any(|h| h.highlight_type == HighlightType::Keyword));
        assert!(highlights
            .iter()
            .any(|h| h.highlight_type == HighlightType::String));
        assert!(highlights
            .iter()
            .any(|h| h.highlight_type == HighlightType::Number));
    }
}
