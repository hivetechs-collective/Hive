...
impl PerformanceMonitor {
    ...
    pub fn record_parse(&self, duration: Duration, is_incremental: bool) {
        ...
        if is_incremental {
            ...
            self.total_incremental_time_us
            ...
            self.total_parse_time_us
                .fetch_add(duration_us, Ordering::Relaxed);

            // Update min/max
            self.min_parse_time_us
                .fetch_min(duration_us, Ordering::Relaxed);
            self.max_parse_time_us
...
impl BenchmarkRunner {
    ...
    pub async fn run_benchmark() -> Result<BenchmarkResult, anyhow::Error> {
        ...
        for (path, content) in files.iter().take(100) {
            ...
            engine
                .analyze_incremental(path, content, &modified, &edit)
                .await?;
