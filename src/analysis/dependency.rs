...
use petgraph::algo::{has_path_connecting, is_cyclic_directed, tarjan_scc, toposort};
...
impl DependencyAnalyzer {
    ...
    fn analyze_circular_dependencies(
        &self,
    ...
    fn calculate_dependency_layers(&self, graph: &DependencyGraph) -> Result<Vec<DependencyLayer>> {
        ...
        let mut current_modules: Vec<PathBuf> = graph
            .module_to_node
            .iter()
            .filter(|(path, &node)| {
                graph
                    ...
                    !processed.contains(path.as_path())
                        && graph
                            ...
                            .all(|dep_node| {
                                ...
                                processed.contains(dep_path.as_path())
                                    || graph.graph[dep_node].is_external
                            })
                })
                .map(|(path, _)| path.clone())
                .collect();
            ...
            if circular.severity >= Severity::High {
                for breaking_point in &circular.breaking_points {
                    suggestions.push(RefactoringSuggestion {
                        ...
                        reason: format!(
                            "Break circular dependency between {} modules",
                            circular.modules.len()
                        ),
        ...
        for (path, &node) in &graph.module_to_node {
            ...
            if in_degree > 10 && out_degree > 10 {
                suggestions.push(RefactoringSuggestion {
                    ...
                    target: path.clone(),
                    reason: format!(
                        "Module has {} dependencies and {} dependents",
        ...
        let mut modules_with_complexity: Vec<_> = cycle
            ...
            .filter_map(|path| {
                graph
                    .module_to_node
                    .get(path)
                    .map(|&node| (path.clone(), graph.graph[node].complexity))
            })
            .collect();
...
impl DependencyGraph {
    ...
    fn update_stats(&mut self) {
        ...
        self.stats.external_dependencies = self
            .graph
            .node_indices()
...
mod tests {
    ...
    async fn test_cycle_severity() {
        ...
        assert_eq!(analyzer.calculate_cycle_severity(4, &[]), Severity::High);
