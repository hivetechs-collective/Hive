...
use crate::analysis::{
    symbol_index::{SymbolIndexer, SymbolEntry},
    dependency::{DependencyAnalyzer, DependencyAnalysis},
};
use crate::core::{SymbolKind, Position};
use crate::core::ast::{ParseResult, CodeMetrics};
...
pub trait ArchitectureDetector: Send + Sync {
    fn detect(&self, symbols: &[SymbolEntry], dependencies: &DependencyAnalysis) -> Option<(ArchitecturePattern, f32)>;
}
...
impl RepositoryAnalyzer {
    ...
    pub async fn analyze_repository(&self, root_path: &Path) -> Result<RepositoryAnalysis> {
        ...
        // Detect architecture
        let architecture = self.detect_architecture(&symbols, &dependency_analysis).await?;
    ...
    async fn get_all_symbols(&self) -> Result<Vec<SymbolEntry>> {
        ...
        self.symbol_indexer.get_all_symbols().await
    ...
    async fn detect_architecture(
        ...
        let confidence = detected_patterns
            .first()
            .map(|(_, c)| *c)
            .unwrap_or(0.0);
    ...
    fn calculate_quality_metrics(&self, symbols: &[SymbolEntry]) -> Result<QualityMetrics> {
        ...
        let avg_complexity = if symbols.is_empty() { 0.0 } else { total_complexity as f32 / symbols.len() as f32 };
        
        let documented_symbols = symbols.iter().filter(|s| s.documentation.is_some()).count();
        let doc_coverage = if symbols.is_empty() { 0.0 } else { documented_symbols as f32 / symbols.len() as f32 };
    ...
    async fn scan_security(&self, root_path: &Path) -> Result<SecurityReport> {
        ...
        for entry in std::fs::read_dir(root_path)? {
            ...
            if path.is_file() {
                ...
                let scannable_extensions = vec!["rs", "js", "ts", "py", "java", "go", "php", "rb", "cpp", "c", "h"];
    ...
    async fn generate_recommendations(
        ...
        if !performance.hotspots.is_empty() {
            recommendations.push(Recommendation {
                ...
                impact: Impact::Significant,
                effort: Effort::Medium,
                specific_actions: performance.optimization_opportunities
                    .iter()
                    .take(3)
    ...
    fn analyze_layers(&self, symbols: &[SymbolEntry], dependencies: &DependencyAnalysis) -> Result<Vec<ArchitectureLayer>> {
        ...
        for symbol in symbols {
            ...
            let layer_name = if path_str.contains("controller") || path_str.contains("handler") || path_str.contains("api") {
                "presentation"
            } else if path_str.contains("service") || path_str.contains("business") || path_str.contains("logic") {
                "business"
            } else if path_str.contains("repository") || path_str.contains("data") || path_str.contains("persistence") {
                "data"
            } else if path_str.contains("model") || path_str.contains("entity") || path_str.contains("domain") {
                "domain"
            ...
            layer_modules.entry(layer_name.to_string())
        ...
        for (layer_name, modules) in layer_modules {
            let responsibilities = match layer_name.as_str() {
                "presentation" => vec!["UI logic".to_string(), "Request handling".to_string(), "Response formatting".to_string()],
                "business" => vec!["Business logic".to_string(), "Use cases".to_string(), "Workflows".to_string()],
                "data" => vec!["Data access".to_string(), "Persistence".to_string(), "External APIs".to_string()],
                "domain" => vec!["Domain models".to_string(), "Business entities".to_string(), "Value objects".to_string()],
                "infrastructure" => vec!["Cross-cutting concerns".to_string(), "Framework integration".to_string()],
            ...
            layers.push(ArchitectureLayer {
                name: layer_name,
                modules: modules.into_iter().collect::<HashSet<_>>().into_iter().collect(),
                responsibilities,
                violations,
            });
        }
    ...
    fn identify_components(&self, symbols: &[SymbolEntry]) -> Result<Vec<Component>> {
        ...
        for symbol in symbols {
            ...
            let (component_kind, component_name) = if name_lower.ends_with("controller") || path_str.contains("controller") {
                (ComponentKind::Controller, symbol.name.replace("Controller", ""))
                ...
                (ComponentKind::Repository, symbol.name.replace("Repository", ""))
    ...
    fn calculate_adherence_score(&self, layers: &[ArchitectureLayer], components: &[Component]) -> Result<f32> {
    ...
    fn identify_quality_hotspots(&self, issues: &[QualityIssue]) -> Result<Vec<QualityHotspot>> {
        ...
        for issue in issues {
            file_issues.entry(issue.location.clone())
                .or_insert_with(Vec::new)
                .push(issue);
        }
        ...
        for (file, file_issues_list) in file_issues {
            ...
            let severity_score: f32 = file_issues_list.iter()
            ...
            let remediation_time: u32 = file_issues_list.iter()
                .map(|issue| issue.remediation_time)
                .sum();
            
            // Score based on issues count, severity, and remediation effort
            let score = (severity_score + (issues_count as f32 * 0.5) + (remediation_time as f32 / 60.0)) / 10.0;
            ...
            if score > 0.3 { // Only include significant hotspots
    ...
    fn identify_performance_hotspots(&self, symbols: &[SymbolEntry]) -> Result<Vec<PerformanceHotspot>> {
        let mut hotspots = Vec::new();
            ...
            if let Some(signature) = &symbol.signature {
                ...
                if loop_count >= 2 {
                    hotspots.push(PerformanceHotspot {
                        ...
                        impact: if loop_count >= 3 { Impact::Major } else { Impact::Significant },
                        description: format!(
    ...
    fn find_bottlenecks(&self, symbols: &[SymbolEntry]) -> Result<Vec<Bottleneck>> {
        ...
        for symbol in symbols {
            let component = symbol.file_path
                .parent()
            ...
            component_usage.entry(component)
                .or_insert_with(Vec::new)
                .push(symbol);
        }
        ...
        for (component, component_symbols) in component_usage {
            let total_complexity: u32 = component_symbols.iter()
                .map(|s| s.complexity)
                .sum();
            let avg_complexity = if component_symbols.is_empty() { 0.0 } else { 
            ...
            if avg_complexity > 15.0 {
                bottlenecks.push(Bottleneck {
                    ...
                    severity: if avg_complexity > 25.0 { Severity::High } else { Severity::Medium },
                    affected_operations: component_symbols.iter()
                        .map(|s| s.name.clone())
                        .collect(),
                });
            }
            ...
            let memory_intensive_ops = component_symbols.iter()
            ...
            let io_operations = component_symbols.iter()
            ...
            let db_operations = component_symbols.iter()
    ...
    fn find_optimization_opportunities(&self, symbols: &[SymbolEntry]) -> Result<Vec<OptimizationOpportunity>> {
        ...
        opportunities.sort_by(|a, b| b.estimated_improvement.partial_cmp(&a.estimated_improvement).unwrap());
    ...
    fn create_debt_payment_plan(&self, items: &[TechnicalDebtItem]) -> Result<Vec<DebtPaymentTask>> {
        let mut tasks = Vec::new();
        let mut remaining_items = items.to_vec();
        
        // Sort by ROI (principal / interest ratio)
        remaining_items.sort_by(|a, b| {
            let roi_a = if a.interest > 0.0 { a.principal / a.interest } else { a.principal };
            let roi_b = if b.interest > 0.0 { b.principal / b.interest } else { b.principal };
            roi_b.partial_cmp(&roi_a).unwrap_or(std::cmp::Ordering::Equal)
        });
        
        // Group into tasks by priority
        let critical_items: Vec<_> = remaining_items.iter()
        ...
        if !critical_items.is_empty() {
            ...
            let avg_roi = critical_items.iter()
                .map(|i| if i.interest > 0.0 { i.principal / i.interest } else { i.principal })
                .sum::<f32>() / critical_items.len() as f32;
        ...
        let medium_items: Vec<_> = remaining_items.iter()
        ...
        if !medium_items.is_empty() {
            ...
            let avg_roi = medium_items.iter()
                .map(|i| if i.interest > 0.0 { i.principal / i.interest } else { i.principal })
                .sum::<f32>() / medium_items.len() as f32;
        ...
        let low_items: Vec<_> = remaining_items.iter()
        ...
        if !low_items.is_empty() {
            ...
            let avg_roi = low_items.iter()
                .map(|i| if i.interest > 0.0 { i.principal / i.interest } else { i.principal })
                .sum::<f32>() / low_items.len() as f32;
    ...
    fn has_excessive_allocation(&self, code: &str) -> bool {
        ...
        let allocation_count = allocation_patterns.iter().map(|pattern| {
            if let Ok(re) = Regex::new(pattern) {
    ...
    fn is_blocking_operation(&self, function_name: &str) -> bool {
        ...
        blocking_keywords.iter().any(|keyword| name_lower.contains(keyword))
    }
    
    fn is_memory_intensive(&self, function_name: &str) -> bool {
        let memory_keywords = vec![
            "cache", "buffer", "load", "parse", "process", "transform", "serialize", "deserialize"
        ];
        
        let name_lower = function_name.to_lowercase();
        memory_keywords.iter().any(|keyword| name_lower.contains(keyword))
    }
    ...
    fn is_io_operation(&self, function_name: &str) -> bool {
        ...
        let name_lower = function_name.to_lowercase();
        io_keywords.iter().any(|keyword| name_lower.contains(keyword))
    }
    
    fn is_database_operation(&self, function_name: &str) -> bool {
        let db_keywords = vec![
            "query", "select", "insert", "update", "delete", "find", "save", "create", "execute", "fetch"
        ];
        
        let name_lower = function_name.to_lowercase();
        db_keywords.iter().any(|keyword| name_lower.contains(keyword))
    }
...
impl ArchitectureDetector for MVCDetector {
    fn detect(&self, symbols: &[SymbolEntry], dependencies: &DependencyAnalysis) -> Option<(ArchitecturePattern, f32)> {
        ...
        let controllers = symbols.iter().filter(|s| {
        ...
        let models = symbols.iter().filter(|s| {
        ...
        let views = symbols.iter().filter(|s| {
...
impl ArchitectureDetector for CleanArchitectureDetector {
    fn detect(&self, symbols: &[SymbolEntry], dependencies: &DependencyAnalysis) -> Option<(ArchitecturePattern, f32)> {
        ...
        // Check for use cases
        let use_cases = symbols.iter().filter(|s| {
        ...
        // Check for entities
        let entities = symbols.iter().filter(|s| {
        ...
        let gateways = symbols.iter().filter(|s| {
...
impl ArchitectureDetector for MicroservicesDetector {
    fn detect(&self, symbols: &[SymbolEntry], dependencies: &DependencyAnalysis) -> Option<(ArchitecturePattern, f32)> {
        ...
        let service_indicators = symbols.iter().filter(|s| {
            s.file_path.to_string_lossy().contains("service") ||
        ...
        let messaging_indicators = symbols.iter().filter(|s| {
...
impl QualityAssessor for DuplicationAssessor {
    fn assess(&self, symbols: &[SymbolEntry], _metrics: &CodeMetrics) -> Vec<QualityIssue> {
        ...
        for symbol in symbols {
            name_counts.entry(symbol.name.clone())
                .or_insert_with(Vec::new)
                .push(symbol);
        }
        ...
        for (name, symbol_list) in name_counts {
            if symbol_list.len() > 1 {
                ...
                let different_files: HashSet<&PathBuf> = symbol_list.iter()
                    .map(|s| &s.file_path)
                    .collect();
        ...
        for symbol in symbols {
            if let Some(sig) = &symbol.signature {
                ...
                let normalized = sig.to_lowercase()
                    .replace(" ", "")
                    .replace("\t", "")
                    .replace("\n", "");
                
                signature_groups.entry(normalized)
                    .or_insert_with(Vec::new)
                    .push(symbol);
            }
        }
        ...
        for (signature, symbol_list) in signature_groups {
            if symbol_list.len() > 1 && signature.len() > 20 { // Only check substantial signatures
                let different_files: HashSet<&PathBuf> = symbol_list.iter()
                    .map(|s| &s.file_path)
                    .collect();
...
impl SecurityScanner for BasicSecurityScanner {
    fn scan(&self, content: &str, path: &Path) -> Vec<SecurityVulnerability> {
        ...
        let patterns = vec![
            ...
            (r#"(?i)\$_GET\[\s*["'][^"']*["']\s*\]"#, VulnerabilityKind::UnvalidatedInput, "CWE-79", "Validate and sanitize all user input"),
            (r#"(?i)\$_POST\[\s*["'][^"']*["']\s*\]"#, VulnerabilityKind::UnvalidatedInput, "CWE-79", "Validate and sanitize all user input"),
            
            // A02:2021 – Cryptographic Failures
            (r#"(?i)password\s*=\s*["'][^"']+["']"#, VulnerabilityKind::HardcodedSecret, "CWE-798", "Use environment variables or secure vaults for secrets"),
            (r#"(?i)api[_-]?key\s*[=:]\s*["'][^"']+["']"#, VulnerabilityKind::HardcodedSecret, "CWE-798", "Use environment variables or secure vaults for API keys"),
            (r#"(?i)secret\s*[=:]\s*["'][^"']+["']"#, VulnerabilityKind::HardcodedSecret, "CWE-798", "Use secure secret management"),
            (r#"(?i)token\s*[=:]\s*["'][^"']+["']"#, VulnerabilityKind::HardcodedSecret, "CWE-798", "Use secure token management"),
            (r#"(?i)md5\("#, VulnerabilityKind::WeakCrypto, "CWE-327", "Use stronger hashing algorithms like bcrypt or SHA-256"),
            (r#"(?i)sha1\("#, VulnerabilityKind::WeakCrypto, "CWE-327", "Use stronger hashing algorithms like SHA-256 or better"),
            
            // A03:2021 – Injection
            (r#"(?i)select\s+.*\s+from\s+.*\s+where.*["']\s*\+\s*"#, VulnerabilityKind::SqlInjection, "CWE-89", "Use parameterized queries or prepared statements"),
            (r#"(?i)\$\{[^}]*\}"#, VulnerabilityKind::SqlInjection, "CWE-89", "Use parameterized queries instead of string interpolation"),
            (r#"(?i)exec\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::CommandInjection, "CWE-78", "Validate and sanitize command inputs"),
            (r#"(?i)system\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::CommandInjection, "CWE-78", "Avoid using system() with user input"),
            (r#"(?i)eval\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::CommandInjection, "CWE-95", "Avoid using eval() with user input"),
            
            // A04:2021 – Insecure Design (Path Traversal)
            (r#"(?i)\.\.[\\/]"#, VulnerabilityKind::PathTraversal, "CWE-22", "Validate file paths and use absolute paths"),
            (r#"(?i)file_get_contents\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::PathTraversal, "CWE-22", "Validate file paths before reading"),
            
            // A05:2021 – Security Misconfiguration
            (r#"(?i)debug\s*[=:]\s*true"#, VulnerabilityKind::UnvalidatedInput, "CWE-489", "Disable debug mode in production"),
            (r#"(?i)error_reporting\s*\(\s*E_ALL"#, VulnerabilityKind::UnvalidatedInput, "CWE-209", "Disable detailed error reporting in production"),
            
            // A06:2021 – Vulnerable and Outdated Components (would need dependency analysis)
            
            // A07:2021 – Identification and Authentication Failures
            (r#"(?i)rand\s*\(\s*\)"#, VulnerabilityKind::InsecureRandom, "CWE-330", "Use cryptographically secure random number generators"),
            (r#"(?i)mt_rand\s*\(\s*\)"#, VulnerabilityKind::InsecureRandom, "CWE-330", "Use cryptographically secure random number generators"),
            ...
            (r#"(?i)curl_setopt\s*\([^,]*,\s*CURLOPT_URL\s*,\s*\$[^)]*\)"#, VulnerabilityKind::SSRF, "CWE-918", "Validate and whitelist URLs for external requests"),
            (r#"(?i)file_get_contents\s*\(["']https?://[^"']*\$[^"']*["']\)"#, VulnerabilityKind::SSRF, "CWE-918", "Validate external URLs"),
            
            // Cross-Site Scripting (XSS)
            (r#"(?i)echo\s+\$[^;]*;"#, VulnerabilityKind::XSS, "CWE-79", "Use proper output encoding/escaping"),
            (r#"(?i)print\s+\$[^;]*;"#, VulnerabilityKind::XSS, "CWE-79", "Use proper output encoding/escaping"),
            (r#"(?i)innerHTML\s*=\s*[^;]*\$[^;]*;"#, VulnerabilityKind::XSS, "CWE-79", "Use proper DOM manipulation methods"),
            
            // XXE (XML External Entity)
            (r#"(?i)simplexml_load_string\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::XXE, "CWE-611", "Disable external entity processing in XML parsers"),
            (r#"(?i)DOMDocument\s*\(\s*\)"#, VulnerabilityKind::XXE, "CWE-611", "Configure XML parser to prevent XXE attacks"),
        ];
...
impl BasicSecurityScanner {
    fn scan_language_specific(&self, content: &str, path: &Path, vulnerabilities: &mut Vec<SecurityVulnerability>) {
        let extension = path.extension().and_then(|ext| ext.to_str()).unwrap_or("");
    ...
    fn scan_javascript(&self, content: &str, path: &Path, vulnerabilities: &mut Vec<SecurityVulnerability>) {
        let js_patterns = vec![
            (r#"(?i)document\.write\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::XSS),
            (r#"(?i)eval\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::CommandInjection),
            (r#"(?i)setTimeout\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::CommandInjection),
            (r#"(?i)setInterval\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::CommandInjection),
        ];
        
        self.apply_patterns(js_patterns, content, path, vulnerabilities);
    }
    
    fn scan_python(&self, content: &str, path: &Path, vulnerabilities: &mut Vec<SecurityVulnerability>) {
        let py_patterns = vec![
            (r#"(?i)exec\s*\([^)]*input[^)]*\)"#, VulnerabilityKind::CommandInjection),
            (r#"(?i)eval\s*\([^)]*input[^)]*\)"#, VulnerabilityKind::CommandInjection),
            (r#"(?i)pickle\.loads\s*\([^)]*\)"#, VulnerabilityKind::UnvalidatedInput),
        ];
        
        self.apply_patterns(py_patterns, content, path, vulnerabilities);
    }
    
    fn scan_java(&self, content: &str, path: &Path, vulnerabilities: &mut Vec<SecurityVulnerability>) {
        let java_patterns = vec![
            (r#"(?i)Runtime\.getRuntime\(\)\.exec"#, VulnerabilityKind::CommandInjection),
            (r#"(?i)ProcessBuilder\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::CommandInjection),
        ];
        
        self.apply_patterns(java_patterns, content, path, vulnerabilities);
    }
    
    fn scan_rust(&self, content: &str, path: &Path, vulnerabilities: &mut Vec<SecurityVulnerability>) {
    ...
    fn scan_go(&self, content: &str, path: &Path, vulnerabilities: &mut Vec<SecurityVulnerability>) {
        let go_patterns = vec![
            (r#"(?i)exec\.Command\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::CommandInjection),
            (r#"(?i)fmt\.Sprintf\s*\([^)]*\$[^)]*\)"#, VulnerabilityKind::UnvalidatedInput),
        ];
        
        self.apply_patterns(go_patterns, content, path, vulnerabilities);
    }
    
    fn apply_patterns(&self, patterns: Vec<(&str, VulnerabilityKind)>, content: &str, path: &Path, vulnerabilities: &mut Vec<SecurityVulnerability>) {
...
mod tests {
    ...
    fn test_quality_score_calculation() {
        let analyzer = RepositoryAnalyzer::new(
            Arc::new(SymbolIndexer::new(Arc::new(DatabaseManager::default())).await.unwrap()),
