...
use super::parser::{TreeSitterParser, Edit, IncrementalParseState};
...
impl IncrementalParser {
    ...
    pub async fn parse_incremental(
        ...
        let (tree, parse_time) = if let Some(prev_tree) = previous_tree {
            ...
            let tree = parser.parser.parse(new_content, None)
        ...
        if has_previous_state && parse_time > 5.0 {
            warn!("Incremental parse took {}ms, exceeding 5ms target", parse_time);
        }
...
impl EditDetector {
    ...
    pub fn detect_edit(old: &str, new: &str) -> Option<Edit> {
        // Find common prefix
        let common_prefix_len = old.bytes()
            .zip(new.bytes())
            .take_while(|(a, b)| a == b)
            .count();
        ...
        let common_suffix_len = old_suffix.iter()
            .rev()
    ...
    async fn test_cache_eviction() {
        ...
        for i in 0..6 {
            ...
            let edit = Edit {
                ...
                new_end_position: crate::core::Position { line: 0, column: content.len(), offset: content.len() },
            };
