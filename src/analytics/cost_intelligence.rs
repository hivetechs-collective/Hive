...
impl CostIntelligence {
    ...
    pub async fn get_insights(&self) -> Result<CostInsights> {
        ...
        // Check budget status
        let budget_status = self.budget_monitor
            .check_budget_status(&spend_analysis).await?;
    ...
    pub async fn analyze_model_efficiency(&self, model: &str) -> Result<ModelEfficiency> {
        ...
        let model_activities: Vec<_> = activities.into_iter()
            .filter(|a| a.model_used.as_ref() == Some(&model.to_string()))
            .collect();
        
        self.cost_analyzer.calculate_model_efficiency(model, &model_activities)
    }
    ...
    fn calculate_efficiency_metrics(&self, analysis: &SpendAnalysis) -> Result<EfficiencyMetrics> {
        ...
        let overall_efficiency = if !model_efficiency.is_empty() {
            model_efficiency.values()
                .map(|e| e.efficiency_score)
                .sum::<f64>() / model_efficiency.len() as f64
        } else {
            0.0
        };
    ...
    fn calculate_efficiency_score(&self, spend: &ModelSpend) -> f64 {
        ...
        let utilization_factor = spend.output_tokens as f64 / (spend.input_tokens + spend.output_tokens) as f64;
    ...
    fn analyze_spending_patterns(&self, activities: &[ActivityLog]) -> Result<Vec<SpendingPattern>> {
    ...
    fn analyze_hourly_pattern(&self, activities: &[ActivityLog]) -> Result<Option<SpendingPattern>> {
        // Group activities by hour
        let mut hourly_costs: HashMap<u32, f64> = HashMap::new();
        
        for activity in activities {
            if let (Ok(timestamp), Some(cost)) = (activity.created_at.parse::<DateTime<Utc>>(), activity.cost) {
        ...
        // Find peak hours
        let max_hour = hourly_costs.iter()
        ...
        for (user, cost) in user_costs {
            ...
            if percentage > 20.0 {
                patterns.push(SpendingPattern {
                    ...
                    description: format!("User {} accounts for {:.1}% of total spend", user, percentage),
    ...
    fn detect_spending_anomalies(&self, activities: &[ActivityLog]) -> Result<Vec<SpendingPattern>> {
        let mut patterns = Vec::new();
        
        // Simple anomaly detection based on cost outliers
        let costs: Vec<f64> = activities.iter()
            .filter_map(|a| a.cost)
            .collect();
    ...
    async fn get_historical_spending(&self, days: u32) -> Result<Vec<DailySpend>> {
        ...
        Ok(daily_spending.into_iter()
            .map(|(date, cost)| DailySpend { date: date.to_string(), cost })
            .collect())
    }
    ...
    fn forecast_spending(&self, historical: &[DailySpend], days: u32) -> Result<SpendingForecast> {
        ...
        for i in 0..days {
            ...
            predicted_spend.push(DailySpend {
                date: (Utc::now() + Duration::days(i as i64)).format("%Y-%m-%d").to_string(),
                cost: predicted,
            });
        }
        ...
        let trend = if slope > 0.01 { SpendingTrend::Increasing }
            else if slope < -0.01 { SpendingTrend::Decreasing }
...
impl CostInsights {
    ...
    pub fn format_markdown(&self) -> Result<String> {
        ...
        output.push_str(&format!("- Cost per Token: ${:.6}\n", self.efficiency_metrics.cost_per_token));
        ...
        output.push_str(&format!("- Current: ${:.2} / ${:.2} ({:.1}%)\n",
        ...
        if !self.optimization_opportunities.is_empty() {
            ...
            for (i, opt) in self.optimization_opportunities.iter().take(3).enumerate() {
                output.push_str(&format!("{}. **{}**\n", i + 1, opt.title));
                output.push_str(&format!("   - Potential Savings: ${:.2}\n", opt.potential_savings));
...
impl CostAnalyzer {
    fn new() -> Self {
        ...
        // OpenAI models
        model_pricing.insert("gpt-4".to_string(), ModelPricing {
        ...
        model_pricing.insert("gpt-4-turbo".to_string(), ModelPricing {
            provider: "openai".to_string(),
        ...
        model_pricing.insert("gpt-3.5-turbo".to_string(), ModelPricing {
            provider: "openai".to_string(),
        ...
        // Anthropic models
        model_pricing.insert("claude-3-opus".to_string(), ModelPricing {
        ...
        model_pricing.insert("claude-3-sonnet".to_string(), ModelPricing {
            provider: "anthropic".to_string(),
        ...
        model_pricing.insert("claude-3-haiku".to_string(), ModelPricing {
            provider: "anthropic".to_string(),
    ...
    async fn analyze_spending(&self, activities: &[ActivityLog]) -> Result<SpendAnalysis> {
            ...
            if let Some(cost) = activity.cost {
                ...
                if let Some(model) = &activity.model_used {
                    ...
                    if let Some(metadata) = &activity.metadata {
                        if let Ok(meta) = serde_json::from_str::<HashMap<String, serde_json::Value>>(metadata) {
        ...
        for model_spend in spend_by_model.values_mut() {
            if model_spend.query_count > 0 {
                model_spend.avg_cost_per_query = model_spend.total_cost / model_spend.query_count as f64;
            }
        }
    ...
    fn calculate_model_efficiency(&self, model: &str, activities: &[ActivityLog]) -> Result<ModelEfficiency> {
        ...
        let avg_cost = if query_count > 0 { total_cost / query_count as f64 } else { 0.0 };
        let efficiency_score = if avg_cost > 0.0 { 100.0 / (1.0 + avg_cost * 10.0) } else { 0.0 };
    ...
    fn new() -> Self {
        ...
        optimization_rules.push(OptimizationRule {
            ...
            condition: Box::new(|insights| {
                insights.spend_by_model.values()
                    .any(|s| s.avg_cost_per_query > 0.15)
            }),
            generator: Box::new(|insights| {
                let high_cost_models: Vec<_> = insights.spend_by_model.iter()
                    .filter(|(_, s)| s.avg_cost_per_query > 0.15)
                    .collect();
                
                let potential_savings = high_cost_models.iter()
        ...
        optimization_rules.push(OptimizationRule {
            ...
            condition: Box::new(|insights| {
                insights.efficiency_metrics.cache_hit_rate < 0.2
            }),
            generator: Box::new(|insights| {
    ...
    fn find_opportunities(
        ...
        opportunities.sort_by(|a, b| {
            a.priority.cmp(&b.priority)
                .then(b.potential_savings.partial_cmp(&a.potential_savings).unwrap())
        });
...
impl BudgetMonitor {
    fn new() -> Self {
        let alert_thresholds = vec![
            AlertThreshold { percentage: 50.0, severity: AlertSeverity::Info },
            AlertThreshold { percentage: 80.0, severity: AlertSeverity::Warning },
            AlertThreshold { percentage: 100.0, severity: AlertSeverity::Critical },
        ];
        ...
        for threshold in &self.alert_thresholds {
            if utilization_percent >= threshold.percentage && budget.alerts_enabled {
                alerts.push(BudgetAlert {
                    alert_type: if utilization_percent >= 100.0 { AlertType::ExceededLimit }
                        else { AlertType::ApproachingLimit },
...
mod tests {
    ...
    fn test_spending_forecast() {
        let historical = vec![
            DailySpend { date: "2024-01-01".to_string(), cost: 10.0 },
            DailySpend { date: "2024-01-02".to_string(), cost: 12.0 },
            DailySpend { date: "2024-01-03".to_string(), cost: 11.0 },
            DailySpend { date: "2024-01-04".to_string(), cost: 15.0 },
            DailySpend { date: "2024-01-05".to_string(), cost: 14.0 },
        ];
        ...
        assert!(matches!(forecast.trend, SpendingTrend::Increasing | SpendingTrend::Stable));
    }
}