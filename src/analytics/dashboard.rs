...
impl DashboardEngine {
    ...
    pub async fn update_widget(&self, widget_id: &str) -> Result<Widget> {
        let layout = self.layout_manager.get_active_layout().await?;
        let widget_config = layout.widgets.iter()
    ...
    pub async fn render_ascii(&self, width: u16, height: u16) -> Result<String> {
        ...
        self.renderer.render_dashboard_ascii(&dashboard, width, height)
    }
    ...
    async fn fetch_gauge_data(&self, data_source: &str) -> Result<WidgetData> {
        ...
        metadata.insert("thresholds".to_string(), serde_json::json!({
            "good": 200,
            "warning": 500,
            "critical": 1000
        }));
    ...
    async fn fetch_table_data(&self, data_source: &str) -> Result<WidgetData> {
        ...
        for activity in activities {
            ...
            let cost = activity.cost.map(|c| format!("${:.3}", c)).unwrap_or("-".to_string());
    ...
    async fn fetch_alerts_data(&self, data_source: &str) -> Result<WidgetData> {
        ...
        let activities = ActivityLog::get_recent(100).await?;
        let total_cost: f64 = activities.iter()
            .filter_map(|a| a.cost)
            .sum();
...
impl WidgetFactory {
    fn new() -> Self {
        ...
        widget_templates.insert(WidgetType::LineChart, WidgetTemplate {
            default_options: WidgetOptions {
        ...
        widget_templates.insert(WidgetType::GaugeChart, WidgetTemplate {
            default_options: WidgetOptions {
...
impl DashboardRenderer {
    ...
    fn render_dashboard_ascii(&self, dashboard: &Dashboard, width: u16, height: u16) -> Result<String> {
        let mut output = String::new();
        
        // Dashboard header
        output.push_str(&format!("╭{:─^width$}╮\n", " Analytics Dashboard ", width = width as usize - 2));
        output.push_str(&format!("│ Last updated: {:^width$} │\n", 
...
impl WidgetRenderer for GaugeRenderer {
    fn render_ascii(&self, widget: &Widget, width: u16, height: u16) -> Result<String> {
        ...
        if let Some(value) = widget.data.primary_value {
            let max = widget.data.metadata.get("max")
                .and_then(|v| v.as_f64())
                .unwrap_or(100.0);
...
mod tests {
    ...
    fn test_sparkline_creation() {
        let points = vec![
            DataPoint { timestamp: Utc::now(), value: 1.0, label: None },
            DataPoint { timestamp: Utc::now(), value: 5.0, label: None },
            DataPoint { timestamp: Utc::now(), value: 3.0, label: None },
            DataPoint { timestamp: Utc::now(), value: 7.0, label: None },
            DataPoint { timestamp: Utc::now(), value: 2.0, label: None },
        ];
    ...
    fn test_refresh_rate_duration() {
        assert_eq!(RefreshRate::Realtime.to_duration(), std::time::Duration::from_millis(500));
        assert_eq!(RefreshRate::Fast.to_duration(), std::time::Duration::from_secs(1));
        assert_eq!(RefreshRate::Normal.to_duration(), std::time::Duration::from_secs(5));
    }
}