...
impl CliFramework {
    ...
    pub async fn new(config: Config) -> HiveResult<Self> {
        ...
        let consensus_engine =
            Arc::new(ConsensusEngine::new(Some(database_manager.clone())).await?);
        ...
        let security_system = Arc::new(
            SecuritySystem::new(security_config, Some(config.core_dirs.data_dir.clone())).await?,
        );
        ...
        let planning_engine = Arc::new(RwLock::new(
            PlanningEngine::new(consensus_engine.clone()).await?,
        ));
    ...
    pub async fn run_interactive(&mut self) -> HiveResult<()> {
        ...
        println!(
            "{}",
            style("🐝 Welcome to Hive AI Interactive Mode")
                .bold()
                .cyan()
        );
        println!(
    ...
    pub async fn plan(&mut self, task: &str, repository: bool) -> HiveResult<()> {
        ...
        if repository {
            ...
            if !self
                .security_system
                .is_directory_trusted(&current_dir, Some("analyze"))
                .await?
            {
                if !self
                    .security_system
                    .request_directory_trust(&current_dir, Some("analyze"))
                    .await?
                {
                    return Err(HiveError::Security {
                        message: "Directory access denied".to_string(),
                    });
                }
            }
        }
        ...
        let plan = if repository {
            ...
            self.planning_engine
                .write()
                .await
                .create_plan_with_repository(task, &current_dir, context)
                .await?
        } else {
            self.planning_engine
                .read()
    ...
    pub async fn analyze(&mut self, path: Option<&Path>, depth: &str) -> HiveResult<()> {
        ...
        if !self
            .security_system
            .is_directory_trusted(&target_path, Some("analyze"))
            .await?
        {
            if !self
                .security_system
                .request_directory_trust(&target_path, Some("analyze"))
                .await?
            {
                return Err(HiveError::Security {
                    message: "Directory access denied".to_string(),
    ...
    pub async fn show_status(&self) -> HiveResult<()> {
        ...
        println!(
            ...
            security_metrics.compliance_violations
        );
        println!();
    ...
    pub async fn list_trust_decisions(&self) -> HiveResult<()> {
        ...
        for decision in decisions {
            let status = if decision.trusted {
                "✅ Trusted"
            } else {
                "❌ Denied"
            };
    ...
    pub async fn show_audit_logs(&self, limit: usize) -> HiveResult<()> {
        ...
        let events = vec![
            ...
            (
                ...
                "TrustGranted",
                "system",
                "Trust granted to /projects/hive",
            ),
            (
                "2024-01-15 14:25:03",
                "ConfigChanged",
            ...
            (
                ...
                "HookRegistered",
                "user@example.com",
                "New hook registered: code-review",
            ),
            (
                "2024-01-15 14:18:12",
                "PlanExecuted",
                "user@example.com",
                "Plan 'feature-implementation' executed",
            ),
        ];
    ...
    pub async fn register_hook(&mut self, config_path: &Path) -> HiveResult<()> {
        ...
        let hook_id = self
            .hooks_system
            .register_hook(config_path.to_path_buf())
            .await?;

        println!(
            "✅ Hook registered successfully with ID: {}",
            style(&hook_id).cyan()
        );
    ...
    pub async fn test_hook(&mut self, config_path: &Path, event: &str) -> HiveResult<()> {
        ...
        let test_event = crate::hooks::events::HookEvent {
            ...
            timestamp: Utc::now(),
            source: crate::hooks::events::EventSource::CLI {
        ...
        let result = self
            .hooks_system
            .test_hook(config_path.to_path_buf(), test_event)
            .await?;
    ...
    pub async fn enable_hook(&mut self, hook_id: &str) -> HiveResult<()> {
        ...
        self.hooks_system
            .enable_hook(&HookId(hook_id.to_string()))
            .await?;
    ...
    pub async fn disable_hook(&mut self, hook_id: &str) -> HiveResult<()> {
        ...
        use crate::hooks::HookId;
        self.hooks_system
            .disable_hook(&HookId(hook_id.to_string()))
            .await?;
    ...
    pub async fn show_config(&self) -> HiveResult<()> {
        ...
        println!(
            "  Data Directory: {}",
            self.config.core_dirs.data_dir.display()
        );
        println!(
            "  Config Directory: {}",
            self.config.core_dirs.config_dir.display()
        );
        println!();
        ...
        println!(
            "  Trust Dialog: {}",
            self.config.security.trust_dialog.enabled
        );
        println!();
        ...
        println!(
            "  Collection Enabled: {}",
            self.config.analytics.collection_enabled
        );
    ...
    pub async fn search_memory(&self, query: &str) -> HiveResult<()> {
        ...
        let results = vec![
            (
                "2024-01-15 14:30",
                "Planning Session",
                "Discussed implementing authentication system with JWT tokens",
            ),
            (
                "2024-01-14 09:15",
                "Code Review",
                "Reviewed security vulnerabilities in user input validation",
            ),
            (
                "2024-01-13 16:45",
                "Architecture",
                "Designed microservices architecture for scaling",
            ),
        ];
    ...
    pub async fn show_compliance_report(&self, standard: &str) -> HiveResult<()> {
        println!(
            "📋 Compliance Report: {}",
            style(standard.to_uppercase()).bold()
        );
        println!(
            "{}",
            style(
    ...
    async fn interactive_hooks(&mut self) -> HiveResult<()> {
        ...
        let options = vec!["List hooks", "Register new hook", "Hook management help"];
        ...
        match selection {
            ...
            2 => {
                ...
                let std_selection = Select::with_theme(&theme)
                    ...
                    .default(0)
                    .interact()?;
                self.show_compliance_report(standards[std_selection])
                    .await?;
            }
    ...
    fn display_plan(&self, plan: &crate::planning::types::Plan) {
        ...
        println!(
            "{}",
            style(&format!("📋 Plan: {}", plan.title)).bold().cyan()
        );
        println!(
            "{}",
            style(
        ...
        for (i, task) in plan.tasks.iter().enumerate() {
            ...
            println!(
                ...
                task.estimated_duration.num_minutes()
            );
            println!();
        }
            ...
            for risk in &plan.risks {
                println!(
                    ...
                    match risk.severity {
                        ...
                        crate::planning::types::RiskSeverity::Critical =>
        ...
        println!(
            ...
            plan.timeline.total_duration.num_hours()
        );
        println!(
            "Earliest start: {}",
            plan.timeline.start_date.format("%Y-%m-%d")
        );
        println!(
            "Expected completion: {}",
            plan.timeline.end_date.format("%Y-%m-%d")
        );
    }
}
