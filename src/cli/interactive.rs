...
use crate::cli::{
    banner::show_startup_banner, check_api_status, check_internet_connection,
    get_current_dir_display, get_memory_usage,
};
use anyhow::Result;
...
impl InteractiveSession {
    ...
    async fn print_help_info(&self) -> Result<()> {
        ...
        println!(
            ...
            style("ask <question>").cyan()
        );
        println!(
            "   {} - Analyze repository or file",
            style("analyze <path>").cyan()
        );
        println!(
            "   {} - Create a development plan",
            style("plan <goal>").cyan()
        );
        println!(
            "   {} - Suggest improvements to a file",
            style("improve <file>").cyan()
        );
        println!(
            "   {} - Show configured enterprise hooks",
            style("hooks list").cyan()
        );
        println!(
    ...
    async fn print_status_info(&self) -> Result<()> {
        ...
        let config_exists = tokio::fs::metadata(config_dir.join("config.toml"))
            .await
            .is_ok();
        let db_exists = tokio::fs::metadata(config_dir.join("hive-ai.db"))
            .await
            .is_ok();
        ...
        println!(
            "   Working Directory: {}",
            style(get_current_dir_display()).cyan()
        );
    ...
    async fn handle_ask_command(&self, question: &str) -> Result<()> {
        ...
        if self.mode == "planning" {
            println!(
                "📋 {} Planning mode - analyzing complexity...",
                style("Mode:").yellow()
            );
        }
    ...
    async fn handle_improve_command(&self, file: &str) -> Result<()> {
        ...
        println!();
        println!(
            "❓ Apply these improvements? {} to proceed",
            style("(y/n)").dim()
        );
