...
use crate::migration::{
    ...
    ui::{MigrationWizard, MigrationUIConfig, UITheme, run_quick_migration},
...
use crate::core::config::{get_config, set_config_value, get_config_value, reset_config, get_hive_config_dir};
use rusqlite::params;

/// Handle a CLI command
pub async fn handle_command(command: Commands) -> Result<()> {
    match command {
        Commands::Initialize { path, force, non_interactive } => {
            handle_init(path, force, non_interactive).await
        }
        Commands::Ask { question, profile, plan, context, max_tokens, stream } => {
            handle_ask(question, profile, plan, context, max_tokens, stream).await
        }
        Commands::Consensus { query, profile, detailed, output } => {
            handle_consensus(query, profile, detailed, output).await
        }
        Commands::Analyze { target, depth, focus, output, dependencies, recommendations } => {
            handle_analyze(target, depth, focus, output, dependencies, recommendations).await
        }
        Commands::Search { query, kind, path, limit, fuzzy } => {
            crate::commands::search::handle_search(query, kind, path, limit, fuzzy).await
        }
        Commands::Plan { goal, depth, collaborative, output, risks, timeline } => {
            crate::commands::planning::handle_plan(goal, depth, collaborative, output, risks, timeline).await
        }
        Commands::Execute { plan, auto, validation, dry_run, continue_on_error } => {
            handle_execute(plan, auto, validation, dry_run, continue_on_error).await
        }
        Commands::Decompose { task, depth, estimate } => {
            crate::commands::planning::handle_decompose(task, depth, estimate).await
        }
        Commands::AnalyzeRisks { project, mitigation } => {
            crate::commands::planning::handle_analyze_risks(project, mitigation).await
        }
        Commands::Timeline { project, dependencies } => {
        ...
        Commands::Apply { changes, preview, approve } => {
        ...
        Commands::Transform { query, target, safe } => {
            crate::commands::improve::handle_transform(query, target, safe).await
        }
        Commands::Undo { transaction: _ } => {
            crate::commands::improve::handle_undo().await
        }
        Commands::Redo { transaction: _ } => {
            crate::commands::improve::handle_redo().await
        }
        ...
        Commands::Analytics { command } => {
            handle_analytics(command).await
        }
        Commands::Memory { command } => {
            handle_memory(command).await
        }
        Commands::Tool { name, params, chain, list } => {
            handle_tool(name, params, chain, list).await
        }
        Commands::Serve { mode, port, host, cors } => {
            handle_serve(mode, port, host, cors).await
        }
        Commands::Index { command } => {
            handle_index(command).await
        }
        Commands::References { symbol, file, line, include_declaration, group_by_file } => {
            crate::commands::search::handle_references(symbol, file, line).await
        }
        Commands::CallGraph { function, depth, format, incoming, outgoing } => {
            crate::commands::search::handle_call_graph(function, Some(depth), Some(format)).await
        }
        Commands::FindCircularDeps { path, format, severe_only, suggest_fixes } => {
            handle_find_circular_deps(path, format, severe_only, suggest_fixes).await
        }
        Commands::DependencyLayers { path, format, show_violations, max_layers } => {
            handle_dependency_layers(path, format, show_violations, max_layers).await
        }
        Commands::DetectLanguage { file, confidence, detailed } => {
            handle_detect_language(file, confidence, detailed).await
        }
        Commands::EditPerformanceTest { iterations, file, language, detailed } => {
        ...
        Commands::Trust { command } => {
            handle_trust(command).await
        }
        Commands::Hooks { command } => {
            handle_hooks(command).await
        }
        Commands::Interactive { mode, no_tui } => {
            handle_interactive(mode, no_tui).await
        }
        Commands::Tui { force, layout } => {
            handle_tui(force, layout).await
        }
        Commands::Status { detailed, check_apis, performance } => {
            handle_status(detailed, check_apis, performance).await
        }
        Commands::Completion { shell, output } => {
            handle_completion(shell, output).await
        }
        Commands::Shell { command } => {
            handle_shell_command(command).await
        }
        Commands::SelfUpdate { check_only, force, version, rollback, list_versions } => {
            handle_self_update(check_only, force, version, rollback, list_versions).await
        }
        Commands::Uninstall { dry_run, preserve_config, preserve_data, force, backup } => {
        ...
        Commands::Security { command } => {
            crate::commands::security::handle_security(command).await
        }
    }
}
...
async fn handle_init(path: Option<PathBuf>, force: bool, _non_interactive: bool) -> Result<()> {
    ...
    if hive_dir.exists() && !force {
        println!("⚠️  {} Hive is already initialized in this directory", style("Warning:").yellow());
...
"#;
    ...
    println!("  1. {} to configure OpenRouter API key", style("hive config set openrouter.api_key <key>").cyan());
    println!("  2. {} to analyze your codebase", style("hive analyze .").cyan());
    println!("  3. {} to start asking questions", style("hive ask \"What does this code do?\"").cyan());
    println!("  4. {} for the interactive interface", style("hive interactive").cyan());
    println!();
...
async fn handle_ask(
    ...
    if let Some(context_file) = context {
        println!("📄 {} {}", style("Including context from").dim(), style(context_file.display()).cyan());
    }
...
async fn handle_consensus(
    ...
    println!("🧠 {} 4-stage consensus analysis...", style("Starting").bold());
    ...
    if std::env::var("OPENROUTER_API_KEY").is_err() {
        println!("⚠️  {} OpenRouter API key not found", style("Warning:").yellow().bold());
        ...
        if let Some(output_path) = output {
            println!("💾 {} result to {}", style("Saving").bold(), style(output_path.display()).cyan());
  ...
  "result": "Simulation consensus complete"
}}"#, query, profile, chrono::Utc::now().to_rfc3339());
            
            tokio::fs::write(&output_path, result).await?;
        }
        
        println!("✅ {} Simulation complete!", style("Success:").green().bold());
        return Ok(());
    }
    ...
    match engine.process(&query, None).await {
        Ok(result) => {
            ...
            println!();
            println!("✅ {} Consensus analysis complete!", style("Success:").green().bold());
            ...
            if let Some(final_result) = &result.result {
                println!("📋 {} {}", style("Result:").bold(), style("─────────────────────────────────────────").dim());
            ...
            if detailed {
                println!();
                println!("🔍 {} {}", style("Detailed Breakdown:").bold(), style("─────────────────────────────").dim());
                for (i, stage_result) in result.stages.iter().enumerate() {
                    ...
                    if let Some(analytics) = &stage_result.analytics {
                        println!("   Duration: {:.2}s, Cost: ${:.4}", analytics.duration, analytics.cost);
                    }
                    println!("   Response: {}", 
        ...
        Err(e) => {
            ...
            println!("💡 Make sure your OPENROUTER_API_KEY is valid and you have sufficient credits");
            return Err(e);
        }
    }
...
async fn handle_execute(
    ...
    if dry_run {
        println!("🧪 {} execution (no changes will be made)...", style("Dry run").yellow().bold());
    } else {
        println!("⚡ {} plan: {}", style("Executing").bold(), style(&plan).cyan());
    }
    ...
    println!();
    if dry_run {
        println!("✅ {} Dry run completed successfully!", style("Success:").green().bold());
        println!("   All tasks are ready for execution");
        println!("   Run without {} to execute for real", style("--dry-run").bold());
    } else {
        println!("✅ {} Plan executed successfully!", style("Success:").green().bold());
...
async fn handle_analytics(command: AnalyticsCommands) -> Result<()> {
    use crate::commands::analytics::{AnalyticsCommand, ReportTypeArg, PeriodArg};
    
    // Convert CLI args to our analytics command format
    let analytics_cmd = match &command {
        AnalyticsCommands::Usage { period: _, detailed } => {
            AnalyticsCommand::Generate { comprehensive: *detailed }
        }
        AnalyticsCommands::Performance { period: _, system } => {
            AnalyticsCommand::Generate { comprehensive: *system }
        }
        AnalyticsCommands::Cost { period, by_model, alerts } => {
            ...
            AnalyticsCommand::Cost { breakdown: *by_model, optimize: *alerts, period: period_arg }
        }
        AnalyticsCommands::Quality { period: _, code_quality } => {
            AnalyticsCommand::Generate { comprehensive: *code_quality }
        }
        AnalyticsCommands::Report { report_type, period, charts, output } => {
            ...
            AnalyticsCommand::Report { r#type: type_arg, period: period_arg, format: "markdown".to_string() }
        }
        AnalyticsCommands::Trends { metric, period, predict } => {
            let period_arg = match period.as_str() {
                "day" => PeriodArg::Day,
                "week" => PeriodArg::Week,
                "month" => PeriodArg::Month,
    ...
    match command {
        AnalyticsCommands::Usage { period, detailed } => {
            println!("📊 {} usage analytics for: {}", style("Generating").bold(), style(&period).cyan());
            ...
            println!();
            println!("📈 {} Analytics ({}):", style("Usage").bold().green(), period);
        ...
        AnalyticsCommands::Performance { period, system } => {
            println!("⚡ {} performance analysis for: {}", style("Analyzing").bold(), style(&period).cyan());
            
            tokio::time::sleep(std::time::Duration::from_millis(400)).await;
            
            println!();
            println!("⚡ {} Metrics ({}):", style("Performance").bold().green(), period);
        ...
        AnalyticsCommands::Cost { period, by_model, alerts } => {
            println!("💰 {} cost analysis for: {}", style("Calculating").bold(), style(&period).cyan());
            ...
            if by_model {
                ...
                println!("    • Claude-3-Opus: ${} ({}%)", style("45.23").cyan(), style("35").dim());
                println!("    • GPT-4-Turbo: ${} ({}%)", style("38.90").cyan(), style("30").dim());
                println!("    • Claude-3-Sonnet: ${} ({}%)", style("28.45").cyan(), style("22").dim());
                println!("    • GPT-4o: ${} ({}%)", style("15.31").cyan(), style("13").dim());
            }
            
            if alerts {
                println!();
                println!("  {} Budget alerts:", style("🚨").yellow());
                println!("    • Monthly budget: {} of ${} used", style("78%").yellow(), style("200").dim());
                println!("    • Daily average: {} over target", style("+12%").red());
            }
        }
        
        AnalyticsCommands::Quality { period, code_quality } => {
            println!("✨ {} quality metrics for: {}", style("Analyzing").bold(), style(&period).cyan());
            
            tokio::time::sleep(std::time::Duration::from_millis(450)).await;
            
            println!();
            println!("✨ {} Metrics ({}):", style("Quality").bold().green(), period);
            println!("  🎯 Accuracy rate: {}%", style("94.2").green().bold());
            println!("  🧠 Consensus agreement: {}%", style("91.5").green().bold());
            ...
            if code_quality {
                ...
                println!("    • Quality score increase: {}%", style("+23").green().bold());
            }
        }
        ...
        AnalyticsCommands::Report { report_type, period, charts, output } => {
            ...
            match report_type.as_str() {
                ...
                "performance" => {
                    ...
                    println!("    • Throughput: {} queries/hour", style("847").blue().bold());
            ...
            if let Some(output_path) = output {
                println!();
                println!("💾 {} report to {}", style("Saving").bold(), style(output_path.display()).cyan());
        ...
        AnalyticsCommands::Trends { metric, period, predict } => {
            ...
            if let Some(days) = predict {
                println!("🔮 {} prediction for {} days", style("Enabling").dim(), days);
            }
            ...
            println!("  📈 Growth rate: {}% ({})", style("+15.3").green(), style("upward trend").dim());
            println!("  📉 Best day: {} queries", style("1,891").green());
            println!("  📊 Average: {} queries/day", style("1,156").blue());
            
            if let Some(days) = predict {
                println!();
                println!("  🔮 {} forecast:", style(&format!("{}-day", days)).yellow());
                println!("    • Predicted peak: {} queries", style("2,100").cyan());
                println!("    • Expected average: {} queries/day", style("1,340").cyan());
...
async fn handle_memory(command: MemoryCommands) -> Result<()> {
    ...
    let memory_cmd = match &command {
        MemoryCommands::Search { query, limit, since: _, context: _ } => {
            MemoryCommand::Search { query: query.clone(), semantic: true, limit: *limit }
        }
        MemoryCommands::Stats { detailed: _, health: _ } => {
            MemoryCommand::Stats
        }
        MemoryCommands::Export { output: _, format: _, since: _, include_private: _ } => {
            // For now, just show a message about export
            println!("📤 Export functionality not yet implemented");
            return Ok(());
        }
        MemoryCommands::Import { file: _, format: _, merge: _ } => {
            // For now, just show a message about import
            println!("📥 Import functionality not yet implemented");
            return Ok(());
        }
        MemoryCommands::Clear { all: _, older_than: _, force } => {
            MemoryCommand::Clear { force: *force }
        }
        MemoryCommands::Knowledge { command: knowledge_cmd } => {
            match knowledge_cmd {
                KnowledgeCommands::Stats { detailed } => {
                    MemoryCommand::Analyze { patterns: false, insights: true }
                }
                KnowledgeCommands::Query { query, limit, paths } => {
                    MemoryCommand::Search { query: query.to_string(), semantic: true, limit: *limit }
                }
                KnowledgeCommands::Export { output, format, attributes } => {
                    MemoryCommand::Knowledge { export: true, format: format.to_string() }
                }
                KnowledgeCommands::Visualize { output, layout, focus } => {
                    MemoryCommand::Knowledge { export: true, format: "dot".to_string() }
                }
            }
        }
    };
    ...
    match command {
        MemoryCommands::Search { query, limit, since, context } => {
            ...
            println!();
            println!("📝 Found {} relevant conversations:", style(&limit).blue().bold());
            ...
            for i in 1..=std::cmp::min(limit, 5) {
                ...
                if context {
                    ...
                    println!("     {} Analyzed code structure and provided 5 optimization strategies", style("→").dim());
                }
            }
        }
        ...
        MemoryCommands::Stats { detailed, health } => {
            ...
            if detailed {
                ...
                println!("    • Code discussions: {} ({}%)", style("423").blue(), style("34").dim());
                println!("    • Architecture queries: {} ({}%)", style("289").blue(), style("23").dim());
                println!("    • Bug analysis: {} ({}%)", style("234").blue(), style("19").dim());
                println!("    • Planning sessions: {} ({}%)", style("178").blue(), style("14").dim());
                println!("    • Other: {} ({}%)", style("123").blue(), style("10").dim());
            }
        ...
        MemoryCommands::Export { output, format, since, include_private } => {
            let output_path = output.unwrap_or_else(|| PathBuf::from(format!("hive_memory_export.{}", format)));
            
            println!("📤 {} conversation history to {}...", 
        ...
        MemoryCommands::Import { file, format, merge } => {
            println!("📥 {} memory from {}...", 
            ...
            println!("   {} duplicate conversations skipped", style("23").yellow());
            println!("   Total conversations: {}", style("1,589").blue().bold());
        }
        
        MemoryCommands::Clear { all, older_than, force } => {
            if all {
                println!("🗑️  {} to clear ALL memory data", style("WARNING: Preparing").red().bold());
            } else if let Some(days) = older_than {
                println!("🗑️  {} conversations older than {} days...", 
                    style("Clearing").bold(),
                    style(days).yellow()
                );
            } else {
            ...
            if !force {
                println!("⚠️  {} This action cannot be undone!", style("WARNING:").yellow().bold());
                println!("   Use {} to proceed without confirmation", style("--force").bold());
                return Ok(());
            }
            ...
            if all {
                println!("✅ {} All memory data cleared", style("Success:").green().bold());
            } else if let Some(_days) = older_than {
                println!("✅ {} Cleared {} old conversations", 
...
async fn handle_knowledge_command(command: KnowledgeCommands) -> Result<()> {
    match command {
        KnowledgeCommands::Stats { detailed } => {
            ...
            println!();
            println!("🕸️  {} Graph Statistics:", style("Knowledge").bold().green());
            ...
            if detailed {
                ...
                println!("    • Concepts: {} ({}%)", style("6,234").blue(), style("40").dim());
                println!("    • Functions: {} ({}%)", style("4,567").blue(), style("29").dim());
                println!("    • Files: {} ({}%)", style("2,891").blue(), style("18").dim());
                println!("    • Classes: {} ({}%)", style("1,234").blue(), style("8").dim());
                println!("    • Other: {} ({}%)", style("708").blue(), style("5").dim());
                
                println!();
                println!("  {} Edge type breakdown:", style("🔗").bold());
                println!("    • Semantic relations: {} ({}%)", style("18,456").blue(), style("39").dim());
                println!("    • Call relationships: {} ({}%)", style("14,233").blue(), style("30").dim());
                println!("    • Import/usage: {} ({}%)", style("9,876").blue(), style("21").dim());
                println!("    • Inheritance: {} ({}%)", style("3,456").blue(), style("7").dim());
                println!("    • Other: {} ({}%)", style("1,870").blue(), style("3").dim());
            }
        }
        ...
        KnowledgeCommands::Query { query, limit, paths } => {
            ...
            println!();
            println!("🔍 {} Query Results (showing top {}):", 
            ...
            for i in 1..=std::cmp::min(limit, 8) {
                ...
                println!("     {} Located in src/consensus/mod.rs:142", style("→").dim());
                println!("     {} Relevance: {}%", style("→").dim(), style("94").green());
                
                if paths && i <= 3 {
                    println!("     {} Path: consensus → validator → result → process", style("🛤️ ").dim());
                }
            }
        }
        
        KnowledgeCommands::Export { output, format, attributes } => {
            println!("📤 {} knowledge graph to {}...", 
                style("Exporting").bold(),
            ...
            let export_content = match format.as_str() {
                "json" => r#"{"nodes": [], "edges": [], "metadata": {"exported_at": "2024-07-02T10:30:00Z"}}"#,
        ...
        KnowledgeCommands::Visualize { output, layout, focus } => {
            println!("🎨 {} knowledge graph visualization...", style("Creating").bold());
            ...
            let viz_content = format!(r#"# Knowledge Graph Visualization
...
"#, 
            ...
            println!();
            println!("✅ {} Visualization created!", style("Success:").green().bold());
            let node_count = if focus.is_empty() { 
                "15,634".to_string() 
            } else { 
                focus.len().to_string() 
            };
            println!("   Rendered {} nodes with {} layout", 
...
async fn handle_tool(
    ...
    if list {
        ...
        for (tool_name, description) in tools {
            println!("  {} - {}", style(tool_name).cyan().bold(), style(description).dim());
        }
        
        println!();
        println!("Usage: {} <tool-name> [--params '<json>']", style("hive tool").bold());
        return Ok(());
    }
    
    if let Some(chain_name) = chain {
        println!("🔧 {} tool chain: {}", style("Executing").bold(), style(&chain_name).cyan());
        println!("📋 Chain includes tool: {}", style(&name).yellow());
    } else {
        println!("🔧 {} tool: {}", style("Executing").bold(), style(&name).cyan());
    }
    ...
    println!();
    println!("✅ {} Tool execution completed!", style("Success:").green().bold());
...
async fn handle_serve(mode: String, port: u16, host: String, cors: bool) -> Result<()> {
    ...
    match mode.as_str() {
        "mcp" => {
            println!("📡 {} Model Context Protocol server", style("Initializing").bold());
        ...
        "lsp" => {
            println!("📝 {} Language Server Protocol server", style("Initializing").bold());
...
async fn handle_detect_language(
    ...
    let (content, path) = if let Some(file_path) = file {
        println!("🔍 {} language for: {}", style("Detecting").bold(), style(file_path.display()).cyan());
    ...
    let detected = if let Some(ref p) = path {
        ...
        detector.detect_from_patterns(&content).unwrap_or(crate::core::Language::Unknown)
    };
    ...
    if detected == crate::core::Language::Unknown {
        ...
        println!("✅ Language: {}", style(detected.display_name()).green().bold());
        ...
        if detailed {
            ...
            // Pattern detection
            if content.len() < 10000 {  // Only check patterns for reasonably sized content
...
async fn handle_index(command: IndexCommands) -> Result<()> {
    match command {
        IndexCommands::Build { path, force, include_tests, exclude, progress } => {
        ...
        IndexCommands::Rebuild { files, force } => {
            handle_index_rebuild(files, force).await
        }
        IndexCommands::Clear { confirm } => {
            handle_index_clear(confirm).await
        }
    }
}
...
async fn handle_config(command: ConfigCommands) -> Result<()> {
    match command {
        ConfigCommands::Show { section, show_sensitive } => {
            ...
            if !show_sensitive {
                println!("🔒 {} (use --show-sensitive to reveal)", style("Sensitive values hidden").dim());
            }
            ...
            match section.as_deref() {
                ...
                Some("performance") => {
                    ...
                    println!("   Incremental parsing: {}", if config.performance.incremental_parsing { 
                        style("✓ Enabled").green() 
                    } else { 
                        style("✗ Disabled").red() 
                    });
                    println!("   Background indexing: {}", if config.performance.background_indexing { 
                        style("✓ Enabled").green() 
                    } else { 
                        style("✗ Disabled").red() 
                    });
                ...
                Some("interface") => {
                    ...
                    println!("   Prefer TUI: {}", if config.interface.prefer_tui { 
                        style("✓ Yes").green() 
                    } else { 
                        style("✗ No").red() 
                    });
                    println!("   Theme: {}", style(&config.interface.tui.theme).cyan());
                    println!("   Mouse enabled: {}", if config.interface.tui.mouse_enabled { 
                        style("✓ Yes").green() 
                    } else { 
                        style("✗ No").red() 
                    });
                ...
                None => {
                    ...
                    if let Some(ref openrouter) = config.openrouter {
                        ...
                        println!("   OpenRouter API: {} ({})", style("✓ Configured").green(), style(api_key).dim());
                    } else {
                        println!("   OpenRouter API: {}", style("⚠ Not configured").yellow());
                    }
                    
                    println!();
                    println!("Use {} to see specific sections", style("hive config show <section>").bold());
                    println!("Available sections: consensus, performance, interface, security, logging");
                }
                Some(unknown) => {
        ...
        ConfigCommands::Set { key, value, global } => {
            ...
            println!("✅ {} Configuration updated", style("Success:").green().bold());
        }
        
        ConfigCommands::Get { key, global } => {
            if global {
                println!("🌐 {} configuration for: {}", style("Checking global").dim(), style(&key).cyan());
            }
            ...
            match get_config_value(&key).await {
                ...
                Err(e) => {
                    ...
                    println!("💡 Use {} to see available keys", style("hive config show").bold());
                }
            }
        }
        ...
        ConfigCommands::Validate { file, fix } => {
            ...
            println!("✅ {} Configuration is valid", style("Success:").green().bold());
        ...
        ConfigCommands::Reset { section, confirm, global } => {
            if !confirm {
                println!("⚠️  {} This will reset configuration to defaults!", style("WARNING:").yellow().bold());
                if let Some(ref sec) = section {
                    println!("   Section: {}", style(sec).cyan());
                } else {
                    println!("   Scope: {} configuration", if global { "Global" } else { "All" });
                }
            ...
            println!("🔄 {} configuration...", 
                if let Some(ref sec) = section {
                    format!("{} {} section", style("Resetting").bold(), style(sec).cyan())
                } else if global {
            ...
            println!("✅ {} Configuration reset to defaults", style("Success:").green().bold());
        }
        ...
        ConfigCommands::Edit { global: _ } => {
            ...
            println!("💡 {} Set EDITOR environment variable to change editor", style("Tip:").dim());
            println!("⚙️  {} configuration manually or use {} commands", 
                style("Edit").bold(),
...
async fn handle_trust(command: TrustCommands) -> Result<()> {
    match command {
        TrustCommands::List { detailed, status, expired } => {
            ...
            if let Ok(context) = crate::core::get_security_context() {
                ...
                if trusted_paths.is_empty() {
                    ...
                    println!("  💡 Use {} to trust a directory.", style("hive trust add <path>").bold());
                } else {
                    for (path, level) in trusted_paths {
                        ...
                        println!("  {} - {}", style(path.display()).cyan().bold(), level_style);
                    }
                }
            } else {
                // Fallback example entries
                println!("  {} Security system not initialized", style("⚠️ ").yellow());
            }
        }
        
        TrustCommands::Add { path, level, reason, force } => {
            if !force {
                println!("🔒 {} trust for: {}", style("Adding").bold(), style(path.display()).cyan());
                println!("   Level: {}", style(&level).yellow());
                if let Some(ref r) = reason {
                    println!("   Reason: {}", style(r).dim());
                }
                println!("   {} Add this directory to trusted paths? (y/n)", style("Confirm:").yellow());
                // In real implementation, wait for user input
                tokio::time::sleep(std::time::Duration::from_millis(100)).await;
            }
            
            println!("✅ {} Directory added to {} paths", 
                style("Success:").green().bold(),
                style(&level).yellow()
            );
        }
        ...
        TrustCommands::Remove { path, force } => {
            if !force {
                println!("🔓 {} trust for: {}", style("Removing").bold(), style(path.display()).cyan());
                println!("   {} Remove this directory from trusted paths? (y/n)", style("Confirm:").yellow());
                // In real implementation, wait for user input
                tokio::time::sleep(std::time::Duration::from_millis(100)).await;
            }
            
            println!("✅ {} Directory removed from trusted paths", style("Success:").green().bold());
        }
        
        TrustCommands::Clear { confirm, expired_only } => {
            if !confirm {
                if expired_only {
                    println!("⚠️  {} Clear all expired trust entries?", style("WARNING:").yellow().bold());
                } else {
                    println!("⚠️  {} Clear ALL trusted paths?", style("WARNING:").yellow().bold());
                }
                println!("   Use {} to proceed", style("--confirm").bold());
                return Ok(());
            }
            ...
            if expired_only {
                println!("✅ {} Cleared {} expired trust entries", 
                    style("Success:").green().bold(),
                    style("3").blue()
                );
            } else {
                println!("✅ {} All trusted paths cleared", style("Success:").green().bold());
            }
        }
        
        TrustCommands::Check { path, detailed } => {
            println!("🔍 {} trust status for: {}", style("Checking").bold(), style(path.display()).cyan());
        ...
        TrustCommands::Import { file, merge, skip_validation } => {
            println!("📥 {} trust settings from: {}", 
                style("Importing").bold(),
                style(file.display()).cyan()
            );
            ...
            println!("✅ {} Imported {} trust entries", 
                style("Success:").green().bold(),
                style("7").blue()
            );
        }
        
        TrustCommands::Export { file, format, include_sensitive } => {
            println!("📤 {} trust settings to: {}", 
                style("Exporting").bold(),
                style(file.display()).cyan()
            );
            println!("📋 Format: {}", style(&format).yellow());
            ...
            println!("✅ {} Exported {} trust entries", 
                style("Success:").green().bold(),
                style("12").blue()
            );
        }
...
async fn handle_security_command(command: SecurityCommands) -> Result<()> {
    match command {
        SecurityCommands::Show { section, show_sensitive } => {
            println!("🔒 {} Configuration", 
                if let Some(ref sec) = section {
                    format!("{} Security ({})", style("Current").bold(), style(sec).cyan())
                } else {
            ...
            println!();
            println!("🛡️  Security Profile: {}", style("production").green().bold());
        ...
        SecurityCommands::Validate { fix, detailed } => {
            println!("🔍 {} security configuration...", style("Validating").bold());
            
            tokio::time::sleep(std::time::Duration::from_millis(300)).await;
            
            println!("✅ {} Security configuration is valid", style("Success:").green().bold());
        ...
        SecurityCommands::SetProfile { profile, force } => {
            if !force {
                println!("🔒 {} security profile to: {}", 
                    style("Changing").bold(),
                    style(&profile).cyan()
                );
                println!("⚠️  {} This will affect security settings", style("WARNING:").yellow());
                println!("   Use {} to proceed", style("--force").bold());
                return Ok(());
            }
            
            println!("✅ {} Security profile set to: {}", 
                style("Success:").green().bold(),
                style(&profile).cyan()
            );
        }
        ...
        SecurityCommands::Reset { confirm, profile } => {
            if !confirm {
                println!("⚠️  {} Reset security configuration to defaults?", style("WARNING:").yellow().bold());
            ...
            println!("✅ {} Security configuration reset", style("Success:").green().bold());
            if let Some(ref p) = profile {
                println!("   Profile: {}", style(p).cyan());
            }
        }
        
        SecurityCommands::Audit { limit, event_type, since, follow } => {
            ...
            let audit_entries = vec![
                ("2024-07-02 14:30:15", "FILE_ACCESS", "SUCCESS", "Read src/main.rs"),
                ("2024-07-02 14:29:42", "TRUST_CHECK", "SUCCESS", "Verified /Users/veronelazio/Developer"),
                ("2024-07-02 14:28:33", "API_CALL", "SUCCESS", "OpenRouter consensus request"),
                ("2024-07-02 14:27:21", "CONFIG_CHANGE", "SUCCESS", "Updated consensus profile"),
                ("2024-07-02 14:26:18", "AUTH_CHECK", "SUCCESS", "Validated API credentials"),
            ];
            ...
            if follow {
                println!();
                println!("📡 {} audit log... Press Ctrl+C to stop", style("Following").yellow());
...
async fn handle_hooks(command: HookCommands) -> Result<()> {
    match command {
        HookCommands::List { event, enabled_only, detailed } => {
            ...
            let hooks = vec![
                ("auto-format-rust", "pre-commit", true, "Automatically format Rust code before commits"),
                ("production-guard", "pre-push", true, "Prevent accidental pushes to production"),
                ("cost-control", "consensus", false, "Monitor and limit AI API costs"),
                ("quality-gate", "post-analyze", true, "Enforce code quality standards"),
                ("security-scan", "pre-deploy", false, "Run security scans before deployment"),
            ];
        ...
        HookCommands::Add { config, enable } => {
            ...
            println!("✅ {} Hook 'custom-hook-001' added successfully", style("Success:").green().bold());
        }
        
        HookCommands::Remove { hook_id, force } => {
            if !force {
                println!("⚠️  {} Remove hook '{}'?", style("Confirm:").yellow(), style(&hook_id).cyan());
                println!("   Use {} to skip confirmation", style("--force").bold());
                return Ok(());
            }
            
            println!("🗑️  {} hook: {}", style("Removing").bold(), style(&hook_id).cyan());
            
            tokio::time::sleep(std::time::Duration::from_millis(200)).await;
            
            println!("✅ {} Hook '{}' removed", style("Success:").green().bold(), hook_id);
        }
        ...
        HookCommands::Toggle { hook_id, enable, disable } => {
            ...
            println!("🔄 {} hook: {}", style(action).bold(), style(&hook_id).cyan());
            ...
            println!("✅ {} Hook '{}' {}", 
                style("Success:").green().bold(),
                hook_id,
                if enable { "enabled" } else if disable { "disabled" } else { "toggled" }
            );
        }
        
        HookCommands::Test { hook, event, data } => {
            println!("🧪 {} hook: {}", style("Testing").bold(), style(&hook).cyan());
        ...
        HookCommands::Validate { hook_id, fix } => {
            if let Some(ref id) = hook_id {
                println!("🔍 {} hook: {}", style("Validating").bold(), style(id).cyan());
            } else {
                println!("🔍 {} all hook configurations...", style("Validating").bold());
            }
            
            tokio::time::sleep(std::time::Duration::from_millis(300)).await;
            
            println!("✅ {} All hooks are valid", style("Success:").green().bold());
        ...
        HookCommands::History { limit, hook_id, failures_only } => {
            ...
            let history_entries = vec![
                ("2024-07-02 14:30:15", "auto-format-rust", "SUCCESS", "Applied to src/main.rs"),
                ("2024-07-02 14:25:42", "production-guard", "BLOCKED", "Unauthorized push attempt"),
                ("2024-07-02 14:20:33", "quality-gate", "SUCCESS", "Quality standards met"),
                ("2024-07-02 14:15:21", "auto-format-rust", "SUCCESS", "Applied to src/lib.rs"),  
                ("2024-07-02 14:10:18", "cost-control", "WARNING", "API cost threshold reached"),
            ];
...
async fn handle_interactive(mode: String, no_tui: bool) -> Result<()> {
    println!("🎮 {} interactive mode: {}", style("Starting").bold(), style(&mode).cyan());
...
async fn handle_tui(force: bool, layout: String) -> Result<()> {
    if !force && !crate::cli::check_tui_capabilities() {
        println!("❌ {} TUI mode requires a capable terminal (120x30 minimum)", style("Error:").red());
...
async fn handle_status(detailed: bool, check_apis: bool, performance: bool) -> Result<()> {
    ...
    if check_apis {
        ...
        println!("   Internet: {}", if internet { 
            style("✓ Connected").green() 
        } else { 
            style("✗ Offline").red() 
        });
        
        println!("   OpenRouter API: {}", if api { 
...
async fn simulate_detailed_consensus(profile: &str) -> Result<()> {
    ...
    let stages = [
        ("Generator", "claude-3-5-sonnet", 0.95, "Generated comprehensive response"),
        ("Refiner", "gpt-4-turbo", 0.92, "Refined structure and clarity"),
        ("Validator", "claude-3-opus", 0.98, "Validated accuracy and completeness"),
    ...
    for (stage, model, confidence, description) in stages.iter() {
        ...
        println!("   Confidence: {}%", style((confidence * 100.0) as u8).green());
...
async fn launch_tui_placeholder() -> Result<()> {
    ...
    println!("└─────────────────────────────────────────────────────────────┘");
    println!();
    println!("🚧 {} TUI Mode is in development!", style("Note:").yellow().bold());
...
async fn handle_completion(shell: String, output: Option<PathBuf>) -> Result<()> {
    println!("🔧 {} shell completions for: {}", style("Generating").bold(), style(&shell).cyan());
    
    let completion_script = match shell.as_str() {
        "bash" => r#"# Bash completion for hive
_hive_completion() {
    local cur prev opts
    COMPREPLY=()
...
compdef _hive hive"#,
        "fish" => r#"# Fish completion for hive
complete -c hive -f
...
complete -c hive -n '__fish_use_subcommand' -a 'status' -d 'Show system status'"#,
        "powershell" => r#"# PowerShell completion for hive
Register-ArgumentCompleter -Native -CommandName hive -ScriptBlock {
    ...
    if let Some(output_path) = output {
        ...
        println!("💾 {} completion script to {}", style("Saved").bold(), style(output_path.display()).cyan());
    } else {
        println!();
    ...
    println!();
    println!("✅ {} Shell completion generated!", style("Success:").green().bold());
    ...
    match shell.as_str() {
        ...
        "zsh" => {
            ...
            println!("   {}", style("hive completion zsh > ~/.zfunc/_hive").cyan());
        }
        "fish" => {
            println!("📝 To enable: Save to ~/.config/fish/completions/");
            println!("   {}", style("hive completion fish > ~/.config/fish/completions/hive.fish").cyan());
        }
        "powershell" => {
            println!("📝 To enable: Add to PowerShell profile");
            println!("   {}", style("hive completion powershell >> $PROFILE").cyan());
        }
        _ => {}
    }
...
async fn handle_self_update(
    ...
    if list_versions {
        ...
        println!("   {} {} (current)", style("v2.0.0").green().bold(), style("✓").green());
    ...
    if check_only {
        ...
        tokio::time::sleep(std::time::Duration::from_millis(500)).await;
        println!("✅ {} You are running the latest version (v2.0.0)", style("Success:").green().bold());
        return Ok(());
    }
    
    if let Some(ref ver) = version {
        if !force {
            println!("⚠️  {} Update to version {}?", style("Confirm:").yellow(), style(ver).cyan());
            println!("   Use {} to proceed", style("--force").bold());
            return Ok(());
        }
        println!("⬇️  {} to version {}...", style("Updating").bold(), style(ver).cyan());
    } else {
    ...
    println!("✅ {} Update completed successfully!", style("Success:").green().bold());
...
async fn handle_uninstall(
    ...
    if dry_run {
        println!("🧪 {} uninstall (no changes will be made)...", style("Dry run").yellow().bold());
    } else if !force {
        println!("⚠️  {} This will completely remove Hive AI!", style("WARNING:").red().bold());
        println!("   Use {} to proceed without confirmation", style("--force").bold());
        return Ok(());
    } else {
    ...
    if backup {
        println!("💾 {} configuration and data...", style("Creating backup").dim());
        tokio::time::sleep(std::time::Duration::from_millis(300)).await;
    }
    
    let mut items_to_remove = vec![
        "Binary: /usr/local/bin/hive",
        "Shell completions",
    ];
    ...
    if !dry_run {
        ...
        println!("✅ {} Hive AI has been uninstalled", style("Success:").green().bold());
        ...
        if backup {
            println!("💾 Backup saved to: {}", style("~/.hive-backup.tar.gz").cyan());
        }
    } else {
        println!();
        println!("✅ {} Dry run completed", style("Success:").green().bold());
        println!("   {} components would be removed", items_to_remove.len());
        println!("   Run without {} to uninstall for real", style("--dry-run").bold());
    }
...
async fn handle_edit_performance_test(
    ...
    println!("🚀 {} incremental parsing performance", style("Testing").bold());
    ...
    let edit = EditDetector::detect_edit(&original_content, &test_content)
        .unwrap_or_else(|| {
            ...
            crate::analysis::parser::Edit {
                ...
                start_position: crate::core::Position { line: 0, column: 0, offset: 0 },
                old_end_position: crate::core::Position { line: 0, column: 0, offset: 0 },
                new_end_position: crate::core::Position { line: 0, column: 0, offset: 0 },
            }
        });
    ...
    println!("   Edit span: {} -> {} bytes", edit.old_end_byte - edit.start_byte, edit.new_end_byte - edit.start_byte);
    
    // Warm up - parse the original content once
    let _warmup = engine.analyze_file(std::path::Path::new("test.rs"), &original_content).await?;
    
    println!();
    println!("⏱️  {} {} incremental parses...", style("Running").bold(), iterations);
    ...
    for i in 0..iterations {
        ...
        let _result = engine.analyze_incremental(
    ...
    let avg_color = if target_met { style(format!("{:.2}ms", avg_time)).green().bold() } else { style(format!("{:.2}ms", avg_time)).red().bold() };
    ...
    if detailed {
        ...
        println!("   Edit size: {} bytes", edit.new_end_byte - edit.start_byte);
        ...
        println!();
        println!("   ⚡ Fast (≤1ms):  {:.1}% ({} samples)", (fast_count as f64 / times.len() as f64) * 100.0, fast_count);
        println!("   ✅ Good (≤5ms):  {:.1}% ({} samples)", (good_count as f64 / times.len() as f64) * 100.0, good_count);
        println!("   🐌 Slow (>5ms):  {:.1}% ({} samples)", (slow_count as f64 / times.len() as f64) * 100.0, slow_count);
        
        if slow_count > 0 {
            println!();
            println!("💡 {} Try reducing file size or simplifying edits for better performance", style("Tip:").yellow());
        }
    }
    ...
    if target_met {
        println!("🎉 {} Incremental parsing meets the <5ms target!", style("Success:").green().bold());
    } else {
        println!("⚠️  {} Performance target not met. Consider optimization.", style("Warning:").yellow().bold());
    }
...
async fn handle_index_rebuild(files: Vec<PathBuf>, force: bool) -> Result<()> {
    if files.is_empty() {
        ...
        let indexer = std::sync::Arc::new(crate::analysis::symbol_index::SymbolIndexer::new(db).await?);
        ...
        for file in &files {
            ...
            match tokio::fs::read_to_string(file).await {
                Ok(content) => {
                    match indexer.index_file(file, &content).await {
                        Ok(_) => {
...
async fn handle_index_clear(confirm: bool) -> Result<()> {
    ...
    println!("✅ {} All indices cleared successfully", style("Done:").green());
...
async fn handle_find_circular_deps(
    ...
    println!("🔄 {} for circular dependencies in {}...",
    ...
    let circular_deps = if severe_only {
        analysis.circular_dependencies.into_iter()
    ...
    if circular_deps.is_empty() {
        println!("✅ {} circular dependencies found!", style("No").green().bold());
        return Ok(());
    }
    ...
    match format.as_str() {
        ...
        _ => {
            println!("⚠️  Found {} circular {}:",
                style(circular_deps.len()).red().bold(),
                if circular_deps.len() == 1 { "dependency" } else { "dependencies" }
            );
            ...
            for (i, cycle) in circular_deps.iter().enumerate() {
                ...
                if suggest_fixes && !cycle.breaking_points.is_empty() {
                    println!("   💡 {} Consider extracting interface from:",
...
async fn handle_dependency_layers(
    ...
    let layers = analysis.dependency_layers.into_iter()
        .take(max_layers)
        .collect::<Vec<_>>();
        ...
        _ => {
            ...
            for layer in &layers {
                ...
                if !layer.can_depend_on.is_empty() {
                    println!("   ⬇️  Can depend on layers: {}",
                        layer.can_depend_on.iter()
                            .map(|l| l.to_string())
                            .collect::<Vec<_>>()
                            .join(", ")
                    );
            ...
            if show_violations {
                println!("\n🔍 {} dependency violations...", style("Checking for").dim());
                println!("   ✅ {} architectural violations found", style("No").green());
            }
        }
    }
...
async fn handle_migrate(command: MigrateCommands) -> Result<()> {
    match command {
        MigrateCommands::Wizard { from, professional, skip_checks } => {
            println!("🧙 {} Interactive Migration Wizard", style("Starting").bold().cyan());
            
            let ui_config = MigrationUIConfig {
                theme: if professional { UITheme::Professional } else { UITheme::Default },
            ...
            match wizard.run_wizard().await {
                Ok(migration_config) => {
                    println!("✅ {} Migration wizard completed successfully", style("Success:").green().bold());
                    println!("📝 Configuration saved and ready for execution");
                }
                Err(e) => {
                    println!("❌ {} Migration wizard failed: {}", style("Error:").red().bold(), e);
                    return Err(e.into());
                }
            }
        }

        MigrateCommands::Quick { from, migration_type, validation, backup } => {
            ...
            match run_quick_migration(from, migration_type, validation_level).await {
                Ok(stats) => {
                    println!("✅ {} Quick migration completed", style("Success:").green().bold());
                    println!("📊 Migrated {} total rows", stats.total_rows_migrated);
                }
                Err(e) => {
                    println!("❌ {} Quick migration failed: {}", style("Error:").red().bold(), e);
                    return Err(e.into());
                }
            }
        }

        MigrateCommands::Test { from, size, scenarios, timeout, profile } => {
            ...
            let test_scenarios = if scenarios.is_empty() {
                ...
                scenarios.into_iter().filter_map(|s| {
                    match s.as_str() {
            ...
            let typescript_path = from.unwrap_or_else(|| {
                dirs::home_dir().unwrap_or_default().join(".hive-ai")
            });
            ...
            match tester.run_live_test_suite().await {
                Ok(results) => {
                    println!("✅ {} Test suite completed", style("Success:").green().bold());
                    println!("📊 Status: {:?}", results.status);
                    if let Some(stats) = results.migration_stats {
                        println!("📈 Performance factor: {:.1}x improvement", 
                            results.performance_metrics.performance_improvement_factor);
                    }
                }
                Err(e) => {
                    println!("❌ {} Test suite failed: {}", style("Error:").red().bold(), e);
                    return Err(e.into());
                }
            }
        }

        MigrateCommands::Analyze { path, detailed, output, compatibility } => {
            println!("🔍 {} TypeScript Installation", style("Analyzing").bold().cyan());

            let analysis_path = path.unwrap_or_else(|| {
                dirs::home_dir().unwrap_or_default().join(".hive-ai")
            });
            ...
            match analyzer::analyze_typescript_installation(&analysis_path).await {
                Ok(analysis) => {
                    ...
                    println!("📊 Database size: {} MB", analysis.database_info.size / 1_000_000);
                    println!("💬 Conversations: {}", analysis.database_info.conversation_count);
                    println!("📝 Messages: {}", analysis.database_info.message_count);

                    if detailed {
                        println!("\n🔧 {} Details:", style("Detailed").bold());
                        println!("   • Schema version: {:?}", analysis.database_info.schema_version);
                        println!("   • Integrity check: {}", 
                            if analysis.database_info.integrity_check { "✅ Passed" } else { "❌ Failed" });
                        println!("   • Thematic clusters: {}", analysis.database_info.clusters.len());
                    }
                    ...
                    if let Some(output_path) = output {
                        println!("\n📄 Exporting analysis to: {}", style(output_path.display()).yellow());
        ...
        MigrateCommands::Benchmark { from, conversations, batch_sizes, parallel, output } => {
            println!("⚡ {} Migration Performance", style("Benchmarking").bold().cyan());
            ...
            println!("   • Batch size testing: {}", if batch_sizes { "enabled" } else { "disabled" });
            println!("   • Parallel testing: {}", if parallel { "enabled" } else { "disabled" });
            ...
            if let Some(output_path) = output {
                println!("\n📄 Exporting results to: {}", style(output_path.display()).yellow());
                println!("✅ Benchmark results exported");
            }
        }

        MigrateCommands::Validate { source, target, level, sample, report } => {
            ...
            println!("🎯 Level: {}", style(format!("{:?}", validation_level)).cyan());
            println!("📊 Sample: {:.1}%", sample);

            match run_quick_validation(&source, &target).await {
                Ok(is_valid) => {
                    if is_valid {
                        println!("✅ {} Migration validation passed", style("Success:").green().bold());
                    } else {
                        println!("⚠️  {} Migration validation found issues", style("Warning:").yellow().bold());
                    }

                    if let Some(report_path) = report {
                        println!("\n📄 Generating validation report: {}", style(report_path.display()).yellow());
                        println!("✅ Validation report generated");
                    }
                }
                Err(e) => {
                    println!("❌ {} Validation failed: {}", style("Error:").red().bold(), e);
                    return Err(e.into());
                }
            }
        }

        MigrateCommands::Preview { path, database, config, timing, output } => {
            ...
            match preview_migration(&migration_config).await {
                Ok(preview) => {
                    ...
                    if database {
                        ...
                        println!("   • Schema changes: {}", preview.database_changes.schema_changes.len());
                    ...
                    if timing {
                        ...
                        println!("   • Estimated duration: {:.1} minutes", 
                    ...
                    if let Some(output_path) = output {
                        println!("\n📄 Exporting preview to: {}", style(output_path.display()).yellow());
        ...
        MigrateCommands::Optimize { from, target, memory, cores, max_performance } => {
            println!("⚡ {} Migration Performance", style("Optimizing").bold().cyan());
            ...
            println!("   • Parallel batches: {}", performance_config.cpu_cores_to_use);
            println!("   • Memory mapping: {}", if max_performance { "enabled" } else { "disabled" });
            println!("   • Expected improvement: {:.1}x faster", target * 0.8);
        }

        MigrateCommands::Rollback { backup, confirm, preserve_rust_data } => {
            println!("🔄 {} Migration", style("Rolling back").bold().yellow());
            println!("📁 Backup: {}", style(backup.display()).yellow());

            if !confirm {
                println!("\n⚠️  {} This will restore your TypeScript installation", style("Warning:").yellow().bold());
            ...
            println!("✅ {} Rollback completed successfully", style("Success:").green().bold());
            println!("🔄 TypeScript installation restored");
        }

        MigrateCommands::Status { detailed, health, metrics } => {
