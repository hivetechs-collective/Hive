...
use crate::core::memory::{MemorySystem, SemanticSearchResult};
...
impl ContextRetriever {
    ...
    pub async fn retrieve(
        ...
        let results = match strategy {
            RetrievalStrategy::Semantic => self.semantic_retrieval(query_embedding, limit).await?,
            ...
            RetrievalStrategy::Hybrid => self.hybrid_retrieval(query_embedding, "", limit).await?,
            ...
            RetrievalStrategy::Adaptive => self.adaptive_retrieval(query_embedding, limit).await?,
        };
    ...
    pub async fn build_context_window(
        ...
        let primary_results = self
            .retrieve(&query_embedding, strategy, self.config.max_results)
            .await?;
        ...
        let summary = self
            .generate_summary(&primary_results, &supporting_results)
            .await?;
    ...
    async fn generate_suggestions(&self, results: &[SemanticSearchResult]) -> Result<Vec<String>> {
