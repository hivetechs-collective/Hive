        ...
        TopicHierarchy {
            ...
            subtopics: {
                ...
                subtopics.insert(
                    "defi".to_string(),
                    vec![
                        "decentralized_finance".to_string(),
                        "lending".to_string(),
                        "trading".to_string(),
                        "yield".to_string(),
                        "staking".to_string(),
    ...
    hierarchy.insert(
        ...
        TopicHierarchy {
            ...
            subtopics: {
                ...
                subtopics.insert(
                    "application_security".to_string(),
                    vec![
...
pub async fn extract_topics_with_ai(
    text: &str,
    provider_name: Option<&str>,
) -> Result<Vec<String>> {
    debug!(
        "Starting AI-powered topic extraction for text: {} chars",
        text.len()
    );
    ...
    let extract_with_ai = async move |text: &str| -> Result<Vec<String>> {
        ...
        match serde_json::from_str::<Vec<String>>(content) {
            Ok(topics) => {
                debug!(
                    "Successfully parsed {} topics from AI response",
                    topics.len()
                );
                Ok(topics)
            }
            Err(_) => {
                ...
                if let Some(captures) = regex::Regex::new(r"\[(.*?)\]").unwrap().captures(content) {
    ...
    if let Some(_provider) = provider_name {
        ...
        match extract_with_ai(text).await {
            ...
            Err(e) => {
                debug!(
                    "AI extraction failed: {}, falling back to keyword extraction",
                    e
                );
                Ok(extract_with_fallback())
            }
...
pub async fn tag_conversation(conversation_id: &str) -> Result<Vec<String>> {
    ...
    let mut stmt = conn
        .prepare("SELECT content FROM messages WHERE conversation_id = ? ORDER BY timestamp ASC")?;

    let message_rows = stmt.query_map([conversation_id], |row| Ok(row.get::<_, String>(0)?))?;
    ...
    if !topics.is_empty() {
        ...
        info!(
            "Tagged conversation {} with {} topics",
            conversation_id,
            topics.len()
        );
    } else {
...
pub async fn find_conversations_by_topic(topic: &str, limit: Option<usize>) -> Result<Vec<String>> {
    ...
    if conversations.is_empty() {
        ...
        if !related_topics.is_empty() {
            ...
            let mut params: Vec<&dyn rusqlite::ToSql> = related_topics
                .iter()
            ...
            let conversation_rows =
                stmt.query_map(params.as_slice(), |row| Ok(row.get::<_, String>(0)?))?;
...
mod tests {
    ...
    fn test_topic_hierarchy_structure() {
        ...
        for (category, details) in &hierarchy {
            assert!(
                !details.related.is_empty(),
                "Category {} missing related terms",
                category
            );
            assert!(
                !details.subtopics.is_empty(),
                "Category {} missing subtopics",
                category
            );
        }
    }
    ...
    async fn test_find_topics_for_query() -> Result<()> {
        ...
        assert!(topics
            .iter()
            .any(|t| t.contains("programming") || t.contains("rust")));
