...
use crate::memory::topic_extraction::{find_conversations_by_topic, find_topics_for_query};
...
impl ThematicCluster {
    ...
    pub async fn find_relevant_knowledge_for_ai(&self, query: &str, stage: &str) -> Result<String> {
        info!(
            "Finding relevant knowledge for {} stage with temporal priority",
            stage
        );
        ...
        if is_follow_up {
            ...
            if !broader_recent.is_empty() {
                info!(
                    "Follow-up detected - found broader recent context for {} stage",
                    stage
                );
                return Ok(self.format_stage_context(
                    &broader_recent,
                    stage,
        ...
        info!(
            "No recent context found, falling back to thematic search for {} stage",
            stage
        );
    ...
    pub async fn detect_follow_up(
        ...
        if pattern_confidence > 0.3 {
            ...
            return self
                .ai_analyze_follow_up(query, recent_conversations, pattern_confidence)
                .await;
        }
        ...
        if self.has_implicit_references(query) && !recent_conversations.is_empty() {
            ...
            return self
                .ai_analyze_follow_up(query, recent_conversations, 0.5)
                .await;
        }
    ...
    async fn ai_analyze_follow_up(
        ...
        let conversation_context = self
            .get_conversation_context(recent_conversation_id)
            .await?;
        ...
        if let Ok(ai_score) = content.trim().parse::<f64>() {
            ...
            debug!(
                "Follow-up analysis: pattern={:.2}, ai={:.2}, combined={:.2}",
    ...
    pub async fn store_conversation_thread(&self, thread: &ConversationThread) -> Result<()> {
        ...
        info!(
            ...
            thread.parent_conversation_id, thread.child_conversation_id, thread.thread_type
        );
    ...
    async fn get_recent_curator_knowledge(&self, _query: &str) -> Result<String> {
        ...
        info!(
            "Found {} recent curator answers (24h) - using as authoritative context",
            recent_answers.len()
        );
    ...
    async fn get_recent_conversations(&self, _query: &str, hours: i64) -> Result<String> {
        ...
        info!(
            "Found {} recent conversations ({} hours)",
            conversations.len(),
            hours
        );
        Ok(formatted)
    ...
    pub fn calculate_message_importance(
        ...
        let length_score = if content.len() > 200 {
            2.0
        ...
        let recency_score = if hours_ago < 1.0 {
            3.0
        ...
        let role_score = if role.to_lowercase() == "user" {
            1.0
        } else {
            0.0
        };
    ...
    pub async fn get_conversation_clusters(
        ...
        debug!(
            "Found {} topic clusters with {} conversations",
            clusters.len(),
            limit
        );
        Ok(clusters)
...
mod tests {
    ...
    async fn test_thread_type_determination() {
        ...
        assert_eq!(
            cluster.determine_thread_type("give me examples"),
            ThreadType::ListReference
        );
        assert_eq!(
            cluster.determine_thread_type("can you explain that?"),
            ThreadType::Clarification
        );
        assert_eq!(
            cluster.determine_thread_type("tell me more"),
            ThreadType::Continuation
        );
        assert_eq!(
            cluster.determine_thread_type("what about this?"),
            ThreadType::FollowUp
        );
    }
