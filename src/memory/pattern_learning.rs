...
impl PatternLearner {
    ...
    pub async fn learn_from_memories(&mut self, _memory_system: &MemorySystem) -> Result<()> {
        ...
        for conv in &conversations {
            self.process_conversation(&conv.question, &conv.final_answer, conv.conversation_context.as_deref())?;
        }
    ...
    pub fn find_similar(&self, input: &str, limit: usize) -> Vec<&Pattern> {
        let mut scored_patterns: Vec<(&Pattern, f32)> = self.patterns
            .iter()
            .map(|pattern| {
    ...
    fn detect_pattern_type(&self, question: &str, answer: &str) -> PatternType {
        ...
        } else if q_lower.contains("transform") || q_lower.contains("convert") || q_lower.contains("refactor") {
            PatternType::CodeTransformation
        } else if q_lower.contains("explain") || q_lower.contains("what is") || q_lower.contains("describe") {
    ...
    fn extract_template(&self, question: &str, answer: &str, pattern_type: &PatternType) -> Result<String> {
        ...
        match pattern_type {
            ...
            PatternType::ErrorResolution => {
                Ok("[ERROR] -> [RESOLUTION]".to_string())
            }
            PatternType::CommandSequence => {
                Ok("[TASK] -> [COMMANDS]".to_string())
            }
    ...
    fn calculate_confidence(&self, pattern: &Pattern, stats: &PatternStatistics) -> f32 {
        ...
        // Quality component
        let avg_quality = pattern.examples.iter()
            .map(|e| e.quality_score)
            .sum::<f32>() / pattern.examples.len() as f32;
    ...
    fn extract_tags(&self, question: &str, answer: &str) -> HashSet<String> {
        ...
        let tech_tags = ["rust", "python", "javascript", "typescript", "api", "database", "cli", "web"];
        ...
        // Common concept tags
        let concept_tags = ["async", "performance", "security", "testing", "debugging", "optimization"];
    ...
    fn cluster_patterns(&mut self) -> Result<()> {
        debug!("Clustering patterns with threshold {}", self.config.clustering_threshold);
    ...
    fn prune_patterns(&mut self) {
        ...
        self.patterns.retain(|p| p.confidence >= self.config.min_confidence);
    ...
    fn calculate_growth_rate(&self) -> f32 {
        ...
        let recent_patterns = self.patterns.iter()
            .filter(|p| {
        ...
        let older_patterns = self.patterns.iter()
            .filter(|p| {
                let days_old = (Utc::now() - p.discovered_at).num_days();
...
mod tests {
    ...
    fn test_pattern_detection() {
        ...
        let test_cases = vec![
            ...
            ("What is the best practice for error handling?", PatternType::BestPractice),
            ("Debug the async function", PatternType::Debugging),
            ("Transform this code to use async/await", PatternType::CodeTransformation),
        ];
