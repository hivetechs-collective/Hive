...
impl SubscriptionManager {
    ...
    async fn setup_file_watcher(&mut self) -> Result<()> {
        ...
        tokio::spawn(async move {
            while let Some(event) = rx.recv().await {
                if let Err(e) =
                    Self::process_file_system_event(event, &event_sender, &subscriptions_clone)
                        .await
                {
            ...
            for subscription in subscriptions_read.values() {
                if Self::matches_subscription(&event, path, subscription) {
                    let subscription_event =
                        Self::create_subscription_event(&event, path, subscription).await?;
    ...
    /// Cancel subscription
    pub async fn cancel_subscription(&self, subscription_id: &str, client_id: &str) -> Result<()> {
        ...
        if let Some(subscription) = subscription {
            ...
            info!(
                "Cancelled subscription {} for client {}",
                subscription_id, client_id
            );
        } else {
            warn!(
                "Attempted to cancel non-existent subscription: {}",
                subscription_id
            );
        }
    ...
    async fn start_health_check_task(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                debug!(
                    "Subscription manager health check: {} active clients",
                    client_count
                );
            }
        });
