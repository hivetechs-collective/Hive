...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env().add_directive("hive=info".parse()?),
        )
        .init();
    ...
    assert!(
        !CONSENSUS_ACTIVE.load(Ordering::Relaxed),
    ...
    let avg_cpu_during_consensus = measurements
        .iter()
