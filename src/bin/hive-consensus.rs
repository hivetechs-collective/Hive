...
use hive_ai::desktop::terminal_cwd_tracker::{
    provide_terminal_cwd_tracker, use_terminal_cwd_tracker,
};
use hive_ai::desktop::terminal_tabs::{TerminalTab, TerminalTabs};
use hive_ai::desktop::terminal_xterm_simple::TerminalXterm;

// LazyGit imports
use hive_ai::desktop::git_ui_wrapper::{ensure_lazygit_installed, find_git_root, LazyGitWrapper};
...
use hive_ai::desktop::git::{
    get_lazygit_file_diff, get_lazygit_file_statuses, initialize_git_statusbar_integration,
    provide_git_context, setup_git_watcher_integration, use_git_context, use_git_state,
    CancellationToken, DiffViewMode, GitEvent, GitOperation, GitOperationProgress, GitOperations,
    GitRepository, GitState, GitStatusMenu, GitToolbar, GitWatcher, ProgressCallback,
};

// Enhanced Status Bar imports
use hive_ai::desktop::status_bar_enhanced::{
    EnhancedStatusBar, StatusBarAlignment, StatusBarItem, StatusBarState,
};

// Event Bus imports
use hive_ai::desktop::events::{event_bus, Event, EventPayload, EventType};
...
use hive_ai::desktop::components::{parse_operations_from_content, OperationStatus};
use hive_ai::desktop::consensus_integration::{
    use_consensus_with_version, DesktopConsensusManager,
};
use hive_ai::desktop::context_menu::{
    ConfirmDialog, ContextMenu, ContextMenuAction, ContextMenuState, FileNameDialog,
};
...
use hive_ai::desktop::status_bar_enhanced::STATUS_BAR_STYLES;
...
fn App() -> Element {
    ...
    use_effect(move || {
        ...
        tracing::info!(
            "API keys version changed to {}, recreating consensus manager",
            version
        );
    ...
    let mut cancellation_flag =
        use_signal(|| std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false))); // Atomic cancellation flag
    ...
    use_effect(move || {
        ...
        spawn(async move {
            ...
            match IDEAIHelperBroker::new(current_dir, file_tree, selected_file).await {
                Ok(broker) => {
                            ...
                            if let Some(repo_path) = current_dir.read().clone() {
                                ...
                                if sync_status.has_upstream {
                                    ...
                                    git_operation_status
                                        .set(Some("Fetching remote changes...".to_string()));
                                    match hive_ai::desktop::git::operations::fetch(&repo_path).await
                                    {
                                        Ok(_) => {
                                            ...
                                            // Step 2: Pull
                                            git_operation_status
                                                .set(Some("Pulling remote changes...".to_string()));
                                            match hive_ai::desktop::git::operations::pull(
                                                &repo_path,
                                            )
                                            .await
                                            {
                                                Ok(_) => {
                                                    ...
                                                    // Step 3: Push
                                                    git_operation_status.set(Some(
                                                        "Pushing local changes...".to_string(),
                                                    ));
                                                    match hive_ai::desktop::git::operations::push(
                                                        &repo_path,
                                                    )
                                                    .await
                                                    {
                                                        Ok(_) => {
                                                            ...
                                                            if let Err(e) = git_state
                                                                .refresh_status(&repo_path)
                                                                .await
                                                            {
                                                            ...
                                                            let mut git_operation_status_clear =
                                                                git_operation_status.clone();
                                                            tokio::time::sleep(
                                        ...
                                        Err(e) => {
                                            ...
                                            {
                                                "❌ Fetch failed - Git credentials not configured"
                                                    .to_string()
                                            } else if e.to_string().contains("unable to access")
                                    ...
                                    if let Some(branch_info) = git_state.branch_info.read().as_ref()
                                    {
                                        ...
                                        git_operation_status.set(Some(format!(
                                            "Publishing branch '{}'...",
                                            branch_name
                                        )));
                                        ...
                                        let result =
                                            hive_ai::desktop::git::operations::push_with_upstream(
                                                &repo_path,
                                                branch_name,
                                            )
                                            .await;
                                        ...
                                        tracing::warn!(
                                            "No branch information available for publishing"
                                        );
                                        git_operation_status.set(Some(
                                            "⚠️ Unable to determine current branch".to_string(),
                                        ));
                                ...
                                git_operation_status.set(Some(
                                    "⚠️ No repository selected - please open a folder first"
                                        .to_string(),
                                ));
    ...
    // Status bar state update effect - handle non-git items
    {
        ...
        use_effect(move || {
            ...
            status_bar_state
                .write()
                .update_item("cursor-position", format!("Ln {}, Col {}", line, col));
        });
    }
                ...
                spawn(async move {
                    ...
                    loop {
                        ...
                        if let Some(repo_path) = current_dir.read().clone() {
                            ...
                            if let Err(e) =
                                hive_ai::desktop::git::operations::fetch(&repo_path).await
                            {
                                ...
                                if let Err(e) = git_state.refresh_status(&repo_path).await {
                                    tracing::warn!(
                                        "Failed to refresh git status after fetch: {}",
                                        e
                                    );
                                }
            ...
            if current_trigger != last_trigger {
                ...
                spawn(async move {
                    ...
                    match fetch_analytics_data().await {
                        Ok(data) => {
                            tracing::debug!(
                                "🎯 Setting refreshed analytics data (trigger: {}):",
                                current_trigger
                            );
                            tracing::debug!("  most_recent_cost: ${:.4}", data.most_recent_cost);
                            tracing::debug!("  today_total_cost: ${:.4}", data.today_total_cost);
                            ...
                            analytics_data.set(data);
                            tracing::info!(
                                "Analytics data refreshed successfully (trigger: {})",
                                current_trigger
                            );
                        }
                        Err(e) => {
    ...
    use_effect({
        ...
        move || {
            ...
            tracing::info!(
                "🔍 Directory changed, scheduling LazyGit update for: {:?}",
                current_path
            );
                ...
                match ensure_lazygit_installed().await {
                    Ok(lazygit_path) => {
                        ...
                        if let Some(git_root) = find_git_root(&current_path) {
                            tracing::info!(
                                "🔍 Found git repository for LazyGit at: {:?}",
                                git_root
                            );
                            ...
                            if let Some(old_id) = lazygit_terminal_id.read().as_ref() {
                                ...
                                use hive_ai::desktop::terminal_registry::{
                                    send_to_terminal, unregister_terminal,
                                };
                            ...
                            tracing::info!(
                                "✅ Created new LazyGit terminal: {} for git repository: {:?}",
                                terminal_id,
                                git_root
                            );
                            ...
                            if lazygit_terminal_id.read().is_some() {
                                ...
                                tracing::info!(
                                    "⚠️ Not in a git repository, cleared LazyGit for: {:?}",
                                    current_path
                                );
                            } else {
                                tracing::info!(
                                    "⚠️ Not in a git repository, LazyGit not available for: {:?}",
                                    current_path
                                );
                            }
                        }
    ...
    use_effect({
        ...
        move || {
            ...
            if let Some(terminal_id) = terminal_id_opt {
                tracing::info!(
                    "⚡ LazyGit width changed to {}, triggering immediate resize",
                    width
                );
                ...
                spawn(async move {
                    ...
                    tracing::info!(
                        "⚡ Forcing LazyGit resize to {}x{} for width {}",
                        new_cols,
                        new_rows,
                        width
                    );
        ...
        move || {
            ...
            spawn(async move {
                ...
                if let Some(current_dir_path) = current_dir_opt {
                    ...
                    match file_system::load_directory_tree(&current_dir_path, &expanded_map, false)
                        .await
                    {
                        Ok(files) => {
                    ...
                    spawn(async move {
                        ...
                        match load_active_profile_from_db().await {
                            Ok(profile) => {
                                ...
                                app_state_for_profile.write().consensus.active_profile_name =
                            ...
                            Err(e) => {
                                tracing::error!(
                                    "❌ Failed to load active profile on startup: {}",
                                    e
                                );
                            }
                        }
                ...
                spawn(async move {
                        ...
                        .await
                    {
                        Ok(files) => {
                            ...
                            tracing::info!(
                                "Initialized current_project with root path: {}",
        ...
        move |action: MenuAction| {
            match action {
                                ...
                                spawn(async move {
                                    ...
                                    if let Ok(repo) = GitRepository::open(&folder_path) {
                                        if let Ok(branch_name) = repo.current_branch() {
                                            ...
                                            let (ahead, behind) =
                                                repo.ahead_behind().unwrap_or((0, 0));
                                            let has_upstream =
                                                repo.upstream_branch().unwrap_or(None).is_some();

                                            let branch_info = hive_ai::desktop::git::BranchInfo {
                                                name: branch_name,
                                                branch_type:
                                                    hive_ai::desktop::git::BranchType::Local,
                                                is_current: true,
                                        ...
                                        match hive_ai::desktop::git::get_lazygit_file_statuses(
                                            &folder_path,
                                        )
                                        .await
                                        {
                                        ...
                                        let active_git_watcher_for_setup =
                                            active_git_watcher_clone.clone();
                                    ...
                                    tracing::info!(
                                        "Updated git context for folder: {:?}",
                                        folder_path
                                    );
                                    ...
                                    if let Some(repo) = git_context_clone.active_repository() {
                                        ...
                                        match GitWatcher::new(&repo_path) {
                                            Ok((watcher, mut event_rx)) => {
                                                tracing::info!(
                                                    "✅ Git watcher started for repository"
                                                );
                                                ...
                                                let mut git_state_for_events =
                                                    git_state_clone.clone();
                                                            ...
                                                            | GitEvent::RemoteChanged => {
                                                                    ...
                                                                    )
                                                                {
                                                                        ...
                                                                        repo.current_branch()
                                                                    {
                                                                        ...
                                                                        git_state_for_events
                                                                            .branch_info
                                                                            .set(Some(branch_info));
                                ...
                                if let Some(manager) = consensus_manager.read().clone() {
                                    ...
                                    spawn(async move {
                                        if let Err(e) = manager
                                            .update_repository_context_with_path(folder_path)
                                            .await
                                        {
                                            tracing::warn!(
                                            ...
                                            tracing::info!(
                                                "✅ Repository context updated for opened folder"
                                            );
                                        }
                                    });
                                }
                                ...
                                spawn(async move {
                                    if let Some(broker) = ide_ai_broker.read().as_ref() {
                                        if let Err(e) = broker.update_repository_context().await {
                                            tracing::warn!(
                                                "IDE AI Helper Broker failed to update context: {}",
                                                e
                                            );
                                        } else {
                                            tracing::info!(
                                                "✅ IDE AI Helper Broker context updated"
                                            );
                                        }
                                    }
                                });
                                ...
                                .await
                                {
                                    Ok(files) => {
                                        ...
                                        let project_info = hive_ai::desktop::state::ProjectInfo {
                                            ...
                                            language: None, // Will be detected by repository analyzer
                                            git_status:
                                                hive_ai::desktop::state::GitStatus::NotRepository,
                                            git_branch: None,
                                            file_count: 0, // Will be updated later
                                        };
                                        ...
                                        tracing::info!(
                                            "Updated current_project with root path: {}",
                                            folder.path().display()
                                        );
                                    }
                                    Err(e) => {
                            ...
                            if let Some(file) = rfd::AsyncFileDialog::new().pick_file().await {
                                ...
                                match file_system::read_file_content(file.path()).await {
                                    ...
                                    Err(e) => {
                                        ...
                                        tab_contents
                                            .write()
                                            .insert(path_string, error_content.clone());
                ...
                MenuAction::CloseFolder => {
                    ...
                    spawn(async move {
                        git_context_clone
                            .set_active_path(std::env::current_dir().unwrap_or_default())
                            .await;
                    ...
                    spawn({
                        ...
                        async move {
                            ...
                            if let Some(folder) = dialog.pick_folder().await {
                                                    ...
                                                    while let Some(event) = event_rx.recv().await {
                                                        ...
                                                        match event {
                                                            ...
                                                            GitEvent::FileStatusChanged(
                                                                ...
                                                                {
                                                                    if let Ok(all_statuses) =
                                                                        repo.file_statuses()
                                                                    {
                                                                        let mut current_status_map =
                                                                            git_state_for_events
                                                                                .file_statuses
                                                                                .read()
                                                                                .clone();
                                ...
                                // Update repository context for AI Helper
                                tracing::info!(
                                    "📁 User opened folder from welcome screen: {}",
                                    folder.path().display()
                                );
...
) -> Result<Vec<(String, String, f64, u64, f64, f64)>, Box<dyn std::error::Error + Send + Sync>> {
    use hive_ai::core::database::get_database;
...
) -> Result<(f64, f64, f64, f64, f64, f64), Box<dyn std::error::Error + Send + Sync>> {
    use hive_ai::core::database::get_database;
