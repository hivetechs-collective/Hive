...
pub fn process_ai_helper_events(mut app_state: Signal<crate::desktop::state::AppState>) {
    if let Some(event) = AI_HELPER_EVENTS.read().as_ref() {
        match event {
            AIHelperUIEvent::NavigateToPath { path, reason } => {
                tracing::info!(
                    "🧭 AI Helper navigating to: {} ({})",
                    path.display(),
                    reason
                );
                ...
                if path.is_dir() {
                    ...
                    spawn(async move {
                        if let Ok(project_info) =
                            crate::desktop::state::ProjectInfo::from_path(path_clone).await
                        {
                    ...
                    if let Some(parent) = path.parent() {
                        app_state.write().file_explorer.selected_directory =
