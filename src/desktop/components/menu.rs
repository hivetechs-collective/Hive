...
pub fn Menu<T: MenuItem>(props: MenuProps<T>) -> Element {
    ...
    let find_next_item = |current: Option<usize>, items: &[T], forward: bool| -> Option<usize> {
        ...
        let start = current.map(|i| if forward { i + 1 } else { i.wrapping_sub(1) }).unwrap_or(0);
    ...
    let style = if let Some(pos) = &props.position {
        format!(
            ...
            pos.x, pos.y, props.width,
            props.max_height.map(|h| format!("max-height: {}px; overflow-y: auto;", h)).unwrap_or_default()
        )
    } else {
        format!(
            "width: {}px; {}",
