...
use dioxus::prelude::*;
use crate::consensus::operation_intelligence::{UnifiedScore, OperationAnalysis};
...
/// Component scores breakdown
#[component]
pub fn ComponentScoresBreakdown(scores: crate::consensus::operation_intelligence::ComponentScores) -> Element {
    rsx! {
        div {
...
#[component]
pub fn ScoringFactorsDisplay(factors: crate::consensus::operation_intelligence::ScoringFactors) -> Element {
    rsx! {
        div {
...
pub fn FactorItem(
    ...
    let impact_class = if impact > 0.0 { "positive" } else if impact < 0.0 { "negative" } else { "neutral" };
...
fn get_operation_type_label(operation: &crate::consensus::stages::file_aware_curator::FileOperation) -> &'static str {
    match operation {
...
fn get_operation_file_name(operation: &crate::consensus::stages::file_aware_curator::FileOperation) -> String {
    match operation {
        ...
        crate::consensus::stages::file_aware_curator::FileOperation::Move { destination, .. } => {
            destination.file_name().unwrap_or_default().to_string_lossy().to_string()
        }
    }
}