...
impl Cursor {
    ...
    pub fn extend_by(
        ...
        self.primary.active = move_position(
            self.primary.active,
            line_delta,
            col_delta,
            max_lines,
        ...
        for selection in &mut self.secondary {
            selection.active =
                move_position(selection.active, line_delta, col_delta, max_lines, max_cols);
        }
    }
    ...
    fn merge_overlapping(&mut self) {
        ...
        self.secondary
            .sort_by_key(|s| (s.start().line, s.start().column));
        self.secondary
...
fn move_position(
    ...
    let new_line =
        (pos.line as isize + line_delta).clamp(0, max_lines.saturating_sub(1) as isize) as usize;
