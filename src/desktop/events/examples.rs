...
use super::{event_bus, Event, EventType, EventPayload, FileChangeType, ConsensusStage};
use anyhow::Result;
use std::path::PathBuf;

/// Example: Git component publishing status changes
pub async fn publish_git_status_change(branch: String, modified: Vec<String>, staged: Vec<String>) -> Result<()> {
    let bus = event_bus();
...
pub async fn publish_build_events() -> Result<()> {
    ...
    // Build completed
    let complete_event = Event::build_completed(
        true,
        vec![],
        vec!["unused variable: `x`".to_string()],
        2000,
    );
...
pub async fn setup_status_bar_subscriptions() -> Result<()> {
    ...
    bus.subscribe(EventType::GitStatusChanged, |event| {
        if let EventPayload::GitStatus { branch, modified_files, .. } = &event.payload {
            tracing::info!("Status bar: Git branch {} with {} modified files", branch, modified_files.len());
        }
        Ok(())
    }).await;
    ...
    bus.subscribe(EventType::BuildCompleted, |event| {
        if let EventPayload::BuildResult { success, duration_ms, .. } = &event.payload {
            tracing::info!("Status bar: Build {} in {}ms", if *success { "succeeded" } else { "failed" }, duration_ms);
        }
        Ok(())
    }).await;
...
pub async fn setup_consensus_ui() -> Result<()> {
    ...
    bus.subscribe_async(EventType::ConsensusStateChanged, |event| async move {
        if let EventPayload::ConsensusState { stage, progress, message } = &event.payload {
            match stage {
...
pub async fn setup_file_explorer() -> Result<()> {
    ...
    let handle = bus.subscribe_async(EventType::FileChanged, |event| async move {
        if let EventPayload::FileChange { path, change_type } = &event.payload {
            match change_type {
                ...
                FileChangeType::Renamed { old_path } => {
                    tracing::info!("File explorer: File renamed from {:?} to {:?}", old_path, path);
                    // Update tree view...
                }
            }
        }
    ...
    bus.subscribe_async(EventType::RepositoryChanged, |event| async move {
        if let EventPayload::RepositoryInfo { path, name, .. } = &event.payload {
            tracing::info!("File explorer: Repository changed to {} at {:?}", name, path);
...
pub async fn setup_config_handler() -> Result<()> {
    ...
    bus.subscribe_async(EventType::ConfigurationChanged, |event| async move {
        if let EventPayload::ConfigChange { section, key, old_value, new_value } = &event.payload {
            tracing::info!(
