...
use crate::desktop::git::{FileStatus, GitRepository, StatusType};
use anyhow::Result;
    ...
    pub async fn handle_git_event(git_event: GitEvent) -> Result<()> {
        ...
        match git_event {
            GitEvent::StatusChanged { repository, branch } => {
                ...
                info!(
                    "Git event: Status changed in {} on branch {}",
                    repository, branch
                );
            }
            GitEvent::BranchChanged {
                ...
                info!(
                    "Git event: Branch changed from {} to {} in {}",
                    old_branch, new_branch, repository
                );
