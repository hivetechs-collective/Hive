...
use git2::{DiffFormat, DiffOptions, Object, Oid, Repository, Tree};
use serde::{Deserialize, Serialize};
use std::path::{Path, PathBuf};
use std::time::{SystemTime, UNIX_EPOCH};
use tracing::{error, info, instrument, warn};

use super::{get_optimized_git_manager, log_performance_stats, GitOperations};
...
impl GitStash {
    ...
    fn get_repo(&self) -> Result<Repository> {
        Repository::discover(&self.repo_path).context("Failed to discover git repository")
    }
    ...
    fn get_stash_changes(
        &self,
        repo: &Repository,
        stash_oid: Oid,
    ) -> Result<(Vec<String>, StashStats)> {
    ...
    pub fn has_stashes(&self) -> bool {
        let Ok(mut repo) = self.get_repo() else {
            return false;
        };
