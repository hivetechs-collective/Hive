...
pub fn compute_diff_with_context(original: &str, modified: &str, file_path: Option<&Path>) -> DiffResult {
    ...
    let diff_prefix = format!(
        "{}_{}", 
        file_path.and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("diff"),
...
pub async fn get_file_diff_with_context(
    ...
    let mut diff_result = compute_diff_with_context(&head_content, &current_content, Some(file_path));
...
impl DiffGitOperations {
    ...
    pub async fn stage_hunk(&self, file_path: &Path, hunk: &DiffHunk) -> Result<DiffActionResult> {
        ...
        let message = if success {
            format!("Staged hunk {} successfully in {:?}", hunk.hunk_id, start_time.elapsed())
        } else {
            format!("Failed to stage hunk {}: {}", hunk.hunk_id, String::from_utf8_lossy(&result.stderr))
        };
        ...
        Ok(DiffActionResult {
            ...
            undo_id: if success { Some(self.generate_undo_id()) } else { None },
        })
    }
    
    /// Unstage a specific hunk by applying reverse patch
    pub async fn unstage_hunk(&self, file_path: &Path, hunk: &DiffHunk) -> Result<DiffActionResult> {
        let start_time = std::time::Instant::now();
        ...
        let success = result.status.success();
        let message = if success {
            format!("Unstaged hunk {} successfully in {:?}", hunk.hunk_id, start_time.elapsed())
        } else {
            format!("Failed to unstage hunk {}: {}", hunk.hunk_id, String::from_utf8_lossy(&result.stderr))
        };
    ...
    pub async fn stage_lines(&self, file_path: &Path, line_ids: &[String], lines: &HashMap<String, DiffLine>) -> Result<DiffActionResult> {
        let start_time = std::time::Instant::now();
        ...
        for line_id in line_ids {
            if let Some(line) = lines.get(line_id) {
                hunks_to_patch.entry(line.hunk_id.clone()).or_insert_with(Vec::new).push(line.clone());
            }
        }
        ...
        for (hunk_id, hunk_lines) in hunks_to_patch {
            ...
            if result.status.success() {
                ...
                messages.push(format!("Staged {} lines from hunk {}", hunk_lines.len(), hunk_id));
            } else {
                all_success = false;
                messages.push(format!("Failed to stage lines from hunk {}: {}", hunk_id, String::from_utf8_lossy(&result.stderr)));
            }
        }
        
        let message = if all_success {
            format!("Staged {} lines successfully in {:?}: {}", line_ids.len(), start_time.elapsed(), messages.join("; "))
        } else {
        ...
        Ok(DiffActionResult {
            ...
            message,
            updated_hunks,
            undo_id: if all_success { Some(self.generate_undo_id()) } else { None },
        })
    }
    ...
    pub async fn revert_hunk(&self, file_path: &Path, hunk: &DiffHunk) -> Result<DiffActionResult> {
        ...
        let success = result.status.success();
        let message = if success {
            format!("Reverted hunk {} successfully in {:?}", hunk.hunk_id, start_time.elapsed())
        } else {
            format!("Failed to revert hunk {}: {}", hunk.hunk_id, String::from_utf8_lossy(&result.stderr))
        };
    ...
    fn create_hunk_patch(&self, file_path: &Path, hunk: &DiffHunk) -> Result<String> {
        ...
        patch.push_str(&format!("@@ -{},{} +{},{} @@\n", 
            hunk.original_start, hunk.original_count,
            hunk.modified_start, hunk.modified_count));
    ...
    fn create_line_patch(&self, file_path: &Path, lines: &[DiffLine]) -> Result<String> {
        ...
        let min_orig = lines.iter().filter_map(|l| l.original_line_number).min().unwrap_or(1);
        let max_orig = lines.iter().filter_map(|l| l.original_line_number).max().unwrap_or(1);
        let min_mod = lines.iter().filter_map(|l| l.modified_line_number).min().unwrap_or(1);
        let max_mod = lines.iter().filter_map(|l| l.modified_line_number).max().unwrap_or(1);
    ...
    async fn write_temp_patch(&self, patch: &str) -> Result<String> {
        ...
        let patch_file = temp_dir.join(format!("hive_patch_{}.patch", 
    ...
    fn generate_undo_id(&self) -> String {
        format!("undo_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis())
    }
}
...
mod tests {
    ...
    fn test_diff_action_state() {
        ...
        assert_eq!(state.lines.len(), diff.hunks.iter().map(|h| h.lines.len()).sum::<usize>());
    }
}