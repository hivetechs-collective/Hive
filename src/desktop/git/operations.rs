...
use git2::{FetchOptions, ObjectType, Oid, PushOptions, RemoteCallbacks, Repository, Signature};
...
use super::{get_optimized_git_manager, log_performance_stats, GitRepository};
...
impl GitOperations {
    ...
    pub async fn stage_files_batch(&self, file_paths: Vec<PathBuf>) -> Vec<Result<()>> {
        if let Some(processor) = &self.batch_processor {
            let repo_path = self.repo_path.clone();
            processor
                .process_batches(file_paths, move |file_path| {
                    let relative_path = file_path
                        .strip_prefix(&repo_path)
                        .context("File is not in repository")?;
    ...
    pub async fn push_with_upstream(&self, remote_name: &str, branch_name: &str) -> Result<()> {
        ...
        info!(
            "Pushed and set upstream for {} to {}/{}",
        ...
        let fetch_head = repo.fetchhead_foreach(|ref_name, remote_url, oid, is_merge| {
            if is_merge {
                info!(
                    "Merging {} from {:?}",
                    oid,
                    std::str::from_utf8(remote_url).unwrap_or("<invalid UTF-8>")
                );
                true
            } else {
                false
