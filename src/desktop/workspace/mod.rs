...
impl WorkspaceState {
    ...
    pub fn save_state(&self) -> Result<()> {
        ...
        let workspace_hash = format!("{:x}", md5::compute(self.root_path.to_string_lossy().as_bytes()));
    ...
    pub fn restore_state(workspace_path: &Path) -> Result<Self> {
        ...
        let workspace_hash = format!("{:x}", md5::compute(workspace_path.to_string_lossy().as_bytes()));
        let state_file = state_dir.join(format!("{}.json", workspace_hash));
        ...
        let mut state: Self = serde_json::from_str(&json)
            .context("Failed to deserialize workspace state")?;
    ...
    pub fn get_active_repository(&self) -> Option<&RepositoryInfo> {
        self.active_repository.as_ref().and_then(|path| {
    ...
    pub fn is_path_excluded(&self, path: &Path) -> bool {
        ...
        self.workspace_settings.exclude_patterns.iter().any(|pattern| {
