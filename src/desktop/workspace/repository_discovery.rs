...
impl RepositoryDiscoveryService {
    ...
    pub fn from_hive_config(hive_config: &HiveConfig) -> Self {
        let config =
            Self::convert_hive_config_to_discovery_config(&hive_config.repository_discovery);
        Self::new(config)
    }

    /// Convert HiveConfig's RepositoryDiscoveryConfig to our DiscoveryConfig
    fn convert_hive_config_to_discovery_config(
        hive_config: &crate::core::config::RepositoryDiscoveryConfig,
    ) -> DiscoveryConfig {
    ...
    pub async fn discover_repositories(&self) -> Result<Vec<RepositoryMetadata>> {
        ...
        info!(
            "Starting repository discovery with mode: {:?}",
            self.config.scanning_mode
        );
