...
use crate::core::error::{ErrorCategory, HiveError};

/// Global cache instance
static CACHE_MANAGER: Lazy<Arc<CacheManager>> =
    Lazy::new(|| Arc::new(CacheManager::new(CacheConfig::default())));
    ...
    pub async fn put(&self, key: String, data: Vec<u8>, category: CacheCategory) -> Result<()> {
        ...
        // Add to memory cache
        {
            ...
            if let Some((_key, old_entry)) = cache.push(key.clone(), entry.clone()) {
                self.update_memory_stats(-(old_entry.meta.size as isize))
                    .await;
            }
...
mod tests {
    ...
    async fn test_cache_operations() -> Result<()> {
        ...
        cache
            .put(key.to_string(), data.clone(), CacheCategory::General)
            .await?;
    ...
    async fn test_cache_stats() -> Result<()> {
        ...
        cache
            .put("key1".to_string(), vec![1, 2, 3], CacheCategory::Ast)
            .await?;
    ...
    fn test_cache_key_builder() {
        ...
        assert_eq!(
            CacheKey::model_response("gpt-4", "hash123"),
            "model:gpt-4:hash123"
        );
    }
}
