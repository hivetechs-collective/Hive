...
use crate::shell::{ShellIntegration, ShellType, ShellIntegrationStatus};
...
pub fn handle_shell(cmd: ShellCommands, config: Config) -> Result<()> {
    ...
    match cmd {
        ShellCommands::Install { shell, force } => {
            handle_install(shell_integration, shell, force)
        }
        ShellCommands::Setup => {
            handle_setup(shell_integration)
        }
        ShellCommands::Status { detailed, format } => {
            handle_status(shell_integration, detailed, format)
        }
        ShellCommands::Completions { shell, output, install } => {
            handle_completions(shell_integration, shell, output, install)
        }
        ShellCommands::Uninstall { shell, preserve_config, validate } => {
...
fn handle_install(shell_integration: ShellIntegration, shell: Option<ShellSelection>, force: bool) -> Result<()> {
    match shell {
...
fn handle_setup(shell_integration: ShellIntegration) -> Result<()> {
    ...
    for shell in shells {
        if let Err(e) = setup.configure_path(shell) {
            eprintln!("Warning: Failed to configure PATH for {}: {}", shell.as_str(), e);
        } else {
...
fn handle_status(shell_integration: ShellIntegration, detailed: bool, format: Option<OutputFormat>) -> Result<()> {
...
fn display_status_text(statuses: &[ShellIntegrationStatus], detailed: bool) {
    ...
    for status in statuses {
        ...
        println!("\n{} {} Shell", shell_icon, status.shell.as_str().to_uppercase());
        println!("   Completions: {}", if status.completions_installed { "✅" } else { "❌" });
        println!("   PATH:        {}", if status.path_configured { "✅" } else { "❌" });
        println!("   Hooks:       {}", if status.hooks_installed { "✅" } else { "❌" });
        println!("   Aliases:     {}", if status.aliases_configured { "✅" } else { "❌" });
    ...
    let fully_integrated = statuses.iter()
        .filter(|s| s.completions_installed && s.path_configured && s.hooks_installed && s.aliases_configured)
        .count();
...
fn display_status_table(statuses: &[ShellIntegrationStatus], detailed: bool) {
    ...
    for status in statuses {
        println!("│ {:11} │ {:11} │ {:4} │ {:5} │ {:7} │",
                 status.shell.as_str(),
                 if status.completions_installed { "✅" } else { "❌" },
                 if status.path_configured { "✅" } else { "❌" },
                 if status.hooks_installed { "✅" } else { "❌" },
                 if status.aliases_configured { "✅" } else { "❌" });
    }
...
fn display_status_yaml(statuses: &[ShellIntegrationStatus], _detailed: bool) -> Result<()> {
    let yaml = serde_yaml::to_string(statuses)
...
fn handle_completions(shell_integration: ShellIntegration, shell: Option<ShellSelection>, output: Option<PathBuf>, install: bool) -> Result<()> {
...
fn handle_uninstall(shell_integration: ShellIntegration, shell: Option<ShellSelection>, preserve_config: bool, validate: bool) -> Result<()> {
...
pub fn get_integration_report(config: Config) -> Result<String> {
    ...
    for status in &statuses {
        ...
        report.push_str(&format!("- Completions: {}\n", if status.completions_installed { "✅" } else { "❌" }));
...
mod tests {
    ...
    fn test_shell_selection_conversion() {
        ...
        assert_eq!(ShellSelection::Powershell.into(), Some(ShellType::PowerShell));
