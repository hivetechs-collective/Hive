...
use crate::migration::{
    ...
    guide::{display_migration_guide, generate_migration_guide},
    preview_migration,
    rollback::perform_rollback,
    MigrationConfig, MigrationManager, MigrationType, ValidationLevel,
};
...
pub async fn handle_migrate(args: MigrateArgs) -> Result<(), HiveError> {
    match args.command {
        ...
        MigrateCommand::Guide { path, output } => handle_guide_command(path, output).await,
        MigrateCommand::Config { path, verify } => handle_config_migration(path, verify).await,
        MigrateCommand::Database { path, verify } => handle_database_migration(path, verify).await,
        MigrateCommand::Full {
            path,
            backup_path,
            preserve_original,
        ...
        MigrateCommand::Status => handle_status_command().await,
    }
}
...
async fn handle_analyze_command(
    ...
    println!(
        "🔍 Analyzing TypeScript installation at: {}",
        path.display()
    );
    ...
    let output_text = match format.as_str() {
        ...
        _ => {
            return Err(HiveError::DatabaseMigration {
...
async fn handle_preview_command(
    ...
    println!(
        "Compatibility: {:.1}%",
        preview.analysis.compatibility.overall_score * 100.0
    );
...
async fn handle_config_migration(path: PathBuf, verify: bool) -> Result<(), HiveError> {
    ...
    if verify {
        ...
        let config_path = dirs::home_dir()
            .ok_or_else(|| HiveError::EnvVarNotSet {
                var: "HOME".to_string(),
            })?
            .join(".hive")
            .join("config.toml");
        ...
        if config_path.exists() {
            ...
            match toml::from_str::<toml::Value>(&content) {
                ...
                Err(e) => {
                    return Err(HiveError::ConfigInvalid {
                        message: format!("Configuration verification failed: {}", e),
                    })
                }
            }
        }
...
async fn handle_verify_command(validation: String, detailed: bool) -> Result<(), HiveError> {
    ...
    if detailed {
        ...
        println!(
            "  • Migration duration: {:?}",
            result.performance_metrics.migration_duration
        );
        println!(
            "  • Data transfer rate: {:.1} MB/s",
            result.performance_metrics.data_transfer_rate
        );
        println!(
            "  • Memory usage: {} MB",
            result.performance_metrics.memory_usage / 1_000_000
        );
    }
...
async fn handle_status_command() -> Result<(), HiveError> {
    ...
    for ts_path in ts_config_paths {
        if ts_path.exists() {
            ts_found = true;
            println!(
                "📦 TypeScript installation: ✅ Found at {}",
                ts_path.display()
            );
            break;
        }
...
fn parse_validation_level(validation: &str) -> Result<ValidationLevel, HiveError> {
    match validation {
        ...
        _ => Err(HiveError::ConfigInvalid {
            message: format!("Unknown validation level: {}", validation),
        }),
    }
}
