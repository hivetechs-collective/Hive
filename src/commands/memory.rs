...
use crate::core::{
    ...
    memory::{
        get_memory_system, initialize_memory, InsightType, MemoryInsight, SemanticSearchResult,
    },
};
...
pub async fn execute(cmd: MemoryCommand) -> Result<()> {
    ...
    match cmd {
        ...
        MemoryCommand::Patterns { learn, analyze } => manage_patterns(learn, analyze).await,
        MemoryCommand::Insights { period, trends } => generate_insights(&period, trends).await,
        MemoryCommand::Similar {
            conversation,
            threshold,
        } => find_similar_conversations(&conversation, threshold).await,
        MemoryCommand::List { limit } => list_memories(limit).await,
        MemoryCommand::Stats => show_memory_stats().await,
        MemoryCommand::Clear { force } => clear_memories(force).await,
        MemoryCommand::Analyze { patterns, insights } => analyze_memories(patterns, insights).await,
        MemoryCommand::Knowledge { export, format } => manage_knowledge(export, format).await,
    }
}
...
async fn show_memory_stats() -> Result<()> {
    ...
    println!(
        "  • Conversations: {}",
        style(stats.conversation_count).bold()
    );
    println!("  • Messages: {}", style(stats.message_count).bold());
    println!(
        "  • Knowledge entries: {}",
        style(stats.knowledge_count).bold()
    );
    ...
    println!(
        "  • Avg size per conversation: {:.2} KB",
...
async fn generate_insights(period: &str, trends: bool) -> Result<()> {
    ...
    println!(
        "{}",
        style("💡 Generating memory insights...").cyan().bold()
    );
    ...
    if insights.is_empty() {
        ...
        for insight in insights {
            ...
            println!("{} {}", icon, style(&insight.description).bold());
    ...
    if trends {
        ...
        println!(
            "   Most active topics: {}",
            style("rust, async, performance").blue()
        );
...
async fn find_similar_conversations(conversation: &str, threshold: f32) -> Result<()> {
    ...
    println!(
        "{}",
        style("🔍 Finding similar conversations...").cyan().bold()
    );
