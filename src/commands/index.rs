...
use crate::analysis::symbol_index::{IndexStatistics, SymbolIndexer};
...
pub async fn handle_index_build(
    ...
    if !stats.cyclic_dependencies.is_empty() {
        println!(
            ...
            if stats.cyclic_dependencies.len() == 1 {
                "circular dependency"
            } else {
...
fn discover_source_files(
    ...
    const SOURCE_EXTENSIONS: &[&str] = &[
        "rs", "ts", "tsx", "js", "jsx", "py", "go", "java", "cpp", "c", "cc", "cxx", "h", "hpp",
...
pub async fn handle_index_stats() -> Result<()> {
    ...
    println!(
        "\n🐝 {} {}",
        style("Symbol Index Statistics").bold().cyan(),
        style("━".repeat(40)).dim()
    );
    ...
    println!(
        "   Files Indexed: {}",
        style(stats.files_indexed).green().bold()
    );
    println!(
        "   Total Symbols: {}",
        style(stats.total_symbols).cyan().bold()
    );
    println!(
        "   Total References: {}",
        style(stats.total_references).yellow()
    );
    ...
    if !stats.cyclic_dependencies.is_empty() {
        ...
        if stats.cyclic_dependencies.len() > 5 {
            println!(
                "   ... and {} more cycles",
                stats.cyclic_dependencies.len() - 5
            );
        }
    }
