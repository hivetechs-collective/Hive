...
use crate::hooks::{
    config::example_configs, ConsensusIntegration, ConsensusIntegrationConfig, CostController,
    EventSource, EventType, HookEvent, HookId, HooksSystem, QualityGateManager,
};
...
pub async fn handle_hooks(command: HookCommands, hooks_system: Arc<HooksSystem>) -> Result<()> {
    match command {
        ...
        HookCommands::Add { config, enable } => handle_add_hook(hooks_system, config, enable).await,
...
async fn handle_list_hooks(
    ...
    for hook in filtered_hooks {
        ...
        if detailed {
            ...
            println!(
                "   Created: {}",
                hook.metadata.created_at.format("%Y-%m-%d %H:%M:%S")
            );
            println!();
        }
    }
    ...
    let event = match event_str.to_lowercase().as_str() {
        ...
        _ => {
            if event_str.starts_with("custom:") {
                ...
                return Err(anyhow!(
                    "Unknown event type: {}. Use 'custom:name' for custom events.",
                    event_str
                ));
            }
        }
...
pub async fn generate_hook_examples(output_dir: &PathBuf) -> Result<()> {
    ...
    println!(
        "📋 Generating example hook configurations in: {}",
        output_dir.display()
    );
...
pub async fn handle_consensus_integration(
    ...
    match subcommand {
        ...
        _ => Err(anyhow!(
            "Unknown consensus integration command: {}",
            subcommand
        )),
    }
}
...
async fn configure_consensus_integration(
    ...
    match args[0].as_str() {
        ...
        "performance-alerts" => {
            if args.len() < 2 {
                return Err(anyhow!(
                    "Performance alert level required (info|warning|error|critical)"
                ));
            }
            let level = &args[1];
            match level.as_str() {
                ...
                _ => {
                    return Err(anyhow!(
                        "Invalid alert level. Use: info|warning|error|critical"
                    ));
                }
            }
        }
