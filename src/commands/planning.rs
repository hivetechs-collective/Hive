...
use crate::core::{get_config, HiveError};
use crate::planning::{ModeType, Plan, PlanningContext, PlanningEngine};
use anyhow::Result;
use chrono::Utc;
use console::style;
...
pub async fn handle_plan(
    ...
    let consensus_engine = ConsensusEngine::new(None).await.map_err(|e| {
        HiveError::Planning(format!("Failed to initialize consensus engine: {}", e))
    })?;
    ...
    let plan = if is_repository {
        ...
        planning_engine
            .create_plan_with_repository(&goal, &current_dir, context)
            .await?
    } else {
    ...
    println!();
    println!(
        "📝 Use {} to execute this plan",
        style("hive execute <plan-file>").cyan()
    );
    println!(
        "🔍 Use {} to view task details",
        style("hive decompose <task>").cyan()
    );
    println!(
        "⚠️  Use {} to analyze risks in detail",
        style("hive analyze-risks <plan-file>").cyan()
    );
...
pub async fn handle_decompose(task: String, depth: Option<usize>, estimate: bool) -> Result<()> {
    println!(
        "🔍 {} task: {}",
        style("Decomposing").bold(),
        style(&task).cyan()
    );
    ...
    let subtasks = decomposer
        .decompose(&task, &context, &*planning_engine.consensus_engine)
        .await?;
    ...
    for (i, subtask) in subtasks.iter().enumerate() {
        ...
        if estimate {
            println!(
                "     ⏱️  Estimated: {} hours",
                subtask.estimated_duration.num_hours()
            );
        }
    ...
    if estimate {
        ...
        println!(
            "⏱️  {} {} hours",
            style("Total Estimated Time:").bold(),
            total_hours
        );
        println!(
            "📅 Approximately {} days with parallel work",
            (total_hours as f64 / 8.0 / 2.0).ceil()
        );
    }
...
pub async fn handle_analyze_risks(project: Option<String>, mitigation: bool) -> Result<()> {
    let project_name = project.unwrap_or_else(|| "current project".to_string());
    println!(
        "⚠️  {} for: {}",
        style("Analyzing Risks").bold(),
...
pub async fn handle_timeline(project: Option<String>, dependencies: bool) -> Result<()> {
    let project_name = project.unwrap_or_else(|| "current project".to_string());
    println!(
        "📅 {} for: {}",
        style("Estimating Timeline").bold(),
        style(&project_name).cyan()
    );
    ...
    println!(
        "   • Start Date: {}",
        style(Utc::now().format("%Y-%m-%d")).cyan()
    );
    println!(
        "   • End Date: {}",
        style(Utc::now().format("%Y-%m-%d")).cyan()
    ...
    println!();
    println!(
        "💡 {} Reduce timeline by 20% through parallel development",
...
pub async fn handle_collaborate(
    ...
    println!(
        "🤝 {} on: {}",
        style("Collaborative Planning").bold(),
        style(&plan).cyan()
    );
    ...
    if share {
        ...
        println!(
            "🔗 Share URL: {}",
            style("https://hive.ai/plans/abc123").cyan().underlined()
        );
...
fn display_plan_summary(plan: &Plan, show_risks: bool, show_timeline: bool) {
    ...
    if show_timeline {
        println!(
            "   Duration: {} days",
            plan.timeline.total_duration.num_days()
        );
