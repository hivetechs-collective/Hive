...
use crate::{
    transformation::{
        TransformationRequest, TransformationEngine, TransformationHistory,
        transform_code, quick_validate,
    },
...
pub async fn handle_improve(
    ...
    if preview {
        ...
        println!("{}: {}", "Description".bold(), transformation_preview.transformation.description);
        println!("{}: {}", "Aspect".bold(), transformation_preview.transformation.request.aspect);
        println!("{}: {} files modified\n", "Impact".bold(), transformation_preview.impact.files_modified);
        ...
        if !apply {
            println!("{}", "To apply these changes, run with --apply flag".yellow());
        }
    }
    ...
    if apply {
        ...
        engine.apply_transformation(&transformation_preview.transformation.id).await?;
        
        println!("✅ {} Applied successfully!", "Transformation".green().bold());
        println!("   Transaction ID: {}", transformation_preview.transformation.id);
        println!("   Files modified: {}", transformation_preview.impact.files_modified);
        
        if transformation_preview.impact.tests_affected {
            println!("\n{} Tests may be affected. Consider running your test suite.", "⚠️ ".yellow());
        }
        
        println!("\n{} Use 'hive undo' to revert these changes if needed.", "💡".cyan());
    }
...
pub async fn handle_undo() -> Result<()> {
    ...
    println!("✅ {} Successfully undone!", "Transformation".green().bold());
...
pub async fn handle_redo() -> Result<()> {
    ...
    println!("✅ {} Successfully redone!", "Transformation".green().bold());
...
pub async fn handle_transform_history(limit: usize) -> Result<()> {
    ...
    if transformations.is_empty() {
        ...
        for (idx, transform) in transformations.iter().enumerate() {
            let status = if transform.applied { "✓ Applied".green() } else { "○ Pending".yellow() };
            println!("{:>3}. {} [{}]", idx + 1, transform.id, status);
            println!("     Time: {}", transform.timestamp.format("%Y-%m-%d %H:%M:%S"));
...
pub async fn handle_apply(
    ...
    let transformation_id = parsed.get("transformation_id")
        .and_then(|v| v.as_str())
    ...
    if preview {
        ...
        if let Some(transformation) = transformations.into_iter().find(|t| t.id == transformation_id) {
...
pub async fn handle_preview(
    ...
    if let Some(changes_str) = changes {
        ...
        // Display preview
        println!("\n{}", "=== AI Suggestions ===".bold());
        println!("{}: {}", "Description".bold(), preview.transformation.description);
        println!("{}: {} files would be modified\n", "Impact".bold(), preview.impact.files_modified);
...
pub async fn handle_transform(
    ...
    for file in files {
        ...
        // Show preview
        println!("  {} changes suggested", preview.transformation.changes.len());
        ...
        if input.trim().to_lowercase() == "y" {
            ...
            engine.apply_transformation(&preview.transformation.id).await?;
    ...
    println!("\n✅ {} Transformation complete!", "AI-guided".green().bold());
...
pub fn list_aspects() {
    ...
    println!();
    println!("{} You can also use custom aspects based on your needs.", "💡".cyan());
}