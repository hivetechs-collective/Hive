...
use crate::analysis::{
    ...
    repository_intelligence::{
        ArchitectureInfo, ArchitecturePattern, Effort, Impact, PerformanceReport, Priority,
        QualityReport, Recommendation, RepositoryAnalyzer, SecurityReport, Severity,
        TechnicalDebtReport,
    },
...
pub async fn analyze_codebase(path: &Path) -> Result<serde_json::Value> {
    ...
    let repository_analyzer =
        RepositoryAnalyzer::new(symbol_indexer.clone(), dependency_analyzer.clone()).await?;
...
pub async fn handle_analyze(
    ...
    match depth.as_str() {
        ...
        _ => {
            ...
            println!(
                "  Tech Debt: ${:.0}",
                analysis.technical_debt.estimated_cost
            );
        }
    }
...
fn display_architecture_info(
    architecture: &crate::analysis::repository_intelligence::ArchitectureInfo,
) {
    println!(
        "\n🏗️  {} {}",
        style("Architecture:").bold().cyan(),
        style("━".repeat(50)).dim()
    );

    println!(
        "  Primary Pattern: {} (confidence: {:.0}%)",
        style(format!("{:?}", architecture.primary_pattern))
            .green()
            .bold(),
...
fn display_quality_report(quality: &crate::analysis::repository_intelligence::QualityReport) {
    ...
    println!(
        "    • Avg Complexity: {:.1}",
        quality.metrics.cyclomatic_complexity
    );
    println!(
        "    • Test Coverage: {:.0}%",
        quality.metrics.test_coverage * 100.0
    );
    println!(
        "    • Documentation: {:.0}%",
        quality.metrics.documentation_coverage * 100.0
    );
    println!(
        "    • Code Duplication: {:.1}%",
        quality.metrics.duplication_ratio * 100.0
    );

    // Display top issues
    if !quality.issues.is_empty() {
        println!("\n  Top Issues:");
        let critical_issues = quality
            .issues
            .iter()
        ...
        for issue in critical_issues {
            ...
            println!("    {} {:?}: {}", severity_icon, issue.kind, issue.message);
        }
...
fn display_performance_summary(
    performance: &crate::analysis::repository_intelligence::PerformanceReport,
) {
    println!(
        "\n⚡ {} {}",
    ...
    println!(
        "  Hotspots: {} | Bottlenecks: {} | Opportunities: {}",
...
fn display_technical_debt(debt: &crate::analysis::repository_intelligence::TechnicalDebtReport) {
    println!(
        "\n💰 {} {}",
        style("Technical Debt:").bold().cyan(),
        style("━".repeat(50)).dim()
    );

    println!(
        "  Total Debt: {} hours (${:.0} @ $100/hr)",
        style(format!("{:.0}", debt.total_debt_hours)).red().bold(),
        debt.estimated_cost
    );

    println!(
        "  Debt Ratio: {:.1}% of development effort",
...
fn display_recommendations(
    recommendations: &[crate::analysis::repository_intelligence::Recommendation],
) {
    ...
    println!(
        "\n💡 {} {}",
        style("Top Recommendations:").bold().cyan(),
        style("━".repeat(50)).dim()
    );
...
fn format_quality_score(score: f32) -> String {
    ...
    format!(
        "{} {}",
        emoji,
        style(format!("{:.1}/10", score)).fg(color).bold()
    )
}
...
pub async fn handle_dependency_analysis(
    ...
    println!(
        "  Total Modules: {}",
        style(analysis.stats.total_modules).green()
    );
    println!(
        "  Total Dependencies: {}",
        style(analysis.stats.total_dependencies).yellow()
    );
    println!(
        "  External Dependencies: {}",
        style(analysis.stats.external_dependencies).blue()
    );
    println!("  Max Dependency Depth: {}", analysis.stats.max_depth);
    println!(
        "  Avg Dependencies/Module: {:.1}",
        analysis.stats.avg_dependencies_per_module
    );
    ...
    if detect_cycles {
        if analysis.circular_dependencies.is_empty() {
            println!(
                "\n✅ {} detected!",
                style("No circular dependencies").green().bold()
            );
        } else {
            println!(
...
pub async fn handle_find_circular_deps(path: PathBuf) -> Result<()> {
    ...
    if analysis.circular_dependencies.is_empty() {
        ...
        println!(
            "\n⚠️  Found {} circular {}:",
            style(analysis.circular_dependencies.len()).red().bold(),
            if analysis.circular_dependencies.len() == 1 {
                "dependency"
            } else {
                "dependencies"
            }
        ...
        for (i, cycle) in analysis.circular_dependencies.iter().enumerate() {
            ...
            println!(
                "  └─→ {} (back to start)",
                style(cycle.modules[0].display()).yellow()
            );
