...
use super::{HookId, ExecutionContext};
...
impl ApprovalWorkflow {
    ...
    pub async fn request_approval(&self, hook_id: &HookId, context: &ExecutionContext) -> Result<bool> {
    ...
    pub async fn submit_response(&self, response: ApprovalResponse) -> Result<()> {
        ...
        let request = {
            ...
            requests.get(&response.request_id).cloned()
        ...
        responses.entry(response.request_id.clone())
    ...
    async fn simulate_approval(&self, request_id: &str) -> Result<bool> {
        ...
        println!("Expires: {}", request.expires_at.format("%Y-%m-%d %H:%M:%S UTC"));
    ...
    async fn handle_notification(&self, notification: ApprovalNotification) -> Result<()> {
        match notification.urgency {
            NotificationUrgency::Critical => {
                ...
                tracing::error!("CRITICAL approval required: {}", notification.request.message);
            }
            NotificationUrgency::High => {
                tracing::warn!("High priority approval required: {}", notification.request.message);
            }
            NotificationUrgency::Normal => {
