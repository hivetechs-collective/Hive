...
pub use approval::{
    ApprovalRequest as BaseApprovalRequest, ApprovalStatus as BaseApprovalStatus,
    ApprovalWorkflow as BaseApprovalWorkflow,
};
pub use approval_workflow::{
    ApprovalDecision, ApprovalPriority, ApprovalProcessResult,
    ApprovalRequest as EnhancedApprovalRequest, ApprovalStatistics,
    ApprovalStatus as EnhancedApprovalStatus, ApprovalWorkflow as EnhancedApprovalWorkflow,
    ApprovalWorkflowConfig,
};
pub use audit::{AuditEvent, AuditEventType, HookAuditLogger};
pub use conditions::{ConditionEvaluator, HookCondition};
pub use config::{HookConfig, HookLoader};
pub use consensus_integration::{
    ConsensusIntegration, ConsensusIntegrationConfig, CostSummary, PerformanceStatus,
    QualityGateResult, QualityGateStatus, StageHookResult,
};
pub use cost_control::{
    AlertSeverity, Budget, BudgetUpdateResult, CostAlert, CostApprovalRequirement,
    CostControlConfig, CostController, CostEstimate, CostEstimationContext,
    CostOptimizationRecommendation, CostSummary as DetailedCostSummary, CostThreshold,
};
pub use dispatcher::{DispatcherConfig, DispatcherStats, EventDispatcher, RoutingRule};
pub use events::{EventBuilder, EventHandler, EventSource, EventType, HookEvent};
pub use execution::{ExecutionContext, ExecutionResult, HookExecutor};
pub use quality_gates::{
    QualityActionRequired, QualityCriterion, QualityEvaluationResult, QualityFailureType,
    QualityGate, QualityGateAction, QualityGateManager, QualityMetric, QualityStatistics,
    QualityThreshold, QualityViolation, QualityViolationSeverity, RemediationSuggestion,
};
pub use rbac::{HookRbacManager, Permission, Role, Team, User};
pub use registry::{Hook, HookAction, HookId, HookMetadata, HookRegistry};
...
impl HooksSystem {
    ...
    pub async fn new(config_dir: PathBuf) -> Result<Self> {
        ...
        let audit_logger =
            Arc::new(HookAuditLogger::new(config_dir.join("hooks_audit.log")).await?);
        ...
        let event_handler = Arc::new(EventHandler::new(registry.clone(), executor.clone()));
    ...
    pub async fn check_user_permission(
        ...
        self.rbac_manager
            .check_permission(user_id, permission, hook_id)
            .await
    }
    ...
    pub async fn remove_user_from_team(&self, user_id: &str, team_name: &str) -> Result<()> {
        self.rbac_manager
            .remove_user_from_team(user_id, team_name)
            .await
    }

    /// Grant team access to specific hooks
    pub async fn grant_team_hook_access(
        &self,
        team_name: &str,
        hook_ids: Vec<HookId>,
    ) -> Result<()> {
    ...
    pub async fn revoke_team_hook_access(
        &self,
        team_name: &str,
        hook_ids: Vec<HookId>,
    ) -> Result<()> {
    ...
    pub async fn get_team_hooks(&self, team_name: &str) -> Result<Vec<Hook>> {
        ...
        let accessible_hooks = all_hooks
            .into_iter()
            .filter(|hook| {
