...
use crate::consensus::types::{Stage, StageResult, StageAnalytics};
...
pub enum RemediationAction {
    ...
    RetryWithAdjustment { parameter_adjustments: HashMap<String, serde_json::Value> },
    
    /// Switch to a different model
    SwitchModel { target_model: String },
    
    /// Adjust prompt or context
    AdjustPrompt { adjustment_type: PromptAdjustmentType },
...
pub enum BackoffStrategy {
    ...
    /// Exponential backoff
    Exponential { base_delay_seconds: u64, multiplier: f64 },
...
impl QualityGateManager {
    ...
    pub async fn add_gate(&self, gate: QualityGate) -> Result<()> {
        ...
        if gates.iter().any(|g| g.id == gate.id) {
            return Err(anyhow::anyhow!("Quality gate with ID '{}' already exists", gate.id));
        }
    ...
    pub async fn evaluate_stage_result(
        ...
        let applicable_gates: Vec<&QualityGate> = gates
            .iter()
            .filter(|gate| {
                gate.enabled && (gate.stage.is_none() || gate.stage == Some(stage))
            })
            .collect();
                ...
                match &gate.failure_action {
                    ...
                    QualityGateAction::RequestApproval => {
                        if let Some(approval_workflow) = &self.approval_workflow {
                            let approval_request = self.create_quality_approval_request(
                                &gate.id,
                                &gate.name,
                                stage,
                                stage_result,
                            ...
                            actions_required.push(QualityActionRequired::Warn {
                                message: format!("Quality gate '{}' failed (no approval workflow configured)", gate.name),
                            });
                        }
                    }
                    ...
                    QualityGateAction::Warn => {
                        actions_required.push(QualityActionRequired::Warn {
                            message: format!("Quality gate '{}' failed: {}", gate.name, gate_result.failure_reason.unwrap_or_default()),
                        });
                    }
                    QualityGateAction::LogOnly => {
                        // Just log, no action required
                        tracing::warn!("Quality gate '{}' failed: {}", gate.name, gate_result.failure_reason.unwrap_or_default());
                    }
        ...
        // Update metrics
        self.update_quality_metrics(stage, stage_result, &stage_scores).await?;
        ...
        let passed = violations.is_empty() || 
                    !actions_required.iter().any(|action| matches!(action, QualityActionRequired::Block { .. }));
    ...
    async fn evaluate_gate(
        ...
        for criterion in &gate.criteria {
            let criterion_result = self.evaluate_criterion(criterion, stage, stage_result).await?;
            ...
            if !criterion_result.passed {
                let violation = QualityViolation {
                    ...
                    severity: self.determine_violation_severity(&criterion_result.failure_type, criterion_result.score),
    ...
    async fn evaluate_criterion(
        ...
        let metric_value = self.calculate_metric_value(&criterion.metric, stage, stage_result).await?;
        
        let (passed, failure_type, failure_reason) = self.check_threshold(
            &criterion.threshold,
            metric_value,
            &criterion.metric,
        )?;
        
        let expected_value = criterion.threshold.target_value
        ...
        Ok(CriterionEvaluationResult {
            passed,
            score: if passed { 1.0 } else { metric_value.max(0.0).min(1.0) },
        ...
        match metric {
            ...
            QualityMetric::ResponseTime => {
                ...
                Ok(stage_result.analytics
                    .as_ref()
                    .map(|a| {
                        let max_time = self.config.metrics_config.performance_thresholds.max_response_time;
            ...
            QualityMetric::TokenEfficiency => {
                ...
                Ok(stage_result.usage
                    .as_ref()
                    .map(|u| {
                        let max_tokens = self.config.metrics_config.performance_thresholds.max_tokens as f64;
            ...
            QualityMetric::ErrorRate => {
                ...
                Ok(stage_result.analytics
                    .as_ref()
                    .map(|a| {
                        let max_error_rate = self.config.metrics_config.performance_thresholds.max_error_rate;
            ...
            QualityMetric::KeywordPresence { keywords } => {
                ...
                let present_count = keywords.iter()
            ...
            QualityMetric::Custom { name: _, evaluator: _ } => {
    ...
    fn determine_failure_type_for_metric(&self, metric: &QualityMetric) -> QualityFailureType {
        match metric {
            QualityMetric::OverallQuality | QualityMetric::Coherence | QualityMetric::Completeness | QualityMetric::Relevance | QualityMetric::Accuracy => {
                QualityFailureType::LowQuality
            }
            QualityMetric::Safety => QualityFailureType::SafetyViolation,
            QualityMetric::ResponseTime | QualityMetric::TokenEfficiency | QualityMetric::CostEfficiency | QualityMetric::ErrorRate => {
                QualityFailureType::PerformanceDegradation
            }
    ...
    fn analyze_relevance(&self, question: &str, answer: &str) -> Result<f64> {
        ...
        let relevant_terms = question_terms.iter()
    ...
    fn analyze_accuracy(&self, text: &str) -> Result<f64> {
        ...
        let confidence_indicators = [
            "certainly", "definitely", "proven", "confirmed", "established", "fact"
        ];
        
        let uncertainty_indicators = [
            "might", "maybe", "possibly", "potentially", "could be", "seems", "appears"
        ];
    ...
    fn analyze_safety(&self, text: &str) -> Result<f64> {
        ...
        let forbidden_keywords = &self.config.metrics_config.content_quality.forbidden_keywords;
        
        let text_lower = text.to_lowercase();
        let violations = forbidden_keywords.iter()
    ...
    fn analyze_language_consistency(&self, text: &str) -> Result<f64> {
        ...
        if let Some(expected_lang) = &self.config.metrics_config.content_quality.language_detection.expected_language {
    ...
    async fn update_quality_metrics(
        ...
        if let Some(analytics) = &stage_result.analytics {
            let perf_metric = PerformanceMetric {
                ...
                token_usage: stage_result.usage.as_ref().map(|u| u.total_tokens).unwrap_or(0),
                cost: analytics.cost,
        ...
        metrics.quality_trend.data_points.push((chrono::Utc::now(), overall_score));
    ...
    fn calculate_quality_trend(&self, trend: &mut QualityTrend) {
        if trend.data_points.len() < 2 {
            trend.current_average = trend.data_points.first().map(|(_, score)| *score).unwrap_or(0.0);
        ...
        let sum_xy: f64 = trend.data_points.iter().enumerate()
            .map(|(i, (_, score))| i as f64 * score)
            .sum();
        let sum_x2: f64 = (0..trend.data_points.len()).map(|i| (i as f64).powi(2)).sum();
    ...
    async fn create_quality_approval_request(
        ...
        metadata.insert("gate_id".to_string(), serde_json::Value::String(gate_id.to_string()));
        metadata.insert("gate_name".to_string(), serde_json::Value::String(gate_name.to_string()));
        metadata.insert("stage".to_string(), serde_json::Value::String(stage.as_str().to_string()));
        metadata.insert("conversation_id".to_string(), serde_json::Value::String(stage_result.conversation_id.clone()));
        metadata.insert("violations_count".to_string(), serde_json::Value::Number(violations.len().into()));
        ...
        Ok(ApprovalRequest {
            ...
            description: format!(
                ...
                gate_name, stage.as_str(), violations.len()
            ),
    ...
    async fn generate_remediation_suggestions(
        ...
        for violation in violations {
            let applicable_strategies: Vec<&RemediationStrategy> = self.config.remediation.strategies
                .iter()
                .filter(|strategy| strategy.applicable_failures.contains(&violation.violation_type))
                .collect();
    ...
    pub async fn get_quality_statistics(&self) -> Result<QualityStatistics> {
        ...
        let violations_by_type = violations.iter()
            .fold(HashMap::new(), |mut acc, v| {
                *acc.entry(format!("{:?}", v.violation_type)).or_insert(0) += 1;
                acc
            });
        
        let violations_by_severity = violations.iter()
            .fold(HashMap::new(), |mut acc, v| {
        ...
        Ok(QualityStatistics {
            ...
            average_scores_by_stage: metrics.stage_scores.iter()
                .map(|(stage, scores)| {
...
pub enum QualityActionRequired {
    Block { gate_id: String, reason: String },
    Warn { message: String },
    RequestApproval { approval_request: ApprovalRequest },
    Remediate { strategies: Vec<RemediationSuggestion> },
    Custom { action_id: String, gate_id: String },
}
...
mod tests {
    use super::*;
    use crate::consensus::types::{TokenUsage, StageAnalytics};
    ...
    async fn test_quality_evaluation() {
        ...
        let stage_result = StageResult {
            ...
            answer: "Rust is a systems programming language focused on safety, speed, and concurrency.".to_string(),
        ...
        let result = manager.evaluate_stage_result(Stage::Generator, &stage_result).await.unwrap();
