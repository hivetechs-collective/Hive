...
use super::{HookRegistry, HookExecutor, ExecutionContext, HookPriority};
...
impl HookEvent {
    ...
    pub fn with_context(mut self, key: impl Into<String>, value: impl Serialize) -> Self {
        self.context.insert(key.into(), serde_json::to_value(value).unwrap_or(Value::Null));
        self
    }
...
impl EventHandler {
    ...
    pub async fn emit_consensus_event(&self, stage: &str, before: bool) -> Result<()> {
        ...
        let event = HookEvent::new(
            event_type,
            EventSource::Consensus { stage: stage.to_string() }
        );
    ...
    pub async fn emit_file_event(&self, path: PathBuf, operation: &str, before: bool) -> Result<()> {
        ...
        let event = HookEvent::new(
            event_type,
            EventSource::FileSystem { path: path.clone() }
    ...
    pub async fn emit_planning_event(&self, event_type: &str, data: HashMap<String, Value>) -> Result<()> {
    ...
    /// Emit a memory event
    pub async fn emit_memory_event(&self, event_type: &str, data: HashMap<String, Value>) -> Result<()> {
        let event_type = match event_type {
    ...
    pub async fn emit_analytics_event(&self, event_type: &str, data: HashMap<String, Value>) -> Result<()> {
        let event_type = match event_type {
...
mod tests {
    ...
    fn test_event_builder() {
        let event = EventBuilder::new(
            ...
            EventSource::FileSystem { path: PathBuf::from("/test.rs") }
        )
        ...
        assert_eq!(event.context.get("file_size").unwrap(), &Value::Number(1024.into()));
