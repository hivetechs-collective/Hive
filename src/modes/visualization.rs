...
impl ModeVisualizer {
    ...
    pub fn new() -> Self {
        ...
        indicators.insert(ModeType::Planning, ModeIndicator {
            symbol: "📋".to_string(),
            color: Color::Blue,
            animation: AnimationType::None,
        });
        
        indicators.insert(ModeType::Execution, ModeIndicator {
            symbol: "⚡".to_string(),
            color: Color::Green,
            animation: AnimationType::Pulse,
        });
        
        indicators.insert(ModeType::Hybrid, ModeIndicator {
            symbol: "🔄".to_string(),
            color: Color::Cyan,
            animation: AnimationType::Spin,
        });
        
        indicators.insert(ModeType::Analysis, ModeIndicator {
            symbol: "🔍".to_string(),
            color: Color::Yellow,
            animation: AnimationType::None,
        });
        
        indicators.insert(ModeType::Learning, ModeIndicator {
            symbol: "🧠".to_string(),
            color: Color::Magenta,
    ...
    pub fn update_switch_result(&mut self, result: &SwitchResult) {
        if result.success {
            self.add_event(StatusEvent {
                ...
                details: format!("Switched from {:?} in {}ms", 
                    result.from_mode, 
    ...
    pub fn format_detailed_status(&self) -> Vec<String> {
        ...
        // Context
        lines.push(format!("│ Context Items: {:<16} │", self.current_status.context_items));
