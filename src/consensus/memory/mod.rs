...
impl ConsensusMemory {
    ...
    pub async fn new(
        db_service: crate::core::db_actor::DatabaseService,
        ai_helpers: Arc<crate::ai_helpers::AIHelperEcosystem>,
    ) -> Result<Self> {
        // Initialize components
        let fingerprinter = Arc::new(SemanticFingerprinter::new().await?);
        let knowledge_store =
            Arc::new(AuthoritativeKnowledgeStore::new(db_service, fingerprinter.clone()).await?);
        let context_injector =
            Arc::new(ContextInjector::new(knowledge_store.clone(), ai_helpers).await?);
    ...
    pub async fn connect_hive_mind(
        &self,
        ai_helpers: &Arc<crate::ai_helpers::AIHelperEcosystem>,
    ) -> Result<()> {
        ...
        tracing::info!(
            "🐝🧠 Hive mind connected! All consensus stages now share collective knowledge"
        );
        Ok(())
    }
    ...
    pub async fn store_curator_output(
        ...
        let fact = CuratedFact {
            ...
            access_count: 1,
            related_facts: self
                .find_related_facts(curator_output)
                .await
                .unwrap_or_default(),
            topics: self
                .extract_topics(curator_output, source_question)
                .await
                .unwrap_or_else(|_| vec!["general".to_string()]),
            entities: self
                .extract_entities(curator_output)
                .await
                .unwrap_or_default(),
            metadata: crate::consensus::memory::authoritative_store::FactMetadata {
                fact_type: crate::consensus::memory::authoritative_store::FactType::Other(
                ...
                verification_status:
                    crate::consensus::memory::authoritative_store::VerificationStatus::Verified,
            },
        };
    ...
    async fn extract_topics(&self, content: &str, question: &str) -> Result<Vec<String>> {
        ...
        if content_lower.contains("ai")
            || content_lower.contains("artificial intelligence")
            || content_lower.contains("consensus")
        {
    ...
    async fn extract_entities(
        &self,
        content: &str,
    ) -> Result<Vec<crate::consensus::memory::authoritative_store::Entity>> {
        ...
        let module_regex =
            regex::Regex::new(r"\b(crate::|mod |module |package )([\w_]+)\b").unwrap();
