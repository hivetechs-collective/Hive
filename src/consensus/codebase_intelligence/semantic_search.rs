...
use super::{ExtractedObject, CodebaseIntelligence};
...
impl SemanticSearch {
    ...
    pub async fn question_relates_to_codebase(
        ...
        info!("🔍 Semantic search for: {} (terms: {:?})", question, search_terms);
    ...
    fn extract_all_search_terms(&self, question: &str) -> Vec<String> {
        ...
        let stop_words = ["the", "is", "at", "which", "on", "a", "an", "and", "or", 
                          "but", "in", "with", "to", "for", "of", "as", "by", "that",
                          "it", "from", "up", "out", "if", "then", "than", "this"];
    ...
    pub async fn search_indexed_codebase(
        ...
        let fields = ["o.name", "o.signature", "o.documentation", "o.context", "k.keyword"];
        let mut conditions = Vec::new();
        
        for term in search_terms {
            let field_conditions: Vec<String> = fields.iter()
        ...
        let results = stmt.query_map([scan_id], |row| {
            Ok(SearchResult {
                ...
                matching_keywords: row.get::<_, Option<String>>(9)?
    ...
    pub fn build_ai_context(&self, results: &[SearchResult], question: &str) -> String {
        ...
        for (idx, result) in results.iter().take(20).enumerate() {
            ...
            if !result.matching_keywords.is_empty() {
                context.push_str(&format!("🏷️ Related concepts: {}\n", result.matching_keywords.join(", ")));
            }
        ...
        context.push_str("Focus on the actual code and implementation details found in your codebase.\n");
