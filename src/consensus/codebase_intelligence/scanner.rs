...
impl CodebaseScanner {
    ...
    async fn scan_file(&self, root_path: &Path, file_path: &Path) -> Result<Option<ScannedFile>> {
        ...
        if size_bytes > self.max_file_size {
            debug!("Skipping large file: {} ({} bytes)", file_path.display(), size_bytes);
            return Ok(None);
        }
        
        // Detect language
        let language = self.language_detector.detect_from_path(file_path)
    ...
    fn should_ignore_path(&self, path: &Path) -> bool {
        ...
        const IGNORE_DIRS: &[&str] = &[
            "target", "dist", "build", "out", ".git", "node_modules",
            ".idea", ".vscode", "coverage", ".pytest_cache", "__pycache__",
            "venv", ".env", "tmp", "temp", ".next", ".nuxt"
        ];
        
        // Common file patterns to ignore
        const IGNORE_PATTERNS: &[&str] = &[
            ".DS_Store", "Thumbs.db", ".lock", ".log", ".tmp",
            ".cache", ".pyc", ".pyo", ".class", ".o", ".so"
        ];
...
mod tests {
    ...
    async fn test_scanner_basic() {
        ...
        fs::write(temp_path.join("main.rs"), "fn main() {}").await.unwrap();
        fs::write(temp_path.join("lib.rs"), "pub fn test() {}").await.unwrap();
        fs::create_dir(temp_path.join("src")).await.unwrap();
        fs::write(temp_path.join("src/module.rs"), "mod tests;").await.unwrap();
