...
impl AIOperationParser {
    pub fn new() -> Self {
        ...
        operation_patterns.insert(
            "nl_update".to_string(),
            Regex::new(r"(?i)(?:update|modify|change|edit)\s+(?:the\s+)?file\s+[`']([^`']+)[`']").unwrap()
        );
    ...
    pub async fn parse_response(&self, response: &str, context: &OperationContext) -> Result<ParsedOperations> {
        ...
        if let Some(recognizer) = &self.pattern_recognizer {
            let ai_insights = self.get_ai_insights(response, &operations, context, recognizer).await?;
        ...
        let final_confidence = (overall_confidence * operation_confidence_avg / 100.0).clamp(0.0, 100.0);
    ...
    async fn parse_code_block(
        ...
        let mut confidence = if block.header.contains(':') { 90.0 } else { 70.0 };
    ...
    fn is_language_identifier(&self, s: &str) -> bool {
        matches!(s, "rust" | "rs" | "python" | "py" | "javascript" | "js" | 
    ...
    fn infer_path_from_content(&self, content: &str) -> String {
        ...
        let patterns = vec![
            ...
            (r"(?m)^class\s+(\w+)", "py"),
            (r"(?m)^export\s+(?:default\s+)?(?:class|function)\s+(\w+)", "ts"),
        ];
    ...
    async fn parse_natural_language_operations(&self, text: &str) -> Result<Vec<FileOperationWithMetadata>> {
        let mut operations = Vec::new();
    ...
    fn analyze_dependencies(&self, operations: &mut [FileOperationWithMetadata]) {
        ...
        for i in 0..operations.len() {
            for j in 0..i {
                if self.operations_have_dependency(&operations[j].operation, &operations[i].operation) {
    ...
    fn operations_have_dependency(&self, op_a: &FileOperation, op_b: &FileOperation) -> bool {
        match (op_a, op_b) {
            ...
            (FileOperation::Create { path: path_a, .. }, FileOperation::Update { path: path_b, .. }) |
            (FileOperation::Create { path: path_a, .. }, FileOperation::Append { path: path_b, .. }) => {
                path_a == path_b
            }
            ...
            (FileOperation::Rename { to, .. }, FileOperation::Delete { path }) => {
                to == path
            }
            _ => false,
        }
    ...
    fn validate_operation_path(&self, op: &FileOperation) -> Option<String> {
        ...
        if path_str.contains("..") {
            return Some(format!("Path '{}' contains parent directory reference", path_str));
        }
        
        if path.is_absolute() {
            return Some(format!("Path '{}' is absolute, relative paths recommended", path_str));
        }
    ...
    fn generate_clarifications(
        ...
        if operations.is_empty() && !unparsed_blocks.is_empty() {
            clarifications.push(
                "No operations could be parsed. Please use explicit format: ```CREATE:path/to/file".to_string()
            );
        }
        ...
        if operations.iter().any(|op| !op.dependencies.is_empty()) {
            clarifications.push(
                "Detected operation dependencies. Please verify the execution order is correct.".to_string()
            );
        }
