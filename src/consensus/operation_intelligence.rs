...
use crate::ai_helpers::{
    ContextRetriever, KnowledgeIndexer, KnowledgeSynthesizer, PatternRecognizer, QualityAnalyzer,
};
use crate::consensus::operation_history::{
    OperationFilters, OperationHistoryDatabase, OperationRecord,
};
...
pub enum ActionRecommendation {
    ...
    AutoExecute { reason: String, confidence: f32 },
...
impl OperationIntelligenceCoordinator {
    ...
    pub async fn get_unified_score(
        ...
        let recommended_mode =
            self.recommend_auto_accept_mode(analysis.confidence, analysis.risk, &scoring_factors);
    ...
    pub async fn should_auto_execute(
        ...
        info!(
            "🤔 Checking if operation should be auto-executed in {:?} mode",
            mode
        );
    ...
    pub async fn analyze_and_group_operations(
        ...
        for operation in operations {
            let (should_auto, analysis) =
                self.should_auto_execute(operation, context, mode).await?;
    ...
    pub async fn analyze_operation(
        ...
        let risk = self
            .calculate_risk_score(&pattern_analysis, &quality_impact, &operation_plan)
            .await?;

        // Generate recommendation
        let recommendation = self
            .generate_recommendation(confidence, risk, &pattern_analysis, &quality_impact)
            .await?;
    ...
    pub async fn analyze_operations_batch(
        ...
        self.analyze_cross_operation_effects(&mut analyses, operations)
            .await?;
    ...
    pub async fn record_operation_outcome_with_id(
        ...
        if let Some(db) = &self.history_database {
            ...
            if let Some(satisfaction) = user_satisfaction {
                db.add_user_feedback(operation_id, satisfaction, satisfaction >= 3.0, None)?;
            }
        }
    ...
    pub async fn get_success_statistics(&self) -> Result<OperationStatistics> {
        ...
        let successful_operations = history.iter().filter(|h| h.outcome.success).count();

        let average_confidence = if !history.is_empty() {
            history.iter().map(|h| h.predicted_confidence).sum::<f32>() / history.len() as f32
        } else {
            0.0
        };

        let average_risk = if !history.is_empty() {
            history.iter().map(|h| h.predicted_risk).sum::<f32>() / history.len() as f32
        } else {
            0.0
        };
    ...
    async fn analyze_historical_patterns(
        ...
        if let Some(db) = &self.history_database {
            ...
            if !similar_from_db.is_empty() {
                let successful = similar_from_db.iter().filter(|r| r.outcome.success).count();

                let failure_modes = similar_from_db
                    .iter()
                ...
                debug!(
                    "Found {} similar operations in database",
                    similar_from_db.len()
                );
    ...
    async fn analyze_operation_context(
        ...
        let precedents = context_analysis
            .relevant_precedents
            .iter()
            .map(|p| {
                format!(
    ...
    async fn analyze_operation_patterns(
        ...
        let dangerous_patterns = safety_analysis
            .dangerous_patterns
            .iter()
        ...
        let anti_patterns = safety_analysis
            .anti_patterns
            .iter()
            .map(|ap| AntiPattern {
    ...
    async fn analyze_quality_impact(
        ...
        Ok(QualityImpact {
            ...
            rollback_complexity: RollbackComplexity {
                ...
                estimated_rollback_time: risk_assessment
                    .rollback_complexity
    ...
    async fn calculate_confidence_score(
        ...
        let historical_score = if historical.similar_operations_count > 0 {
            ...
            let failure_mode_penalty =
                (historical.common_failure_modes.len() as f32 * 0.05).min(0.3);
        ...
        let context_score = {
            ...
            let precedent_bonus = if context.relevant_precedents.len() > 3 {
                0.1
            } else {
                0.0
            };
        ...
        let pattern_score = {
            ...
            let cluster_bonus = if patterns.cluster_info.is_some() {
                0.1
            } else {
                0.0
            };
        ...
        let weighted_confidence = historical_score * historical_weight
    ...
    async fn calculate_risk_score(
        ...
        let total_risk = risk_components
            .iter()
            .map(|(name, score)| {
                debug!("  {} = {:.1}", name, score);
                score
    ...
    async fn generate_recommendation(
        ...
        let has_critical_danger = patterns
            .dangerous_patterns
            .iter()
            .any(|p| matches!(p.severity, DangerousSeverity::Critical));

        let has_unrecoverable_risk = patterns.dangerous_patterns.iter().any(|p| {
            matches!(
                p.pattern_type,
                DangerousPatternType::DataLoss | DangerousPatternType::SecurityVulnerability
            )
        }) && !quality.rollback_complexity.rollback_possible;

        if has_critical_danger || has_unrecoverable_risk || risk > 80.0 {
            // Block dangerous operations
            let mut dangers = patterns
                .dangerous_patterns
                .iter()
                .map(|p| {
                    format!(
        ...
        let needs_modification =
            patterns.anti_patterns.len() > 2 || (confidence < 50.0 && risk > 40.0);

        if needs_modification && patterns.anti_patterns.len() > 0 {
            // Suggest modifications
            let modifications = patterns
                .anti_patterns
                .iter()
        ...
        if confidence >= auto_execute_threshold && risk < 30.0 {
            ...
            return Ok(ActionRecommendation::AutoExecute { reason, confidence });
        }
        ...
        if quality.overall_impact > 0.0 {
            benefits.push(format!(
                "Positive quality impact: +{:.0}%",
                quality.overall_impact
            ));
        }
        ...
        Ok(ActionRecommendation::RequestConfirmation {
            reason: match (confidence, risk) {
                ...
                (c, _) if c < 60.0 => "Insufficient confidence for automatic execution".to_string(),
                (_, r) if r > 40.0 => "Risk level requires user confirmation".to_string(),
                _ => format!(
    ...
    async fn generate_explanation(
        ...
        let mut explanation = format!("🤖 AI Analysis: Operation to {}\n\n", operation_description);
        ...
        match recommendation {
            ...
            ActionRecommendation::RequestConfirmation {
                risks, benefits, ..
            } => {
                if !risks.is_empty() {
                    explanation.push_str("\n⚠️  Risks to consider:\n");
    ...
    async fn update_helpers_with_outcome(&self, history_entry: &OperationHistory) -> Result<()> {
        // Update AI helpers with learning data
        ...
        if let Some(post_quality) = &history_entry.outcome.post_operation_quality {
            debug!(
                "Recording quality metrics for Quality Analyzer: {:?}",
                post_quality
            );
        }
