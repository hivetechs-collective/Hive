...
use crate::consensus::openrouter::{OpenRouterClient, OpenRouterMessage, OpenRouterRequest};
use crate::consensus::stages::file_aware_curator::FileOperation;
use crate::consensus::stages::{ConsensusStage, GeneratorStage};
use crate::consensus::streaming::{ConsensusEvent, StreamingCallbacks};
...
use crate::consensus::types::{ConsensusProfile, Stage, StageResult};
...
impl InlineOperationParser {
    ...
    fn create_file_operation(&self, op: &PendingOperation) -> FileOperation {
        ...
        match op.operation_type.as_str() {
            "creating" | "writing to" | "adding to" => FileOperation::Create {
                path,
                content: op.content.clone(),
            },
            "updating" | "modifying" => FileOperation::Update {
                path,
                content: op.content.clone(),
            },
            "deleting" => FileOperation::Delete { path },
            _ => {
...
impl DirectExecutionHandler {
    pub fn new(
        ...
        let ai_file_executor = Arc::new(AIConsensusFileExecutor::new((*ai_helpers).clone()));
    ...
    pub async fn handle_request(
        ...
        let mut messages = self
            .generator
            .build_messages(request, None, full_context.as_deref())?;
        ...
        if is_file_operation {
            for message in &mut messages {
                if message.role == "system" {
                    ...
                    message
                        .content
                        .push_str("You are in Direct Execution mode for simple file operations. ");
            ...
            for message in &mut messages {
                if message.role == "system" {
                    ...
                    message.content.push_str(
                        "Please provide a direct, concise answer to the user's question.",
                    );
                    break;
                }
            }
    ...
    async fn get_memory_context(
        ...
        tracing::info!(
            "🧠 Direct mode: Retrieving memory context for conversation {}",
            conversation_id
        );
        ...
        let context = tokio::task::spawn_blocking(move || -> Result<Option<String>> {
            ...
            if found_any {
                tracing::info!(
                    "✅ Direct mode: Found {} recent conversation(s) for context",
                    context_parts.len() / 2
                );
    ...
    pub async fn should_use_direct_path(&self, request: &str) -> Result<bool> {
        ...
        let is_simple = simple_operations
            .iter()
            .any(|&op| request_lower.contains(op));
        ...
        let is_complex = complex_indicators
            .iter()
...
impl crate::consensus::openrouter::StreamingCallbacks for DirectStreamingCallbacks {
                ...
                .load(std::sync::atomic::Ordering::Relaxed)
            {
                ...
                tokio::spawn(async move {
                    ...
                    if let Some(operation) = parser_guard.parse_chunk(&chunk) {
                        ...
                        match ai_file_executor
                            .execute_curator_operations(operations)
                            .await
                        {
                            Ok(report) => {
                                if report.success {
                                    tracing::debug!(
                                        "🤖 AI Helper successfully executed {} operations",
                                    ...
                                    tracing::warn!(
                                        "⚠️ AI Helper execution had errors: {:?}",
                                        report.errors
                                    );
                                }
                            }
                ...
                tokio::spawn(async move {
                    ...
                    if let Some(operation) = parser_guard.parse_chunk(&chunk) {
                        tracing::debug!(
                            "📝 Parsed operation during consensus (execution deferred): {:?}",
                            operation
                        );
                    }
                });
    ...
    fn on_complete(
        ...
        usage: Option<crate::consensus::openrouter::Usage>,
    ) {
        ...
        if let Some(usage_data) = usage {
            ...
            *guard = Some(usage_data);
            tracing::debug!(
                "Direct mode: Captured usage data - prompt: {}, completion: {}",
...
impl DirectExecutionBuilder {
    ...
    pub fn build(self) -> Result<DirectExecutionHandler> {
        Ok(DirectExecutionHandler::new(
            self.generator
                .ok_or_else(|| anyhow::anyhow!("Generator required"))?,
            self.ai_helpers
                .ok_or_else(|| anyhow::anyhow!("AI helpers required"))?,
            self.executor
                .ok_or_else(|| anyhow::anyhow!("Executor required"))?,
            self.client
