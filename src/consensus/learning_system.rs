...
use crate::consensus::outcome_tracker::{
    OperationOutcomeTracker, TrackedOutcome, LearningInsights, RetrainingResult,
    AccuracyMetrics, HelperAccuracy, ErrorPattern
};
...
impl ContinuousLearningSystem {
    ...
    pub async fn run_learning_cycle(&self) -> Result<LearningCycleResult> {
        ...
        let cycle_result = {
            ...
            let result = LearningCycleResult {
                ...
                timestamp: Utc::now(),
                insights,
                experiments_conducted: experiments.iter().map(|e| e.experiment_id.clone()).collect(),
                improvements_applied: applied_improvements.clone(),
                performance_change: self.calculate_performance_change(&applied_improvements).await,
    ...
    pub async fn get_pending_improvements(&self) -> Vec<PendingImprovement> {
        ...
        state.pending_experiments.iter()
    ...
    pub async fn approve_improvement(&self, experiment_id: &str) -> Result<()> {
        let experiment = {
            let mut state = self.learning_state.write().await;
            let experiment_index = state.pending_experiments.iter()
        ...
        if let Some(result) = experiment.results {
            match result.recommendation {
                ExperimentRecommendation::Apply => {
                    self.apply_weight_changes(&experiment.proposed_changes).await?;
                ...
                ExperimentRecommendation::ApplyWithModifications(ref changes) => {
                    ...
                    info!("Applied modified improvement from experiment: {}", experiment_id);
                }
                _ => {
                    warn!("Cannot apply improvement with recommendation: {:?}", result.recommendation);
                }
            }
        }
    ...
    async fn update_performance_baseline(&self, insights: &LearningInsights) -> Result<()> {
        ...
        if state.baseline_performance.is_none() {
            ...
            state.baseline_performance = Some(PerformanceBaseline {
                ...
                helper_accuracies: insights.helper_performance.iter()
    ...
    async fn plan_learning_experiments(&self, insights: &LearningInsights) -> Result<Vec<LearningExperiment>> {
        ...
        for (helper_name, accuracy) in &insights.helper_performance {
            if accuracy.prediction_accuracy < 0.7 {
                ...
                experiments.push(LearningExperiment {
                    ...
                    hypothesis: format!("Reducing weight of {} should improve overall accuracy", helper_name),
        ...
        for pattern in &insights.error_patterns {
            if pattern.frequency > 0.1 { // If pattern occurs in >10% of cases
                experiments.push(LearningExperiment {
                    experiment_id: format!("pattern_fix_{}_{}", pattern.pattern_type, Utc::now().timestamp()),
                    experiment_type: ExperimentType::PatternLearning,
                    proposed_changes: self.pattern_to_weight_adjustments(pattern),
                    hypothesis: format!("Addressing {} pattern should reduce error rate", pattern.pattern_type),
        ...
        for (helper_name, accuracy) in &insights.helper_performance {
            if accuracy.prediction_accuracy > 0.85 && accuracy.suggested_weight_adjustment > 1.0 {
                experiments.push(LearningExperiment {
                    ...
                    hypothesis: format!("Increasing weight of high-performing {} should improve accuracy", helper_name),
    ...
    async fn execute_experiment(&self, experiment: &LearningExperiment) -> Result<ExperimentResult> {
        ...
        self.apply_temporary_changes(&experiment.proposed_changes).await?;
        
        // Run performance evaluation
        let test_performance = self.performance_monitor.evaluate_performance_sample(50).await?;
        
        // Revert changes
        self.revert_temporary_changes(&experiment.proposed_changes).await?;
        
        // Calculate results
        let performance_improvement = test_performance.overall_accuracy - baseline_performance.overall_accuracy;
        let accuracy_change = test_performance.overall_accuracy - baseline_performance.overall_accuracy;
        
        let recommendation = if performance_improvement > self.configuration.min_improvement_threshold {
            if performance_improvement > 0.1 {
                ExperimentRecommendation::Apply
            } else {
                // Apply with reduced magnitude
                let reduced_changes: HashMap<String, f32> = experiment.proposed_changes.iter()
    ...
    async fn apply_improvements(&self, improvements: &[WeightAdjustment]) -> Result<HashMap<String, f32>> {
    ...
    async fn queue_improvements_for_approval(&self, improvements: &[WeightAdjustment]) -> Result<HashMap<String, f32>> {
        let mut state = self.learning_state.write().await;
        ...
        info!("Queued {} improvements for manual approval", improvements.len());
    ...
    async fn apply_weight_changes(&self, changes: &HashMap<String, f32>) -> Result<()> {
        ...
        for (helper_name, adjustment) in changes {
            let current_weight = state.current_weights.get(helper_name).copied().unwrap_or(1.0);
            let new_weight = (current_weight + adjustment).max(0.1).min(2.0); // Clamp between 0.1 and 2.0
            
            state.current_weights.insert(helper_name.clone(), new_weight);
            info!("Updated {} weight: {:.3} -> {:.3}", helper_name, current_weight, new_weight);
        }
    ...
    fn calculate_learning_trend(&self, history: &[LearningCycleResult]) -> LearningTrend {
        ...
        let recent_improvements: f32 = history.iter()
            .rev()
            .take(5)
...
impl PerformanceMonitor {
    ...
    pub async fn evaluate_performance_sample(&self, _sample_size: usize) -> Result<PerformanceMetrics> {
