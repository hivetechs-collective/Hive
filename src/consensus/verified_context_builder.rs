...
use crate::consensus::verification::{build_stage_context, RepositoryFacts, RepositoryVerifier};
...
static GENERAL_KNOWLEDGE_PATTERNS: Lazy<Vec<Regex>> = Lazy::new(|| {
    vec![
        ...
        Regex::new(r"(?i)(city|cities|country|countries|population|capital|state|province)")
            .unwrap(),
...
impl VerifiedContextBuilder {
    ...
    pub fn is_repository_related_question(&self, question: &str) -> bool {
        ...
        let contains_repo_keyword = repo_keywords
            .iter()
            .any(|&keyword| question_lower.contains(keyword));

        // Check if it matches general knowledge patterns
        let is_general_knowledge = GENERAL_KNOWLEDGE_PATTERNS
            .iter()
    ...
    pub async fn with_repository_verification(
        ...
        tracing::info!(
            "Initializing repository verification for: {}",
            repo_path.display()
        );
        ...
        tracing::info!(
            "Repository verification complete: {} v{} ({} deps, {} modules)",
            facts.name,
            facts.version,
    ...
    pub async fn build_verified_stage_context(
        ...
        if is_repo_related {
            if let Some(facts) = &self.repository_facts {
                ...
                tracing::info!(
                    "Question is repository-related, added verification context for {:?} stage",
                    stage
                );
            } else {
        ...
        if let Some(ai_helpers) = ai_helpers {
            match self
                .get_ai_helper_context(&ai_helpers, question, stage)
                .await
            {
                Ok(helper_context) => {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Failed to get AI helper context for {:?} stage: {}",
                        stage,
                        e
                    );
                }
        ...
        tracing::info!(
            "Built verified context for {:?} stage: {} characters",
            stage,
            full_context.len()
        );
    ...
    async fn get_ai_helper_context(
        ...
        if self.is_repository_related_question(question) {
            if let Err(e) = ai_helpers
                .update_repository_facts(self.repository_facts.clone())
                .await
            {
        ...
        match ai_helpers
            .prepare_stage_context(question, stage, 2048)
            .await
        {
            Ok(stage_context) => {
    ...
    fn format_temporal_context(&self, temporal: TemporalContext) -> String {
        ...
        if let Some(business_context) = temporal.business_context {
            parts.push(format!(
                "Business context: Quarter {}, Market hours: {}",
...
"#;
    ...
    async fn test_verified_context_builder() {
        ...
        builder
            .with_repository_verification(root_path)
            .await
            .unwrap();
