...
impl FactChecker {
    ...
    pub fn new(repository_facts: RepositoryFacts) -> Self {
        ...
        claim_extractors.insert(
            ...
            Regex::new(r"(?i)(?:project|package|crate)(?:\s+name)?\s+(?:is\s+)?([a-zA-Z0-9_-]+)").unwrap()
        );
                ...
                if claimed_is_enterprise != self.repository_facts.is_enterprise {
                    return Ok(Some(Contradiction {
                        ...
                        explanation: format!(
                            ...
                            claim.value,
                            if self.repository_facts.is_enterprise { "enterprise-grade" } else { "simple" }
                        ),
                    }));
                }
            },
    ...
    fn calculate_severity_score(&self, contradictions: &[Contradiction]) -> f64 {
        ...
        let total_score: f64 = contradictions.iter().map(|c| {
            match c.severity {
    ...
    fn determine_action(&self, contradictions: &[Contradiction], severity_score: f64) -> RecommendedAction {
        let critical_count = contradictions.iter()
...
mod tests {
    ...
    fn test_correct_claims_pass_validation() {
        ...
        let output = "This is the hive-ai project version 2.0.2 with 100 dependencies and 25 modules. \
                      It's an enterprise-grade application with 150 files.";
        
        let result = checker.validate_stage_output(Stage::Generator, output).unwrap();
    ...
    fn test_incorrect_claims_fail_validation() {
        ...
        let output = "This is a minimal project called simple-app version 0.1.0 with no dependencies.";
        
        let result = checker.validate_stage_output(Stage::Validator, output).unwrap();
        
        match result {
            ValidationResult::Failed { contradictions, .. } => {
                assert!(!contradictions.is_empty());
                assert!(contradictions.iter().any(|c| c.claim.claim_type == ClaimType::ProjectName));
            },
    ...
    fn test_claim_extraction() {
        ...
        assert!(claims.iter().any(|c| c.claim_type == ClaimType::ProjectName));
        assert!(claims.iter().any(|c| c.claim_type == ClaimType::Version));
        assert!(claims.iter().any(|c| c.claim_type == ClaimType::DependencyCount));
    }
}