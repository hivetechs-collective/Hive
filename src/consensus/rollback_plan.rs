...
pub enum RollbackAction {
    ...
    RestoreFromBackup { path: PathBuf, backup_path: PathBuf },
...
impl RollbackPlanGenerator {
    ...
    pub async fn generate_plan(
        ...
        for (idx, (op_meta, result)) in operations
            .iter()
    ...
    async fn create_rollback_operation(
        ...
        let rollback_action = match &op_meta.operation {
            FileOperation::Create { path, .. } => {
                RollbackAction::DeleteCreatedFile { path: path.clone() }
            }
            ...
            FileOperation::Rename { from, to } => RollbackAction::UndoRename {
    ...
    fn generate_checkpoints(&self, operations: &[RollbackOperation]) -> Vec<RollbackCheckpoint> {
        ...
        for (idx, chunk) in operations
            .chunks(self.config.checkpoint_interval)
            .enumerate()
        {
            let operations_completed: Vec<usize> = operations
                [..idx * self.config.checkpoint_interval]
                .iter()
    ...
    fn assess_rollback_risks(
        ...
        let has_complex_deps = operations.iter().any(|op| op.depends_on.len() > 2);
        if has_complex_deps {
            risk_factors.push(RiskFactor {
...
impl RollbackExecutor {
    ...
    async fn validate_operation(&self, operation: &RollbackOperation) -> bool {
        for step in &operation.validation_steps {
            if !self
                .perform_validation_step(step, &operation.rollback_action)
                .await
            {
                if step.critical {
                    return false;
    ...
    async fn execute_rollback_operation(&self, operation: &RollbackOperation) -> Result<()> {
        match &operation.rollback_action {
            RollbackAction::DeleteCreatedFile { path } => {
                std::fs::remove_file(path).map_err(|e| anyhow!("Failed to delete file: {}", e))?;
            }
...
mod tests {
    ...
    fn test_rollback_priority() {
        let generator =
            RollbackPlanGenerator::new(RollbackConfig::default(), PathBuf::from("/tmp/backups"));
    ...
    fn test_circular_dependency_detection() {
        ...
        let mut operations = vec![
            RollbackOperation {
                ...
                rollback_action: RollbackAction::NoOp {
