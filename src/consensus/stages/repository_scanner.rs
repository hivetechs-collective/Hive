...
impl RepositoryScanner {
    ...
    fn is_important_dotfile(name: &str) -> bool {
        matches!(
            name,
            ".gitignore" | ".env" | ".env.example" | ".dockerignore" | ".editorconfig"
        )
    }
    ...
    fn determine_file_priority(path: &Path, root_path: &Path) -> FilePriority {
        ...
        if path_str.ends_with("main.rs")
            ...
            || path_str.ends_with("app.js")
            || path_str.ends_with("app.py")
        {
        ...
        // Test files
        if path_str.contains("test") || path_str.contains("spec") || path_str.starts_with("tests/")
        {
    ...
    pub fn add_repository_summary(context: &mut String, files: &[FileInfo]) {
        ...
        // Count files by type
        let mut by_language: std::collections::HashMap<String, u32> =
        ...
        context.push_str(&format!(
            ...
            files
                .iter()
                .filter(|f| f.priority == FilePriority::Critical)
                .count()
        ));
        context.push_str(&format!(
            "**High priority files:** {}\n\n",
            files
                .iter()
                .filter(|f| f.priority == FilePriority::High)
                .count()
        ));
    }
}
