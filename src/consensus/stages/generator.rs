...
impl ConsensusStage for GeneratorStage {
    ...
    fn build_messages(
        ...
        let mut messages = vec![Message {
            ...
            content: self.enhance_system_prompt_with_context(question).to_string(),
        }];
...
impl GeneratorStage {
    ...
    pub fn structure_context(&self, context: &str, question: &str) -> String {
        ...
        if self.needs_temporal_context(question) {
            structured.push_str("- Use current date information to prioritize recent developments\n");
            structured.push_str("- Include temporal awareness in web searches and information retrieval\n");
        }
    ...
    pub fn needs_temporal_context(&self, question: &str) -> bool {
        let temporal_indicators = [
            ...
            "search", "lookup", "find online", "web search", "news", "trends",
            "latest release", "just released", "recently published",
            "stock price", "market", "cryptocurrency", "bitcoin",
        ];
