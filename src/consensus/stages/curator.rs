...
impl ConsensusStage for CuratorStage {
    ...
    fn build_messages(
        ...
        let mut messages = vec![Message {
            ...
            content: self.build_curation_system_prompt(question, validated_response).to_string(),
        }];
...
impl CuratorStage {
    ...
    pub fn analyze_curation_opportunities(&self, response: &str) -> String {
        ...
        if response.len() > 2000 && !response.contains("Summary") && !response.contains("## Key Points") {
        ...
        if !response.contains("you can") && !response.contains("next steps") && !response.contains("recommended") {
            opportunities.push("🎯 Consider adding actionable recommendations");
        }
        
        // Check for examples
        if response.len() > 800 && !response.contains("example") && !response.contains("for instance") {
            opportunities.push("💡 Consider adding concrete examples");
        }
        
        // Check flow and transitions
        let paragraph_count = response.split("\n\n").count();
        if paragraph_count > 5 && !response.contains("Additionally") && !response.contains("Furthermore") {
    ...
    fn analyze_content_type(&self, response: &str) -> &'static str {
        if response.contains("```") || response.contains("function") || response.contains("class ") {
            "code"
        } else if response.contains("explanation") || response.contains("understand") || response.contains("concept") {
            "explanation"
        } else if response.contains("analysis") || response.contains("findings") || response.contains("conclusion") {
            "analysis"
        } else {
            "general"
        }
