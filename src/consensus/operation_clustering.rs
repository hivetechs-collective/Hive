...
impl OperationClusterer {
    ...
    pub async fn cluster_operations(
        ...
        if self.config.optimize_for_safety {
            self.optimize_for_safety(&mut clusters, &execution_sequence)
                .await?;
        }
    ...
    fn calculate_path_similarity(&self, path1: &Path, path2: &Path) -> f32 {
        ...
        let common_prefix_len = components1
            .iter()
    ...
    async fn identify_pattern_type(&self, operations: &[FileOperation]) -> Result<ClusterType> {
        ...
        for op in operations {
            ...
            if path_str.contains("config")
                ...
                || path
                    .extension()
                    .map_or(false, |ext| ext == "toml" || ext == "yaml" || ext == "json")
            {
                has_config = true;
            }
            ...
            if path
                .extension()
                .map_or(false, |ext| ext == "md" || ext == "rst")
            {
                has_docs = true;
            }
        }
    ...
    async fn infer_cluster_type_from_patterns(
        &self,
        operations: &[FileOperation],
    ) -> Result<ClusterType> {
    ...
    fn has_dependency(&self, cluster1: &OperationCluster, cluster2: &OperationCluster) -> bool {
        ...
        if matches!(cluster1.cluster_type, ClusterType::TestCreation)
            && matches!(
                cluster2.cluster_type,
                ClusterType::FeatureAddition | ClusterType::BugFix
            )
        {
            // Check if test files reference implementation files
            if let (Some(test_path), Some(impl_path)) =
                (&cluster1.common_path, &cluster2.common_path)
            {
    ...
    fn determine_execution_sequence(
        ...
        match toposort(graph, None) {
            ...
            Err(_) => {
                ...
                sorted_clusters.sort_by(|a, b| a.risk_level.partial_cmp(&b.risk_level).unwrap());
    ...
    async fn optimize_for_safety(
        ...
        for cluster in clusters.iter_mut() {
            if cluster.risk_level > 70.0 && cluster.operations.len() > 3 {
                info!(
                    "High-risk cluster {} will be executed with extra safety checks",
                    cluster.id
                );
        ...
        let sequence_map: HashMap<_, _> = execution_sequence
            .iter()
            .enumerate()
            .map(|(i, id)| (id.clone(), i))
            .collect();
        ...
        for cluster in clusters.iter() {
            if matches!(cluster.cluster_type, ClusterType::Deletion) {
                warn!(
                    "Deletion cluster {} requires backup before execution",
                    cluster.id
                );
            }
            if matches!(cluster.cluster_type, ClusterType::Migration) {
                warn!(
                    "Migration cluster {} may require database backup",
                    cluster.id
                );
            }
        }
