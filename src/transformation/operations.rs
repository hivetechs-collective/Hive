...
impl OperationsManager {
    ...
    pub fn change_to_operations(&self, change: &CodeChange) -> Result<Vec<Operation>> {
        ...
        let insert_content = new_lines[(start_line-1)..(start_line-1 + (end_line - start_line + 1)).min(new_lines.len())]
            .join("\n");
    ...
    pub fn apply_operation(&self, content: &str, operation: &Operation) -> Result<String> {
        ...
        match operation {
            ...
            Operation::Replace { position, length, content } => {
                if *position + *length > chars.len() {
    ...
    pub fn transform_operation(&self, op_a: &Operation, op_b: &Operation) -> Result<Operation> {
        ...
        match (op_a, op_b) {
            // Insert vs Insert
            (Insert { position: pos_a, content: content_a }, 
             Insert { position: pos_b, content: content_b }) => {
                if pos_a < pos_b {
                    Ok(op_a.clone())
                } else if pos_a > pos_b {
                    Ok(Insert {
            ...
            // Insert vs Delete
            (Insert { position: pos_a, content }, 
             Delete { position: pos_b, length }) => {
                if *pos_a <= *pos_b {
                    Ok(op_a.clone())
            ...
            // Delete vs Insert
            (Delete { position: pos_a, length: len_a }, 
             Insert { position: pos_b, content }) => {
            ...
            // Delete vs Delete
            (Delete { position: pos_a, length: len_a }, 
             Delete { position: pos_b, length: len_b }) => {
                if *pos_a + *len_a <= *pos_b {
                    Ok(op_a.clone())
            ...
            // Replace operations
            (Replace { position: pos_a, length: len_a, content: content_a },
             Replace { position: pos_b, length: len_b, content: content_b }) => {
            ...
            _ => {
                ...
                Err(anyhow!("Complex operation transformation not yet implemented"))
            }
        }
    }
    ...
    pub fn operations_conflict(&self, op_a: &Operation, op_b: &Operation) -> bool {
        ...
        match (op_a, op_b) {
            (Insert { position: pos_a, .. }, Insert { position: pos_b, .. }) => {
                pos_a == pos_b
            }
            ...
            (Replace { position: pos_a, length: len_a, .. },
             Replace { position: pos_b, length: len_b, .. }) => {
                let range_a = *pos_a..(*pos_a + *len_a);
                let range_b = *pos_b..(*pos_b + *len_b);
                range_a.start < range_b.end && range_b.start < range_a.end
            }
