...
})        return Err(HiveError::Migration { message: })
})            format!("TypeScript installation not found at: {}", path.display())})
})        ));})
})    }})
})})
...
})            return analyze_package_version(&path.join("node_modules/@hivetechs/hive-ai")).await;})
})        }})
})        return Err(HiveError::Migration { message: "package.json not found".to_string()));})
})    }})
})})
...
})        Err(HiveError::Migration { message: "Version not found in package.json".to_string()))})
})    }})
})}})
})})
...
})    Err(HiveError::Migration { message: "No database found".to_string()))})
})}})
})})
...
})    config_compat = config_compat.max(0.0);})
})    })
