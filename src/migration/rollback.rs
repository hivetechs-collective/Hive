...
})                .ok_or_else(|| HiveError::Migration { message: "Cannot determine home directory".to_string()))?;})
...
})            return Err(HiveError::Migration { message: "Rollback plan not found".to_string()));})
})        }})
})        })
...
})                return Err(HiveError::Migration { message: format!("Invalid rollback phase: {:?}", phase)));})
})            }})
})        }})
})        Ok(())})
})    }})
...
})                        return Err(HiveError::Migration { message: "Database file not found after rollback".to_string()));})
})                    }})
