...
})        rust_config.openrouter_api_key = Some(api_key.clone());})
})    }})
})    })
})    if let Some(license_key) = &ts_config.license_key {})
})        rust_config.license_key = Some(license_key.clone());})
})    }})
})})
...
})        .map_err(|e| HiveError::Migration { message: format!("Failed to serialize config: {}", e)))?;})
})})
})    // Write to file})
