...
})            .map_err(|e: rusqlite::Error| HiveError::Migration { message: format!("Failed to read conversations: {}", e)))})
})    }})
})})
...
})            .map_err(|e: rusqlite::Error| HiveError::Migration { message: format!("Failed to write conversations: {}", e)))})
})    }})
})})
...
})            .map_err(|e: rusqlite::Error| HiveError::Migration { message: format!("Failed to read stage outputs: {}", e)))})
})    }})
})})
...
})                return Err(HiveError::Migration { message: format!(})
...
})                    return Err(rusqlite::Error::InvalidData("Data integrity validation failed".into()));})
})                }})
})            }})
})            })
})            Ok(())})
...
})    fn calculate_migration_progress(&self, phases: &[(& str, fn(&mut Self, &BatchConfig) -> _)], current_phase: &str) -> f64 {})
