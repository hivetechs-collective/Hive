...
impl ProductionDatabase {
    ...
    pub async fn new(source_path: &Path, target_path: &Path) -> Result<Self, HiveError> {
        ...
        let source_conn = task::spawn_blocking({
            ...
            move || {
                Connection::open(&source_path).map_err(|e| HiveError::Migration {
        ...
        let target_conn = task::spawn_blocking({
            ...
            move || {
                let conn = Connection::open(&target_path).map_err(|e| HiveError::Migration {
    ...
    fn initialize_target_schema_sync(conn: &Connection) -> Result<(), HiveError> {
        ...
        conn.execute_batch(schema_sql)
            .map_err(|e| HiveError::Migration {
                message: format!("Failed to initialize target schema: {}", e),
            })?;
    ...
    pub async fn migrate_complete_database(
        &mut self,
        config: BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        log::info!(
            "Starting complete database migration with batch size {}",
            config.batch_size
        );
        ...
        for phase_name in phases.iter() {
            ...
            log::info!(
                "Phase '{}' completed in {}ms, {} rows migrated",
                phase_name,
                phase_duration,
                phase_stats.total_rows_migrated
            );

            self.migration_stats.total_rows_migrated += phase_stats.total_rows_migrated;
            self.migration_stats
                .performance_metrics
                .total_processing_time_ms += phase_duration;
    ...
    async fn migrate_conversations_batch(
        &mut self,
        config: &BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        ...
        while offset < total_count {
            ...
            let conversations = self
                .read_conversations_batch(offset, config.batch_size)
                .await?;
            ...
            log::debug!(
                "Migrated conversation batch: {} rows in {}ms",
                rows_in_batch,
                batch_duration
            );
        }
        ...
        batch_stats
            .performance_metrics
            .average_row_processing_time_ms =
            batch_stats.performance_metrics.total_processing_time_ms as f64
                / batch_stats.total_rows_migrated.max(1) as f64;

        Ok(batch_stats)
    }

    /// Read conversations batch from source database
    async fn read_conversations_batch(
        &self,
        offset: u64,
        limit: u32,
    ) -> Result<Vec<TypeScriptConversation>, HiveError> {
        ...
        task::spawn_blocking(move || -> Result<Vec<TypeScriptConversation>, HiveError> {
            ...
            let conversations = stmt
                ...
                .map_err(|e| HiveError::Migration {
                    message: format!("Query map failed: {}", e),
                })?
    ...
    async fn write_conversations_batch(
        &self,
        conversations: &[TypeScriptConversation],
    ) -> Result<(), HiveError> {
        ...
        .map_err(|e| HiveError::Migration {
            message: format!("Failed to write conversations: {}", e),
        })
    }

    /// Migrate stage outputs with enhanced performance
    async fn migrate_stage_outputs_batch(
        &mut self,
        config: &BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        let mut batch_stats = MigrationStats::default();
        ...
        while offset < total_count {
            let stage_outputs = self
                .read_stage_outputs_batch(offset, config.batch_size)
                .await?;
    ...
    async fn read_stage_outputs_batch(
        &self,
        offset: u64,
        limit: u32,
    ) -> Result<Vec<TypeScriptStageOutput>, HiveError> {
        let source_conn = self.source_connection.clone();

        task::spawn_blocking(
            move || -> Result<Vec<TypeScriptStageOutput>, rusqlite::Error> {
                let conn = source_conn.lock().unwrap();
        ...
        .map_err(|e| HiveError::Migration {
            message: format!("Failed to read stage outputs: {}", e),
        })
    }

    /// Write stage outputs batch to target
    async fn write_stage_outputs_batch(
        &self,
        stage_outputs: &[TypeScriptStageOutput],
    ) -> Result<(), HiveError> {
        let target_conn = self.target_connection.clone();
        ...
        .await?
        .map_err(|e: rusqlite::Error| HiveError::Migration {
            message: format!("Failed to write stage outputs: {}", e),
        })
    }

    /// Migrate knowledge base entries
    async fn migrate_knowledge_base_batch(
        &mut self,
        config: &BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        let mut batch_stats = MigrationStats::default();
        ...
        while offset < total_count {
            let knowledge_entries = self
                .read_knowledge_base_batch(offset, config.batch_size)
                .await?;
    ...
    async fn read_knowledge_base_batch(
        &self,
        offset: u64,
        limit: u32,
    ) -> Result<Vec<TypeScriptKnowledgeEntry>, HiveError> {
        let source_conn = self.source_connection.clone();
        ...
        .await?
        .map_err(|e: rusqlite::Error| HiveError::Migration {
            message: format!("Failed to read knowledge base: {}", e),
        })
    }

    /// Write knowledge base batch to target
    async fn write_knowledge_base_batch(
        &self,
        knowledge_entries: &[TypeScriptKnowledgeEntry],
    ) -> Result<(), HiveError> {
        let target_conn = self.target_connection.clone();
        ...
        .await?
        .map_err(|e: rusqlite::Error| HiveError::Migration {
            message: format!("Failed to write knowledge base: {}", e),
        })
    }
    ...
    async fn get_source_table_count(&self, table_name: &str) -> Result<u64, HiveError> {
        ...
        .await?
        .map_err(|e: rusqlite::Error| HiveError::Migration {
            message: format!("Failed to get table count: {}", e),
        })
    }
    ...
    async fn get_target_table_count(&self, table_name: &str) -> Result<u64, HiveError> {
        ...
        .await?
        .map_err(|e: rusqlite::Error| HiveError::Migration {
            message: format!("Failed to get target table count: {}", e),
        })
    }
    ...
    async fn validate_data_sampling(&self) -> Result<(), HiveError> {
        ...
        task::spawn_blocking(move || {
            ...
            let mut stmt =
                source.prepare("SELECT id FROM conversations ORDER BY RANDOM() LIMIT ?")?;

            let conversation_ids: Vec<String> = stmt
                .query_map([sample_size], |row| Ok(row.get::<_, String>("id")?))?
                .collect::<SqliteResult<Vec<_>>>()?;
            ...
            for conv_id in conversation_ids {
                ...
                if source_conv != target_conv {
                    return Err(rusqlite::Error::InvalidPath(
                        "Data integrity validation failed".into(),
                    ));
                }
            }
        ...
        .await?
        .map_err(|e: rusqlite::Error| HiveError::Migration {
            message: format!("Data sampling validation failed: {}", e),
        })
    }
    ...
    async fn calculate_final_metrics(&mut self) -> Result<(), HiveError> {
        ...
        if total_rows > 0 {
            self.migration_stats
                .performance_metrics
                .batch_processing_rate_rows_per_sec =
    ...
    fn calculate_migration_progress(&self, phases: &[&str], current_phase: &str) -> f64 {
        let phase_index = phases
            .iter()
            .position(|&name| name == current_phase)
            .unwrap_or(0);
    ...
    async fn migrate_keywords_batch(
        &mut self,
        _config: &BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        Ok(MigrationStats::default())
    ...
    async fn migrate_context_batch(
        &mut self,
        _config: &BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        Ok(MigrationStats::default())
    ...
    async fn migrate_confidence_batch(
        &mut self,
        _config: &BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        Ok(MigrationStats::default())
    ...
    async fn migrate_metrics_batch(
        &mut self,
        _config: &BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        Ok(MigrationStats::default())
    ...
    async fn migrate_curator_truths_batch(
        &mut self,
        _config: &BatchConfig,
    ) -> Result<MigrationStats, HiveError> {
        Ok(MigrationStats::default())
