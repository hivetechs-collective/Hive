...
use crate::cli::accessibility::{
    AccessibilityManager, AccessibilityRegion, AnnouncementPriority, ScreenReaderAnnouncer,
};
use crate::cli::tui_capabilities::{LayoutConstraints, TuiCapabilities, TuiMode};
...
impl InteractiveTui {
    pub async fn new() -> Result<Self> {
        ...
        let default_theme =
            if capabilities.color_depth == crate::cli::tui_capabilities::ColorDepth::Monochrome {
                "high_contrast"
                    ...
                    if key.kind == KeyEventKind::Press {
                        match key.code {
                            ...
                            KeyCode::PageDown => {
                                ...
                                self.scroll_offset = (self.scroll_offset + 5)
                                    .min(self.messages.len().saturating_sub(10));
                            }
    ...
    fn draw_consensus_progress_static(
        f: &mut Frame,
        area: Rect,
        progress: &Option<ConsensusProgress>,
    ) {
    ...
    fn draw_status_line_static(f: &mut Frame, area: Rect, status_line: &StatusLine) {
        let status_text = format!(
            ...
            if status_line.auto_accept_edits {
                "ON"
            } else {
                "OFF"
            }
    ...
    fn draw_input_box(&self, f: &mut Frame, area: Rect) {
        ...
        let input_style = if self.input_buffer.is_empty() {
            Style::default().fg(self
                .theme_manager
                .to_ratatui_color(&theme.colors.status_text))
        } else {
            self.theme_manager
                .to_ratatui_style(&theme.styles.input, &theme.colors.user_input)
        };
        ...
        let paragraph = Paragraph::new(format!("> {}", input_text))
            ...
            .block(
                Block::default()
                    ...
                    .border_style(
                        Style::default()
                            .fg(self.theme_manager.to_ratatui_color(&theme.colors.border)),
                    )
                    .title(" HiveTechs Consensus ")
                    .title_style(
                        self.theme_manager
                            .to_ratatui_style(&theme.styles.title, &theme.colors.info),
                    ),
            )
            .wrap(Wrap { trim: true });
    ...
    fn draw_consensus_progress(&self, f: &mut Frame, area: Rect) {
        if let Some(ref progress) = self.consensus_progress {
            ...
            let block = Block::default()
                .borders(Borders::ALL)
                .border_style(
                    Style::default().fg(self.theme_manager.to_ratatui_color(&theme.colors.border)),
                )
                .title(" 🧠 Consensus Pipeline ")
                .title_style(
                    self.theme_manager
                        .to_ratatui_style(&theme.styles.title, &theme.colors.consensus_progress),
                );
    ...
    fn draw_stage_progress(&self, f: &mut Frame, area: Rect, name: &str, stage: &StageProgress) {
        ...
        let name_style = match stage.status {
            StageStatus::Waiting => Style::default().fg(self
                .theme_manager
                .to_ratatui_color(&theme.colors.progress_waiting)),
            StageStatus::Running => Style::default().fg(self
                .theme_manager
                .to_ratatui_color(&theme.colors.progress_active)),
            StageStatus::Completed => Style::default().fg(self
                .theme_manager
                .to_ratatui_color(&theme.colors.progress_complete)),
            StageStatus::Error => Style::default().fg(self
                .theme_manager
                .to_ratatui_color(&theme.colors.progress_error)),
        };

        f.render_widget(Paragraph::new(name).style(name_style), layout[0]);
        ...
        let progress_color = match stage.status {
            ...
            StageStatus::Completed => self
                .theme_manager
                .to_ratatui_color(&theme.colors.progress_complete),
            StageStatus::Error => self
                .theme_manager
                .to_ratatui_color(&theme.colors.progress_error),
        };

        let filled = (stage.progress as usize * layout[2].width as usize / 100)
            .min(layout[2].width as usize);
        ...
        f.render_widget(
            Paragraph::new(format!("({})", stage.model)).style(
                Style::default().fg(self.theme_manager.to_ratatui_color(&theme.colors.info)),
            ),
            layout[3],
        );
    }
    ...
    fn draw_status_line(&self, f: &mut Frame, area: Rect) {
        ...
        let status_text = if self.accessibility.is_screen_reader_mode() {
            ...
            format!(
                ...
                auto_accept_status, context_info, focus_info
            )
        };
        ...
        let paragraph = Paragraph::new(status_text)
            .style(
                self.theme_manager
                    .to_ratatui_style(&theme.styles.status_line, &theme.colors.status_text),
            )
            .block(Block::default().borders(Borders::NONE))
    ...
    async fn process_input(&mut self) -> Result<()> {
        ...
        if let Some(processor) = &self.command_processor {
            match processor.process_command(&input).await {
                ...
                Err(e) => {
                    self.add_message(&format!("❌ Command failed: {}", e), MessageType::Error)
                        .await;
                }
            }
        } else {
    ...
    async fn handle_tui_event(&mut self, event: TuiEvent) -> Result<()> {
        match event {
            ...
            TuiEvent::Error(error) => {
                self.add_message(&format!("❌ {}", error), MessageType::Error)
                    .await;
            }
    ...
    async fn fallback_command_handling(&mut self, input: &str) -> Result<()> {
        match input.trim() {
            ...
            "/exit" | "exit" | "quit" => {
                self.add_message(
                    "👋 Thanks for using HiveTechs Consensus!",
    ...
    async fn process_ask_command(&mut self, question: &str) -> Result<()> {
        if question.is_empty() {
            self.add_message("❌ Usage: ask <question>", MessageType::Error)
                .await;
            return Ok(());
        }
        ...
        self.add_message("✨ Consensus Response:", MessageType::SystemResponse)
            .await;
        ...
        if needs_temporal {
            ...
            self.add_message(
                &format!("IMPORTANT: {}", temporal_context.search_instruction),
                MessageType::SystemResponse,
            )
            .await;
            self.add_message("", MessageType::SystemResponse).await;
    ...
    async fn process_analyze_command(&mut self, path: &str) -> Result<()> {
        if path.is_empty() {
            self.add_message("❌ Usage: analyze <path>", MessageType::Error)
                .await;
            return Ok(());
        }

        self.add_message(
            &format!("🔍 Analyzing: {}", path),
            MessageType::SystemResponse,
        )
        .await;
        self.add_message("📊 Repository Intelligence:", MessageType::SystemResponse)
            .await;
    ...
    async fn process_plan_command(&mut self, goal: &str) -> Result<()> {
        if goal.is_empty() {
            self.add_message("❌ Usage: plan <goal>", MessageType::Error)
                .await;
            return Ok(());
        }

        self.add_message(
            &format!("📋 Creating development plan for: {}", goal),
            MessageType::SystemResponse,
        )
        .await;
    ...
    async fn cycle_theme(&mut self) {
        ...
        if let Err(e) = self.theme_manager.load_theme(&next_theme) {
            self.add_message(
                &format!("Failed to load theme '{}': {}", next_theme, e),
                MessageType::Error,
            )
            .await;
        } else {
            self.add_message(
                &format!("🎨 Switched to '{}' theme", next_theme),
                MessageType::SystemResponse,
            )
            .await;
        }
