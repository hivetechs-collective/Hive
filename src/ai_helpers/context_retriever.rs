...
impl ContextRetriever {
    ...
    pub async fn get_generator_context(
        ...
        if should_use_repo && self.active_repository.read().await.is_none() {
            tracing::info!("🤖 AI detected repository-related question, auto-discovering repository...");
            if let Some(repo_info) = self.auto_discover_repository().await? {
                tracing::info!("✅ Auto-discovered repository: {} at {}", repo_info.project_type, repo_info.path.display());
        ...
        let base_facts = if should_use_repo {
            ...
            self.retrieve_repository_context(question, context_limit).await?
        } else {
    ...
    async fn retrieve_repository_context(
        ...
        if let Some(ref knowledge_store) = *knowledge_store_guard {
            let repo_facts = knowledge_store.find_similar("repository code project", 3).await?;
        ...
        if context_items.len() < limit {
            let general_facts = self.retrieve_broad_context(question, limit - context_items.len()).await?;
    ...
    pub async fn auto_discover_repository(&self) -> Result<Option<RepositoryInfo>> {
        ...
        if !directory_analysis.is_repository {
            tracing::debug!("❌ AI determined {} is not a repository", current_dir.display());
            return Ok(None);
        }
    ...
    fn parse_directory_analysis(&self, result: &serde_json::Value, entries: &[String]) -> Result<DirectoryAnalysis> {
    ...
    pub async fn should_use_repository_context(
        ...
        if context_decision.should_use_repo {
            if has_open_repository {
                ...
                tracing::info!("🔍 AI detected repository-specific question - attempting auto-discovery");
