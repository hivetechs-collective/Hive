...
use crate::ai_helpers::{IndexedKnowledge, QualityIssue, QualityReport, Severity};
use crate::consensus::operation_intelligence::{
    ConflictType, OperationConflict, OperationContext, QualityImpact,
    QualityMetrics as OperationQualityMetrics, RollbackComplexity,
};
...
impl QualityAnalyzer {
    ...
    pub async fn evaluate_quality(
        ...
        let overall_score = consistency_score * self.config.consistency_weight
        ...
        self.analysis_cache
            .write()
            .await
            .put(indexed.id.clone(), report.clone());
    ...
    async fn analyze_consistency(
        ...
        let analysis_result = self
            .python_service
            .analyze_code(&self.config.analysis_model, raw_output, "consistency")
            .await?;
        ...
        if let Some(source) = indexed.metadata.get("source_question") {
            if !raw_output
                .to_lowercase()
                .contains(&source.as_str().unwrap_or("").to_lowercase())
            {
                score -= 0.05;
            }
        }
    ...
    async fn analyze_accuracy(&self, indexed: &IndexedKnowledge, raw_output: &str) -> Result<f64> {
        // Use AI model to analyze accuracy
        let analysis_result = self
            .python_service
            .analyze_code(&self.config.analysis_model, raw_output, "accuracy")
            .await?;
...
impl QualityAnalyzer {
    ...
    pub async fn assess_operation_risk(
        ...
        let mitigation_suggestions = self
            .generate_mitigation_suggestions(&risk_factors, &conflicts, &rollback_complexity)
            .await?;
    ...
    async fn analyze_quality_impact(
        ...
        match operation {
            ...
            FileOperation::Update { path, content } => {
                ...
                maintainability_impact =
                    (new_quality.maintainability - current_quality.maintainability) * 100.0;
                reliability_impact =
                    (new_quality.code_quality - current_quality.code_quality) * 100.0;
                performance_impact =
                    (new_quality.performance_impact - current_quality.performance_impact) * 100.0;
            }
        ...
        let overall_impact =
            (maintainability_impact + reliability_impact + performance_impact) / 3.0;
    ...
    async fn detect_operation_conflicts(
        ...
        if let Some(path) = target_path {
            ...
            if self
                .detect_race_condition(&tracker, &path, operation)
                .await?
            {
                conflicts.push(OperationConflict {
                    conflicting_operations: vec![0],
                    conflict_type: ConflictType::RaceCondition,
                    description: "Multiple operations targeting the same file concurrently"
                        .to_string(),
    ...
    async fn identify_risk_factors(
        ...
        if matches!(
            operation,
            FileOperation::Delete { .. } | FileOperation::Update { .. }
        ) {
            risk_factors.push(RiskFactor {
        ...
        if self
            .detect_maintainability_risks(operation, context)
            .await?
        {
            risk_factors.push(RiskFactor {
    ...
    async fn calculate_risk_score(
        ...
        for factor in risk_factors {
            let factor_score = factor.probability
                * factor.impact
        ...
        risk_score +=
            rollback_complexity.complexity_score * self.config.rollback_complexity_weight as f32;
        ...
        for factor in risk_factors {
            match factor.risk_type {
                RiskType::CodeQuality => {
                    suggestions
                        .push("Refactor code to reduce complexity before proceeding".to_string());
                }
                RiskType::Security => {
                    suggestions
                        .push("Run security audit and fix vulnerabilities first".to_string());
                }
    ...
    async fn analyze_code_complexity(&self, content: &str) -> Result<f64> {
        ...
        let indentation_levels = content
            .lines()
        ...
        Ok((lines as f64 * 0.1)
            + (indentation_levels as f64 * 2.0)
            + (cyclomatic_complexity as f64 * 3.0))
    }
    ...
    async fn analyze_code_quality(&self, content: &str) -> Result<OperationQualityMetrics> {
        ...
        let analysis_result = self
            .python_service
            .analyze_code(&self.config.analysis_model, content, "quality_metrics")
            .await?;

        // Extract metrics from analysis
        Ok(OperationQualityMetrics {
            code_quality: analysis_result
                .get("code_quality")
                .and_then(|v| v.as_f64())
                .unwrap_or(0.5) as f32,
            test_coverage_change: 0.0, // Would need test analysis
            performance_impact: analysis_result
                .get("performance")
                .and_then(|v| v.as_f64())
                .unwrap_or(0.0) as f32,
            maintainability: analysis_result
                .get("maintainability")
                .and_then(|v| v.as_f64())
                .unwrap_or(0.5) as f32,
        })
