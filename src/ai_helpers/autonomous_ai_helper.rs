...
use crate::ai_helpers::{
    AIHelperEcosystem, ContextRetriever, IntelligentContextOrchestrator, KnowledgeIndexer,
    KnowledgeSynthesizer, PatternRecognizer, QualityAnalyzer,
};
...
impl AutonomousAIHelper {
    ...
    pub async fn process_autonomously(&self, raw_user_input: &str) -> Result<AutonomousDecision> {
        ...
        let actions = self
            .decide_actions(&intent_analysis, raw_user_input)
            .await?;
        ...
        let needs_consensus = self
            .evaluate_consensus_need(&intent_analysis, &gathered_context)
            .await?;

        // Step 5: Check if AI can provide a direct response
        let direct_response = self
            .formulate_direct_response(&intent_analysis, &gathered_context)
            .await?;
    ...
    async fn decide_actions(
        ...
        if semantic_analysis.concepts.iter().any(|c| {
            ...
            if self.is_in_repository_at(&effective_dir).await {
                ...
                let relevant_files = self
                    .identify_relevant_files_at(raw_input, &effective_dir)
                    .await?;
                if !relevant_files.is_empty() {
                    actions.push(AutonomousAction::ReadFiles {
                        files: relevant_files,
                        reason: "AI identified these files as potentially relevant to the query"
                            .to_string(),
                    });
                }
            }
        ...
        if semantic_analysis.requires_code_analysis {
            ...
            let search_query = self
                .formulate_search_query(raw_input, &semantic_analysis)
                .await?;
        ...
        for action in actions {
            match action {
                ...
                AutonomousAction::SynthesizeKnowledge { sources } => {
                    info!(
                        "🎓 AI is synthesizing knowledge from {} sources",
                        sources.len()
                    );
                ...
                AutonomousAction::OpenFile { path } => {
                    ...
                    if path.exists() && path.is_file() {
                        ...
                        use crate::desktop::ai_ui_events::{send_ai_helper_event, AIHelperUIEvent};
                        send_ai_helper_event(AIHelperUIEvent::OpenFile { path: path.clone() });
                    } else {
                        context_parts.push(format!(
                    ...
                    if path.exists() {
                        ...
                        match tokio::fs::write(path, content).await {
                            Ok(_) => {
                                ...
                                use crate::desktop::ai_ui_events::{
                                    send_ai_helper_event, AIHelperUIEvent,
                                };
    ...
    async fn formulate_search_query(
        ...
        let key_concepts: Vec<&str> = analysis
            .concepts
            .iter()
    ...
    async fn analyze_file_relevance(
        &self,
        path: &Path,
        content: &str,
    ) -> Result<FileRelevanceAnalysis> {
    ...
    async fn evaluate_consensus_need(
        ...
        if intent.complexity > 0.5 {
            info!(
                "🎯 Routing to consensus: high complexity ({})",
                intent.complexity
            );
            return Ok(true);
        }
        ...
        if code_keywords
            .iter()
            .any(|&keyword| intent.primary_intent.to_lowercase().contains(keyword))
        {
        ...
        if intent.primary_intent.contains("repository_status")
            || intent.primary_intent.contains("simple_fact")
        {
    ...
    async fn extract_path_from_input(&self, input: &str) -> Result<Option<PathBuf>> {
        ...
        let extensions = vec![
            ".rs", ".ts", ".js", ".py", ".go", ".java", ".cpp", ".c", ".h", ".md", ".txt", ".toml",
            ".json",
        ];
    ...
    async fn infer_navigation_target(&self, input: &str) -> Result<Option<PathBuf>> {
        ...
        if input_lower.contains("main")
            && (input_lower.contains("file") || input_lower.contains("entry"))
        {
        ...
        if input_lower.contains("test")
            && (input_lower.contains("file") || input_lower.contains("folder"))
        {
    ...
    async fn understand_file_creation_request(
        &self,
        input: &str,
...
impl IntelligentContextOrchestrator {
    ...
    pub async fn extract_semantic_indicators(&self, text: &str) -> Result<Vec<SemanticIndicator>> {
        ...
        if text_lower.contains("repo")
            || text_lower.contains("project")
            || text_lower.contains("our")
        {
...
impl ContextRetriever {
    ...
    pub async fn search_codebase_with_ai(
        &self,
        query: &str,
        semantic: bool,
    ) -> Result<Option<String>> {
