...
impl ChromaVectorStore {
    ...
    pub async fn search(
        ...
        if let Some(cached) = self.search_cache.read().await.peek(&cache_key) {
            return Ok(cached.iter()
                .map(|r| (r.id.clone(), r.content.clone(), r.embedding.clone(), r.metadata.clone()))
                .collect());
        }
        ...
        // Cache results
        self.search_cache.write().await.put(cache_key, results.clone());
    ...
    async fn similarity_search(
        ...
        let mut scored_docs: Vec<(f64, &Document)> = client.documents.iter()
            .map(|doc| {
        ...
        // Take top results
        let results: Vec<SearchResult> = scored_docs.into_iter()
...
mod tests {
    ...
    async fn test_vector_operations() {
        ...
        store.add_document("test1", "Test content", &embedding, &metadata).await.unwrap();
