...
use super::{
    IndexedKnowledge, Pattern, QualityReport, Insight,
    KnowledgeIndexer, PatternRecognizer, QualityAnalyzer, KnowledgeSynthesizer,
};
...
impl ParallelProcessor {
    ...
    pub async fn process_parallel(
        ...
        let quality = if let Some(q) = quality_opt {
            ...
            quality_analyzer.evaluate_quality(indexed, curator_output).await?
        };
        ...
        for result in indexed_results {
            match result {
                Ok(Ok((indexed, curator_output))) => {
                    let parallel_result = self.process_parallel(
                        &indexed,
                        &curator_output,
