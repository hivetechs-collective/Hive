...
use crate::consensus::operation_intelligence::{
    OperationContext, OperationOutcome
};
...
impl KnowledgeIndexer {
    ...
    pub async fn index_operation_outcome(
        ...
        let metadata = self.create_operation_metadata(operation, context, outcome, user_satisfaction);
        self.vector_store.add_document(
            &operation_id,
        ...
        debug!("✅ Successfully indexed operation outcome: {}", operation_id);
        Ok(operation_id)
    }
    ...
    pub async fn find_similar_operations(
        ...
        let search_results = self.vector_store.search(&query_embedding, limit * 2).await?;
        ...
        for (id, content, embedding, metadata) in search_results {
            ...
            if let Some(indexed_op) = self.find_operation_by_id(&id).await? {
                ...
                let similarity_reasons = self.analyze_similarity_reasons(operation, context, &indexed_op);
    ...
    pub async fn predict_operation_success(
        ...
        let weighted_success_rate = similar_operations.iter()
            .map(|op| if op.operation.outcome.success { op.similarity } else { 0.0 })
            .sum::<f32>() / similar_operations.iter()
        ...
        let common_failure_modes: Vec<String> = failure_modes.into_iter()
        ...
        let avg_similarity = similar_operations.iter()
            .map(|op| op.similarity)
            .sum::<f32>() / similar_operations.len() as f32;
        
        let confidence = (avg_similarity * (similar_operations.len() as f32 / 20.0).min(1.0)).clamp(0.0, 1.0);
        ...
        info!("📊 Success prediction: {:.2} probability, {:.2} confidence", 
    ...
    pub async fn get_operation_statistics(&self, operation_type: &str) -> Result<OperationStats> {
        ...
        let matching_operations: Vec<_> = history.iter()
        ...
        let successful = matching_operations.iter()
            .filter(|op| op.outcome.success)
            .count();
        
        let avg_execution_time = Duration::from_nanos(
            matching_operations.iter()
                .map(|op| op.outcome.execution_time.as_nanos() as u64)
                .sum::<u64>() / matching_operations.len() as u64
        );
        
        let satisfactions: Vec<f32> = matching_operations.iter()
    ...
    fn create_operation_description(&self, operation: &FileOperation, context: &OperationContext) -> String {
        let operation_text = match operation {
            FileOperation::Create { path, content } => {
                format!("Create file {} with content length {}", path.display(), content.len())
            }
            FileOperation::Update { path, content } => {
                format!("Update file {} with content length {}", path.display(), content.len())
            }
            FileOperation::Append { path, content } => {
                format!("Append to file {} with content length {}", path.display(), content.len())
            }
    ...
    fn analyze_similarity_reasons(
        ...
        let query_words: std::collections::HashSet<String> = query_context.source_question
            .to_lowercase()
            .split_whitespace()
            .map(|s| s.to_string())
            .collect();
        let similar_words: std::collections::HashSet<String> = similar_op.context.source_question
            .to_lowercase()
            .split_whitespace()
            .map(|s| s.to_string())
            .collect();
