...
use crate::consensus::operation_intelligence::{
    DangerousPattern, DangerousPatternType, DangerousSeverity, OperationCluster, 
    ClusterType, AntiPattern, OperationContext
};
...
impl PatternRecognizer {
    ...
    pub async fn analyze_operation_safety(
        ...
        info!("🔍 Analyzing {} operations for safety patterns", operations.len());
        ...
        let operation_clusters = self.analyze_operation_clustering(operations, context).await?;
        
        // Detect anti-patterns
        let anti_patterns = self.detect_anti_patterns(operations, context).await?;
        
        // Calculate overall safety score
        let safety_score = self.calculate_safety_score(&dangerous_patterns, &operation_clusters, &anti_patterns);
        
        // Generate safety recommendations
        let safety_recommendations = self.generate_safety_recommendations(
            &dangerous_patterns, 
            &operation_clusters, 
            &anti_patterns,
            context
        ...
        info!("📊 Safety analysis complete: {:.1}% safety score", analysis.safety_score);
        Ok(analysis)
    }
    ...
    async fn detect_dangerous_patterns(
        ...
        if let Some(security) = self.detect_security_vulnerabilities(operations, context).await? {
            dangerous_patterns.push(security);
        }
        
        debug!("🚨 Detected {} dangerous patterns", dangerous_patterns.len());
        Ok(dangerous_patterns)
    }
    
    /// Detect mass deletion patterns
    async fn detect_mass_deletion(&self, operations: &[FileOperation]) -> Result<Option<DangerousPattern>> {
        ...
        if is_mass_deletion {
            return Ok(Some(DangerousPattern {
                ...
                description: format!("Mass deletion detected: {} files being deleted", delete_count),
                severity: if delete_count > 10 { 
                    DangerousSeverity::Critical 
                } else { 
                    DangerousSeverity::High 
                },
                mitigation: Some("Create backups before deletion, consider soft deletion instead".to_string()),
            }));
        }
    ...
    async fn detect_overwrite_without_backup(&self, operations: &[FileOperation]) -> Result<Option<DangerousPattern>> {
        ...
        if update_count > 3 {
            return Ok(Some(DangerousPattern {
                ...
                description: format!("Multiple file overwrites detected: {} updates without backup", update_count),
                severity: DangerousSeverity::Medium,
                mitigation: Some("Enable automatic backups or create explicit backup operations".to_string()),
            }));
        }
    ...
    async fn detect_breaking_changes(&self, operations: &[FileOperation], context: &OperationContext) -> Result<Option<DangerousPattern>> {
        let breaking_indicators = [
            "breaking", "deprecated", "removed", "deleted", "incompatible",
            "major version", "api change", "interface change"
        ];
        ...
        if has_breaking_change {
            return Ok(Some(DangerousPattern {
                ...
                mitigation: Some("Review API compatibility, add deprecation notices, update documentation".to_string()),
            }));
        }
    ...
    async fn detect_data_loss_patterns(&self, operations: &[FileOperation]) -> Result<Option<DangerousPattern>> {
        ...
        if data_loss_risk {
            return Ok(Some(DangerousPattern {
                ...
                description: "Operations target sensitive files that could result in data loss".to_string(),
                severity: DangerousSeverity::Critical,
                mitigation: Some("Create full backups, verify file importance, use read-only mode first".to_string()),
            }));
        }
    ...
    async fn detect_security_vulnerabilities(&self, operations: &[FileOperation], context: &OperationContext) -> Result<Option<DangerousPattern>> {
        let security_risks = [
        ...
        let has_security_risk = operations.iter().any(|op| {
            ...
            let content_risk = content.map_or(false, |c| 
                security_risks.iter().any(|risk| c.to_lowercase().contains(risk))
            );
    ...
    async fn analyze_operation_clustering(
        ...
        for operation in operations {
            ...
            operation_groups.entry(group_key).or_insert_with(Vec::new).push(operation);
        }
        
        // Create clusters for each significant group
        for (group_type, group_operations) in operation_groups {
            if group_operations.len() >= 2 {
                let cluster_type = self.determine_cluster_type(&group_type, &group_operations, context);
    ...
    fn classify_operation_group(&self, operation: &FileOperation, context: &OperationContext) -> String {
        match operation {
    ...
    fn determine_cluster_type(&self, group_type: &str, operations: &[&FileOperation], context: &OperationContext) -> ClusterType {
        match group_type {
    ...
    fn determine_execution_order(&self, operations: &[&FileOperation]) -> Vec<usize> {
        ...
        for (i, op) in operations.iter().enumerate() {
            if matches!(op, FileOperation::Update { .. } | FileOperation::Append { .. }) {
                ordered_indices.push(i);
            }
        }
    ...
    async fn detect_anti_patterns(&self, operations: &[FileOperation], context: &OperationContext) -> Result<Vec<AntiPattern>> {
    ...
    fn detect_duplicate_operations(&self, operations: &[FileOperation]) -> Option<AntiPattern> {
        ...
        for operation in operations {
            ...
            file_operations.entry(path.clone()).or_insert_with(Vec::new).push(operation);
        }
        
        // Check for files with multiple operations
        let duplicates: Vec<_> = file_operations.iter()
        ...
        if !duplicates.is_empty() {
            return Some(AntiPattern {
                ...
                description: format!("Multiple operations on same files: {} files affected", duplicates.len()),
    ...
    fn detect_inefficient_sequences(&self, operations: &[FileOperation]) -> Option<AntiPattern> {
        ...
        if !deletes.is_empty() {
            return Some(AntiPattern {
                ...
                description: format!("Files created and then deleted in same operation set: {} files", deletes.len()),
                alternative: Some("Remove unnecessary create operations or review workflow".to_string()),
            });
        }
    ...
    fn detect_risky_combinations(&self, operations: &[FileOperation], context: &OperationContext) -> Option<AntiPattern> {
        let has_deletes = operations.iter().any(|op| matches!(op, FileOperation::Delete { .. }));
        let has_creates = operations.iter().any(|op| matches!(op, FileOperation::Create { .. }));
        let has_renames = operations.iter().any(|op| matches!(op, FileOperation::Rename { .. }));
        ...
        if has_deletes && (has_creates || has_renames) {
            return Some(AntiPattern {
                ...
                description: "Combining deletions with creates/renames increases error risk".to_string(),
                alternative: Some("Separate destructive operations from constructive ones".to_string()),
            });
        }
    ...
    fn detect_poor_separation(&self, operations: &[FileOperation], context: &OperationContext) -> Option<AntiPattern> {
        ...
        if concerns.len() > 2 {
            return Some(AntiPattern {
                ...
                description: format!("Operations span {} different concerns in single batch", concerns.len()),
                alternative: Some("Separate operations by concern (tests, docs, config, code)".to_string()),
            });
        }
    ...
    async fn generate_safety_recommendations(
        ...
        if dangerous_patterns.len() > 2 {
            recommendations.push("💡 Consider breaking this operation into smaller, safer batches".to_string());
        }
        
        if operation_clusters.iter().any(|c| matches!(c.cluster_type, ClusterType::Migration)) {
            recommendations.push("🔄 Test migration operations in a safe environment first".to_string());
        }
        
        // Repository-specific recommendations
        if context.related_files.len() > 10 {
            recommendations.push("📁 Large number of related files - ensure proper validation".to_string());
        }
    ...
    async fn record_safety_analysis(&self, analysis: &SafetyPatternAnalysis, operations: &[FileOperation]) -> Result<()> {
    ...
    pub async fn get_repository_safety_metrics(&self, context: &OperationContext) -> Result<RepositorySafetyMetrics> {
        ...
        // Calculate metrics
        let risky_operations = history.iter()
        ...
        let improvements = self.suggest_safety_improvements(&history, risky_operations).await?;
    ...
    fn calculate_safety_trend(&self, history: &[OperationSafetyRecord]) -> SafetyTrend {
        ...
        let first_avg = first_half.iter().map(|r| r.safety_score).sum::<f32>() / first_half.len() as f32;
        let second_avg = second_half.iter().map(|r| r.safety_score).sum::<f32>() / second_half.len() as f32;
        
        let improvement_rate = (second_avg - first_avg) / first_avg;
        
        if improvement_rate > 0.05 {
            SafetyTrend::Improving { rate: improvement_rate }
        } else if improvement_rate < -0.05 {
            SafetyTrend::Declining { rate: improvement_rate.abs() }
        } else {
            SafetyTrend::Stable
        }
    }
    ...
    async fn analyze_common_danger_patterns(&self, history: &[OperationSafetyRecord]) -> Result<Vec<DangerousPatternType>> {
        ...
        for record in history {
            if record.dangerous_patterns_count > 0 {
                ...
                if record.operation_type == "delete" && record.safety_score < 50.0 {
                    *pattern_counts.entry(DangerousPatternType::MassDeletion).or_insert(0) += 1;
                } else if record.operation_type == "update" && record.safety_score < 60.0 {
                    *pattern_counts.entry(DangerousPatternType::OverwriteWithoutBackup).or_insert(0) += 1;
                }
            }
        }
        
        let mut common_patterns: Vec<_> = pattern_counts.into_iter()
    ...
    async fn suggest_safety_improvements(&self, history: &[OperationSafetyRecord], risky_count: usize) -> Result<Vec<String>> {
        ...
        if history.iter().any(|r| r.operation_type == "delete" && r.safety_score < 40.0) {
            improvements.push("🗑️ Implement soft deletion for critical operations".to_string());
        }
        
        if history.len() > 50 {
            improvements.push("📊 Regular safety audit recommended due to high operation volume".to_string());
        }
    ...
    fn generate_safety_cache_key(&self, operations: &[FileOperation], context: &OperationContext) -> String {
    ...
    fn is_config_file(&self, path: &PathBuf) -> bool {
        let path_str = path.to_string_lossy().to_lowercase();
        path_str.contains("config") || path_str.ends_with(".toml") || path_str.ends_with(".yml") ||
        path_str.ends_with(".yaml") || path_str.ends_with(".json") || path_str.contains(".env")
    }
    
    /// Check if file is documentation
    fn is_documentation_file(&self, path: &PathBuf) -> bool {
        let path_str = path.to_string_lossy().to_lowercase();
        path_str.ends_with(".md") || path_str.ends_with(".txt") || path_str.contains("readme") ||
        path_str.contains("doc") || path_str.contains("manual")
    }
    
    /// Check if file is source code
    fn is_source_file(&self, path: &PathBuf) -> bool {
        let path_str = path.to_string_lossy().to_lowercase();
        path_str.ends_with(".rs") || path_str.ends_with(".js") || path_str.ends_with(".ts") ||
        path_str.ends_with(".py") || path_str.ends_with(".java") || path_str.ends_with(".cpp") ||
        path_str.ends_with(".c") || path_str.ends_with(".go")
    }
    
    /// Check if file is temporary
    fn is_temporary_file(&self, path: &PathBuf) -> bool {
        let path_str = path.to_string_lossy().to_lowercase();
        path_str.starts_with("/tmp/") || path_str.contains("temp") || path_str.ends_with(".tmp") ||
        path_str.ends_with(".bak") || path_str.ends_with("~")
    }
