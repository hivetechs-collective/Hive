...
impl SemanticIndex {
    ...
    pub async fn index_file(&self, path: &Path) -> Result<()> {
        ...
        // Store symbols
        {
            ...
            symbols
                .by_file
                .insert(path.to_path_buf(), parse_result.symbols.clone());
    ...
    pub async fn search(&self, query: SearchQuery) -> Result<Vec<SearchResult>> {
        ...
        for (name, locations) in &symbols.global_index {
            ...
            if let Some(kinds) = &query.kinds {
                let has_matching_kind =
                    locations.iter().any(|loc| kinds.contains(&loc.symbol.kind));
    ...
    async fn matches_file_patterns(&self, file: &Path, query: &SearchQuery) -> bool {
        ...
        if let Some(includes) = &query.include_patterns {
            let matches_include = includes.iter().any(|pattern| file_str.contains(pattern));
            if !matches_include {
                return false;
            }
        }
        ...
        if let Some(excludes) = &query.exclude_patterns {
            let matches_exclude = excludes.iter().any(|pattern| file_str.contains(pattern));
            if matches_exclude {
                return false;
            }
        }
