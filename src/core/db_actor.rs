...
impl DatabaseActor {
    ...
    async fn run(&mut self) {
        ...
        while let Some(cmd) = self.receiver.recv().await {
            match cmd {
                ...
                DbCommand::FindSimilarFacts { content, limit, respond_to } => {
                    let result = self.handle_find_similar_facts(&content, limit);
                    let _ = respond_to.send(result);
                }
                DbCommand::CheckFactExists { fingerprint, respond_to } => {
                    let result = self.handle_check_fact_exists(&fingerprint);
                    let _ = respond_to.send(result);
                }
                DbCommand::RecordFactAccess { fact_id, respond_to } => {
                ...
                DbCommand::SetActiveProfile { profile_id, respond_to } => {
                ...
                DbCommand::StoreConversation { id, user_id, profile_id, respond_to } => {
                    let result = self.handle_store_conversation(&id, user_id.as_deref(), profile_id.as_deref());
                    let _ = respond_to.send(result);
                }
                DbCommand::UpdateConversationCost { conversation_id, cost, tokens_in, tokens_out, respond_to } => {
                    let result = self.handle_update_conversation_cost(&conversation_id, cost, tokens_in, tokens_out);
                    let _ = respond_to.send(result);
                }
                ...
                DbCommand::StoreStageUsage { conversation_id, stage_name, model_id, prompt_tokens, completion_tokens, total_tokens, cost, duration_ms, respond_to } => {
                    let result = self.handle_store_stage_usage(&conversation_id, &stage_name, &model_id, prompt_tokens, completion_tokens, total_tokens, cost, duration_ms);
                    let _ = respond_to.send(result);
                }
    ...
    fn handle_store_fact(&self, fact: CuratedFact) -> Result<()> {
        ...
        if count > 0 {
            info!("Fact already exists with fingerprint: {}", fact.semantic_fingerprint);
            return Ok(());
        }
    ...
    fn handle_find_similar_facts(&self, _content: &str, limit: usize) -> Result<Vec<CuratedFact>> {
        ...
        let facts = stmt.query_map([limit], |row| {
            Ok(CuratedFact {
                id: row.get(0)?,
                semantic_fingerprint: row.get(1)?,
                content: row.get(2)?,
    ...
    fn handle_store_conversation(&self, id: &str, user_id: Option<&str>, profile_id: Option<&str>) -> Result<()> {
        let conn = self.manager.get_connection()?;
    ...
    fn handle_update_conversation_cost(&self, conversation_id: &str, cost: f64, tokens_in: u32, tokens_out: u32) -> Result<()> {
        let conn = self.manager.get_connection()?;
    ...
    fn handle_store_stage_usage(&self, conversation_id: &str, stage_name: &str, model_id: &str, 
                               prompt_tokens: u32, completion_tokens: u32, total_tokens: u32, 
                               cost: f64, duration_ms: u64) -> Result<()> {
        let conn = self.manager.get_connection()?;
...
impl DatabaseService {
    ...
    pub async fn store_fact(&self, fact: CuratedFact) -> Result<()> {
        ...
        self.sender.send(DbCommand::StoreFact { fact, respond_to: tx }).await?;
        rx.await?
    }
    
    pub async fn load_recent_facts(&self, cutoff: DateTime<Utc>) -> Result<Vec<CuratedFact>> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::LoadRecentFacts { cutoff, respond_to: tx }).await?;
        rx.await?
    }
    
    pub async fn find_similar_facts(&self, content: &str, limit: usize) -> Result<Vec<CuratedFact>> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::FindSimilarFacts { 
    ...
    pub async fn check_fact_exists(&self, fingerprint: &str) -> Result<bool> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::CheckFactExists { 
    ...
    pub async fn record_fact_access(&self, fact_id: &str) -> Result<()> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::RecordFactAccess { 
    ...
    pub async fn get_all_facts_sorted_by_date(&self) -> Result<Vec<CuratedFact>> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::GetAllFactsSortedByDate { respond_to: tx }).await?;
        rx.await?
    }
    
    pub async fn get_active_profile(&self) -> Result<ConsensusProfile> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::GetActiveProfile { respond_to: tx }).await?;
        rx.await?
    }
    
    pub async fn set_active_profile(&self, profile_id: &str) -> Result<()> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::SetActiveProfile { 
    ...
    pub async fn get_profile_by_name(&self, name: &str) -> Result<Option<ConsensusProfile>> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::GetProfileByName { 
            name: name.to_string(), 
            respond_to: tx 
        }).await?;
        rx.await?
    ...
    pub async fn store_conversation(&self, id: &str, user_id: Option<String>, profile_id: Option<String>) -> Result<()> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::StoreConversation { 
            id: id.to_string(),
            user_id,
            profile_id,
            respond_to: tx 
    ...
    pub async fn update_conversation_cost(&self, conversation_id: &str, cost: f64, tokens_in: u32, tokens_out: u32) -> Result<()> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::UpdateConversationCost { 
    ...
    pub async fn health_check(&self) -> Result<()> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::HealthCheck { respond_to: tx }).await?;
        rx.await?
    }
    
    pub async fn get_license_key(&self) -> Result<Option<String>> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::GetLicenseKey { respond_to: tx }).await?;
        rx.await?
    }
    
    pub async fn store_stage_usage(&self, conversation_id: &str, stage_name: &str, model_id: &str,
                                  prompt_tokens: u32, completion_tokens: u32, total_tokens: u32,
                                  cost: f64, duration_ms: u64) -> Result<()> {
        let (tx, rx) = oneshot::channel();
        self.sender.send(DbCommand::StoreStageUsage {
            conversation_id: conversation_id.to_string(),
