...
use crate::core::license::{LicenseInfo, LicenseTier};
...
impl UsageTracker {
            ...
            if let Some(trial_end) = usage_info.trial_end_date {
                if Utc::now() < trial_end {
                    return Ok(UsageCheckResult {
                        allowed: true,
                        reason: format!(
                            "Trial period active - {} days remaining",
                            usage_info.trial_days_remaining
                        ),
    ...
    pub async fn record_conversation_usage(
        ...
        if usage_info.trial_active && usage_info.trial_days_remaining > 0 {
            if let Some(trial_end) = usage_info.trial_end_date {
                if Utc::now() < trial_end {
                    info!(
                        "Skipping usage recording for user {} - in trial period",
                        user_id
                    );
                    return Ok(());
                }
        ...
        if usage_info.is_unlimited {
            info!(
                "Skipping usage recording for user {} - unlimited tier",
                user_id
            );
            return Ok(());
        }
        ...
        info!(
            "Recorded usage for user {} - daily usage now: {}/{}",
    ...
    async fn get_or_create_user_usage(&self, user_id: &str) -> Result<UserUsageInfo> {
        ...
        let (tier_str, daily_limit, daily_usage, usage_reset_str, created_at_str, updated_at_str) =
            if let Some(user_data) = user_result {
                ...
                info!(
                    "Created new user {} with 7-day trial until {}",
                    user_id, trial_end
                );
    ...
    fn calculate_trial_status(
        ...
        if let Some(created) = created_at {
            ...
            let has_configured_license =
                updated_at.is_some() && updated_at != Some(&created.to_rfc3339());
    ...
    pub async fn get_usage_display(&self, user_id: &str) -> Result<UsageDisplay> {
        ...
        let status_message = if usage_info.trial_active {
            ...
            format!(
                "{}/{} today",
                usage_info.daily_usage, usage_info.daily_limit
            )
        };

        // Calculate reset time
        let reset_time = usage_info.usage_reset_date.map(|d| {
