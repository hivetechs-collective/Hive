...
use hive::desktop::workspace::{
    DiscoveryConfig, ProjectType, RepositoryDiscoveryService, ScanningMode,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    for repo in &repositories {
        ...
        println!(
            "      Confidence: {:.1}%",
            repo.discovery.confidence * 100.0
        );
    }
    ...
    let shallow_config = DiscoveryConfig {
        ...
        scan_paths: vec![std::env::current_dir()?],
    ...
    println!(
        "Shallow scan found {} repositories in current directory",
        shallow_repos.len()
    );

    // Example 3: Filter repositories by project type
    println!("\n🦀 Example 3: Filter by Project Type");
    let rust_repos: Vec<_> = repositories
        .iter()
        .filter(|repo| {
            repo.project_info
                .as_ref()
