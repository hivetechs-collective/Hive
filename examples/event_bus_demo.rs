...
use hive::desktop::events::{event_bus, ConsensusStage, Event, EventType, FileChangeType};
...
async fn main() -> anyhow::Result<()> {
    ...
    bus.subscribe(EventType::GitStatusChanged, |event| {
        if let hive::desktop::events::EventPayload::GitStatus {
            branch,
            modified_files,
            ..
        } = &event.payload
    ...
    bus.subscribe(EventType::FileChanged, |event| {
        if let hive::desktop::events::EventPayload::FileChange { path, change_type } =
            &event.payload
        {
    ...
    bus.subscribe_async(EventType::ConsensusStateChanged, |event| async move {
        if let hive::desktop::events::EventPayload::ConsensusState {
            stage, progress, ..
        } = &event.payload
        {
    ...
    bus.subscribe(EventType::BuildCompleted, |event| {
        if let hive::desktop::events::EventPayload::BuildResult {
            success,
            duration_ms,
            ..
        } = &event.payload
    ...
    for (stage, progress) in stages {
        bus.publish_async(Event::consensus_state_changed(stage, progress, None))
            .await?;
