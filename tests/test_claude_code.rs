...
mod tests {
    use crate::consensus::{
        ConsensusConfig, ConsensusEngine, ConsensusProfile, ExecutionMode, ModeDetector,
    };
    ...
    async fn test_mode_detection_for_simple_operations() {
        ...
        let mode = detector
            .detect_mode("analyze the architecture of this codebase and suggest improvements")
            .await;
        assert_eq!(mode, ExecutionMode::Consensus);

        // Test feature implementation
        let mode = detector
            .detect_mode("implement a login feature with authentication")
            .await;
