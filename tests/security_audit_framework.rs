...
use hive_ai::hooks::{security::SecurityHook, execution::HookExecutor, registry::HookRegistry};
...
impl SecurityTestResult {
    ...
    fn new_vulnerability(severity: SecuritySeverity, details: String, recommendations: Vec<String>) -> Self {
        Self {
            passed: false,
...
async fn test_filesystem_access_controls() {
    ...
    tokio::fs::write(&sensitive_file, "sensitive data").await.unwrap();
    ...
    #[cfg(unix)]
    {
        ...
        if std::os::unix::fs::symlink("/etc", &symlink_path).is_ok() {
            let result = security_manager.validate_file_access(symlink_path.to_str().unwrap()).await;
            assert!(result.is_err(), "Should block symlink to system directories");
        }
    }
...
async fn test_trust_system_security() {
    ...
    match trust_result {
        Ok(_) => {
            ...
            assert!(is_trusted.unwrap_or(false), "Trust decision should be persisted");
        }
        Err(_) => {
            // If trust was denied, directory should not be trusted
            let is_trusted = trust_dialog.is_directory_trusted(project_path).await;
            assert!(!is_trusted.unwrap_or(true), "Denied trust should be remembered");
        }
    }
    ...
    if trust_dialog.is_directory_trusted(project_path).await.unwrap_or(false) {
        assert!(!is_parent_trusted.unwrap_or(true), 
...
"#;
    ...
    tokio::fs::write(&hook_file, malicious_hook_config).await.unwrap();
    
    let hook_registry = HookRegistry::new();
    let load_result = hook_registry.load_hook_from_file(&hook_file).await;
    
    // Should detect and reject dangerous commands
    assert!(load_result.is_err(), "Should reject hooks with dangerous commands");
...
"#;
    ...
    tokio::fs::write(&injection_file, injection_hook_config).await.unwrap();
...
"#;
    ...
    tokio::fs::write(&resource_file, resource_intensive_hook).await.unwrap();
...
async fn test_input_validation() {
    ...
    for malicious_input in malicious_inputs {
        ...
        let db = hive_ai::core::database::HiveDatabase::new(&db_path).await.unwrap();
        ...
        match search_result {
            Ok(_) => {
                ...
                println!("✅ SQL injection attempt safely handled: {}", malicious_input);
            }
            Err(e) => {
    ...
    for malicious_filename in malicious_filenames {
        ...
        assert!(read_result.is_err(), 
               "Should safely fail for malicious filename: {}", malicious_filename);
        
        println!("✅ Command injection in filename safely handled: {}", malicious_filename);
    }
    ...
    for malicious_code in malicious_code_inputs {
        ...
        println!("✅ Malicious code safely contained in context: {}", malicious_code);
    }
...
"#;
    ...
    tokio::fs::write(&config_file, malicious_config).await.unwrap();
    ...
    match config_result {
        Ok(config) => {
            ...
            assert!(!config_str.contains("sk-or-"), 
                   "API keys should not be exposed in debug output");
            
            // Should not allow insecure configurations
            assert!(!config.hooks.as_ref().map_or(false, |h| h.allow_dangerous_operations.unwrap_or(false)),
...
async fn test_network_security() {
    ...
    for insecure_url in insecure_urls {
        ...
        match request_result {
            Ok(_) => {
                println!("⚠️  Insecure URL allowed (should be validated at runtime): {}", insecure_url);
            }
            Err(_) => {
...
async fn test_comprehensive_security_audit() {
    ...
    let test_results = vec![
        ("File System Access Controls", test_filesystem_access_controls().await),
        ("Trust System Security", test_trust_system_security().await),
        ("Hook Execution Security", test_hook_execution_security().await),
        ("Input Validation", test_input_validation().await),
        ("Configuration Security", test_configuration_security().await),
    ...
    assert_eq!(vulnerabilities_found, 0, "Security vulnerabilities must be addressed before release");
...
async fn test_security_monitoring() {
    ...
    security_manager.log_security_event("trust_violation", 
                                       &serde_json::json!({
    ...
    match metrics {
        Ok(metrics) => {
            ...
            assert!(metrics.contains_key("total_security_events") || 
                   metrics.is_empty(), "Should track security metrics");
        }
        Err(e) => {
...
async fn test_security_performance_impact() {
    ...
    assert!(avg_duration < std::time::Duration::from_millis(5),
