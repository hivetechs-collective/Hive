...
async fn create_mock_typescript_database(path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sample_conversations = vec![
        ...
        TypeScriptConversation {
            ...
            tags: vec!["ai".to_string(), "ethics".to_string(), "philosophy".to_string()],
...
async fn test_database_schema_migration() {
    ...
    let migration_result = migration_engine.migrate_from_typescript(&ts_db_path, &rust_db_path).await;
    ...
    match migration_result {
        Ok(migration_stats) => {
            ...
            assert!(schema_valid.is_ok(), "Migrated database should have valid schema");
...
async fn test_migration_data_integrity() {
    ...
    // Perform migration
    let migration_engine = MigrationEngine::new();
    migration_engine.migrate_from_typescript(&ts_db_path, &rust_db_path).await.unwrap();
    ...
    let conversations = rust_db.list_conversations(None, None).await.unwrap();
    assert_eq!(conversations.len(), original_data.conversation_count,
    ...
    for original_conv in &original_data.conversations {
        ...
        assert!(migrated_conv.is_some(), "Conversation {} should exist", original_conv.id);
        
        let migrated_conv = migrated_conv.unwrap();
        
        assert_eq!(migrated_conv.title, original_conv.title, "Title should match");
        assert_eq!(migrated_conv.messages.len(), original_conv.messages.len(), 
                   "Message count should match");
        assert_eq!(migrated_conv.tags, original_conv.tags, "Tags should match");
        assert_eq!(migrated_conv.quality_score, original_conv.quality_score, 
                   "Quality score should match");
        assert_eq!(migrated_conv.token_count, original_conv.token_count, 
                   "Token count should match");
        assert_eq!(migrated_conv.summary, original_conv.summary, 
                   "Summary should match");
    }
    
    // Test 3: Memory cluster migration
    let clusters = rust_db.list_memory_clusters().await.unwrap();
    assert_eq!(clusters.len(), original_data.cluster_count,
    ...
    assert_eq!(preferences.len(), original_data.preference_count,
...
async fn test_migration_rollback() {
    ...
    let rollback_result = migration_engine.rollback_migration(&backup_path, &ts_db_path).await;
    ...
    match rollback_result {
        Ok(()) => {
            ...
            assert_eq!(original_stats.conversation_count, 2, 
...
async fn test_incremental_migration() {
    ...
    let incremental_result = migration_engine.incremental_migrate(&ts_db_path, &rust_db_path).await;
    ...
    match incremental_result {
        Ok(stats) => {
            ...
            assert!(conversations.len() >= 2, "Should have at least original conversations");
...
async fn test_migration_performance() {
    ...
    create_large_typescript_database(&ts_db_path, 1000).await.unwrap();
    
    // Measure migration time
    let start = std::time::Instant::now();
    
    let migration_engine = MigrationEngine::new();
    let result = migration_engine.migrate_from_typescript(&ts_db_path, &rust_db_path).await;
    
    let duration = start.elapsed();
    
    match result {
        Ok(stats) => {
            println!("Migration of {} conversations took: {:?}", stats.conversations_migrated, duration);
            
            // Migration should be reasonably fast
            let conversations_per_second = stats.conversations_migrated as f64 / duration.as_secs_f64();
            println!("Migration rate: {:.1} conversations/second", conversations_per_second);
            
            // Should migrate at least 100 conversations per second
            assert!(conversations_per_second > 100.0, 
                   "Migration should be efficient: {} conv/sec", conversations_per_second);
            
            // Verify all data was migrated
            let rust_db = HiveDatabase::new(&rust_db_path).await.unwrap();
            let migrated_count = rust_db.list_conversations(None, None).await.unwrap().len();
            assert_eq!(migrated_count, stats.conversations_migrated as usize,
...
async fn test_migration_error_handling() {
    ...
    create_corrupted_typescript_database(&ts_db_path).await.unwrap();
    ...
    match result {
        Ok(stats) => {
            ...
            assert!(stats.errors_encountered > 0, "Should report errors encountered");
            assert!(stats.conversations_migrated >= 0, "Should migrate valid data");
...
async fn test_migration_compatibility() {
    ...
    let migration_engine = MigrationEngine::new();
    let compatibility_check = migration_engine.check_migration_compatibility(&ts_db_path).await;
    
    match compatibility_check {
        Ok(compatibility_report) => {
            println!("Compatibility check passed: {:?}", compatibility_report);
            
            assert!(compatibility_report.is_compatible, "Database should be compatible");
            assert!(compatibility_report.warnings.len() <= 5, "Should have minimal warnings");
            
            if !compatibility_report.warnings.is_empty() {
                println!("Compatibility warnings: {:?}", compatibility_report.warnings);
            }
        }
        Err(e) => {
    ...
    let version_check = migration_engine.check_version_compatibility(&ts_db_path).await;
    
    match version_check {
        Ok(version_info) => {
            println!("Version compatibility: {:?}", version_info);
            
            assert!(!version_info.source_version.is_empty(), "Should detect source version");
...
async fn get_typescript_database_stats(path: &PathBuf) -> Result<TypeScriptDatabaseStats, Box<dyn std::error::Error>> {
    ...
    let preference_count: i64 = conn.query_row(
...
async fn create_large_typescript_database(path: &PathBuf, count: usize) -> Result<(), Box<dyn std::error::Error>> {
    use rusqlite::{Connection, params};
...
async fn create_corrupted_typescript_database(path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    use rusqlite::{Connection, params};
