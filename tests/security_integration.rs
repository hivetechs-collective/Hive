...
use hive_ai::core::{
    initialize_security, get_security_context, SecurityContext, 
    TrustLevel, check_read_access, check_write_access
};
...
async fn test_temporary_trust() {
    ...
    // Set temporary trust
    context.set_trust_level(&test_path, TrustLevel::Temporary, Some("Temporary access".to_string())).unwrap();
...
async fn test_security_event_logging() {
    ...
    context.set_trust_level(&test_path, TrustLevel::Trusted, Some("Test logging".to_string())).unwrap();
    ...
    let has_trust_decision = events.iter().any(|e| {
        matches!(e.event_type, hive_ai::core::SecurityEventType::TrustDecision)
    });
...
async fn test_trusted_paths_listing() {
    ...
    context.set_trust_level(&path2, TrustLevel::Trusted, None).unwrap();
    context.set_trust_level(&path3, TrustLevel::Temporary, None).unwrap();
    ...
    // Check trust levels
    let permanent_count = trusted.iter().filter(|(_, level)| *level == TrustLevel::Trusted).count();
    let temporary_count = trusted.iter().filter(|(_, level)| *level == TrustLevel::Temporary).count();
