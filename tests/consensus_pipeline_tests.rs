...
use hive_ai::consensus::{
    ConsensusEngine, ConsensusPipeline, TemporalContextProvider,
    ConsensusConfig, ConsensusProfile,
    streaming::{ConsoleCallbacks, ConsensusMetrics, EnhancedProgressTracker},
    stages::{GeneratorStage, RefinerStage, ValidatorStage, CuratorStage, ConsensusStage},
    types::{Stage, RetryPolicy, ContextInjectionStrategy},
};
...
async fn test_basic_consensus_pipeline() -> Result<()> {
    ...
    let stage_names: Vec<&str> = result.stages.iter()
...
async fn test_refiner_stage_quality_analysis() -> Result<()> {
    ...
    assert_eq!(refiner.detect_response_type(explanation_response), "explanation");
    ...
    let vague_response = "This might possibly work, maybe you could try this approach, perhaps it might help.";
...
async fn test_curator_stage_formatting() -> Result<()> {
    ...
    assert_eq!(curator.analyze_content_type(explanation_content), "explanation");
...
async fn test_streaming_progress_tracking() -> Result<()> {
    let callbacks = Arc::new(ConsoleCallbacks { show_progress: false });
...
async fn test_consensus_performance() -> Result<()> {
    ...
    let handles: Vec<_> = (0..5).map(|i| {
        ...
        tokio::spawn(async move {
            engine_clone.process(&format!("Test question {}", i), None).await
        })
    }).collect();
...
async fn test_context_integration() -> Result<()> {
    ...
    let result = engine.process("What does this code do?", Some(semantic_context.to_string())).await?;
...
async fn test_complete_consensus_workflow() -> Result<()> {
    ...
    let code_context = "```rust\nfn risky_operation() -> String {\n    \"result\".to_string()\n}\n```";
    let result = engine.process(code_query, Some(code_context.to_string())).await?;
