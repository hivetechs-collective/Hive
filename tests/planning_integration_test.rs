...
async fn test_planning_engine_creation() {
    ...
    let planning_engine = PlanningEngine::new(std::sync::Arc::new(consensus_engine))
        .await
        .unwrap();
...
async fn test_mode_detection() {
    ...
    let mode = planning_engine
        .detect_mode("Fix the bug in the login function", &context)
        .unwrap();
...
async fn test_timeline_estimation() {
    ...
    // Estimate timeline
    let timeline = timeline_estimator
